/-
# Spectral Gap Positivity Theorem (PF anchor)

Self-contained copy of the canonical spectral gap theorem, so that the
`PF` library can depend on it without importing a separate top-level
module. The content matches `SpectralGap.lean` in this project.
-/
