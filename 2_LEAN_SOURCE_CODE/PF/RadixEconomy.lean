/-
# Base-3 Radix Economy Theorem (PF anchor)

Self-contained copy of the canonical radix economy theorem, so that the
`PF` library can depend on it without importing a separate top-level
module. The content matches `RadixEconomy.lean` in this project.
-/
