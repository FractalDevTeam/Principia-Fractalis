...
| 2 | `cfcₙ f a` lands in the closed two-sided ideal generated by `a` (for `f 0 = 0`), via Stone–Weierstrass approximation by constant-free polynomials | A2 | queued |
| 3 | Spectral projections for **clopen** spectral sets: indicator of a clopen subset of the spectrum is continuous on the spectrum, so `cfc` yields a projection; API for the resulting `p = 1_S(a)` | B1 (partial) | queued |
