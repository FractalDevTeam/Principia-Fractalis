...
| 3 | Projection-in-ideal via cfcₙ: for a **clopen** spectral set `U ⊆ σₙ(a)` with `0 ∉ U`, the indicator `χ_U ∈ C(σₙ,ℝ)₀`, so `cfcₙ χ_U a` is a projection (`IsIdempotentElem` + `IsSelfAdjoint` from `χ_U²=χ_U`, `star χ_U=χ_U`) **landing in `I`** by PR-2. Direct WALL-A2 discharge. (Refined plan from PR-2 recon: use cfcₙ not unital cfc, so `f 0 = 0` keeps the projection in the ideal.) | B1 (partial) | queued — next |
