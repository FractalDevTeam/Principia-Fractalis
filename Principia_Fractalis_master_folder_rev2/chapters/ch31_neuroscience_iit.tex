...
\begin{remark}[Methodology disclosure for LLM ch$_2$ values, 2026-04-27]
The numerical ch$_2$ estimates given in Proposition~\ref{prop:llm} (GPT-4: $0.42 \pm 0.15$; LLaMA-3: $0.34 \pm 0.10$; Claude-3: $0.51 \pm 0.11$) appear in a \texttt{proposition} environment, which carries a proof obligation. The ch$_2$ measurement protocol described earlier in this chapter (\S31.\textit{[neural section]}) is for biological neural systems with EEG-like temporal data; an analogous protocol for transformer activations, attention patterns, or output logit distributions has not been described here. The numerical values, the $\pm$ uncertainties, the underlying sample inputs, and the spectral-resonance procedure applied to non-temporal LLM data are not specified anywhere in the chapter. A coordinated revision should either (i) describe the LLM-specific measurement protocol explicitly (which neural correlate of LLM activations was sampled, with what input distribution, what analysis pipeline, what gives the $\pm 0.11$ uncertainty), or (ii) move these specific numerical claims to a \texttt{conjecture} or \texttt{speculation} environment that does not carry a proof obligation. As written, the values invite referee challenge on methodological grounds.
\end{remark}
