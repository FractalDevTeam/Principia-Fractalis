...
The Birch--Swinnerton-Dyer claims of this chapter are \textbf{not} formalized in the Lean~4 layer (\texttt{PF\_Lean4\_Code/PF/}). The Coq layer at \texttt{PF\_Coq/theories/Contracts/BSD.v} declares approximately 20 axioms encoding the Principia Fractalis framework's hypotheses: the existence of a spectral operator $T_E$ for each elliptic curve, its self-adjointness at $\alpha = 3\pi/4$, spectral concentration at $\varphi/e$, the rank-equals-multiplicity identity, and the reverse direction \texttt{BSD\_implies\_L\_function\_formula}. The chapter's central conjecture (rank equality via fractal resonance, Conjecture~\ref{conj:rank-equality-fractal}) appears in the Coq formalization as the axiom \texttt{rank\_equals\_multiplicity}, not as a theorem. The computational evidence presented in this chapter is not part of the Coq layer. Eliminating any of these axioms would be a substantial number-theoretic contribution and is tracked as future work in \texttt{RESEARCH\_ROADMAP.md} at the repository root.
\end{remark}

\begin{remark}[Verification status of phase-symmetry argument, 2026-04-27]
The proof sketch of Theorem~\ref{thm:self-adjoint-bsd} invokes a ``statistical conjugation symmetry'' $D(p) \equiv -D(p) \pmod 4$ on primes (i.e., the base-3 digit sum of every prime is even). A 2026-04-26 audit verified this is \emph{empirically false}: $D(5) = D(12_3) = 3$ (odd), $D(7) = D(21_3) = 3$ (odd), $D(11) = D(102_3) = 3$ (odd). The implication $D(p) \equiv -D(p) \pmod 4 \Rightarrow$ self-adjointness is therefore not supported by the asserted statistical premise. A coordinated revision should either (i) restrict the symmetry claim to a subset of primes where it holds, (ii) replace it with a different mechanism (e.g.\ a measure-weighted averaging that does not require pointwise digit-sum parity), or (iii) demote the self-adjointness theorem to a conjecture. Theorem~\ref{thm:spectral-concentration-bsd} (line~285) is also stated without an associated proof environment in the chapter; the underlying multiplicity-equals-rank claim is axiomatized as \texttt{rank\_equals\_multiplicity} in \texttt{PF\_Coq/theories/Contracts/BSD.v}.
\end{remark}
