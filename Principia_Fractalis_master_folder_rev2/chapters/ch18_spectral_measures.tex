...
\begin{remark}[Wave 55 audit — Table 18.1 vs.\ Thm 18.9 vs.\ Lean Ch2$\Phi$Bridge; 2026-06-01]
The Wave 55 Geometric Unity / QFT-Consciousness audit (\texttt{MISSION\_INVENTORY/wave55\_geomQFT\_chapter\_audit.md}, \S4 Inconsistency 6) records that the numerical pairs in Table~\ref{tab:phi-ch2-correlation} are simultaneously inconsistent with \emph{both} the power-law form $\Phi = k\cdot[\text{ch}_2]^\alpha$ ($\alpha\approx 1.2$) given downstream in Thm 18.9, and with the saturation form proved axiom-free in the Lean carrier \texttt{PF\_Lean4\_Code/PF/Consciousness/Ch2PhiBridge.lean} / \texttt{Ch2PhiBridgeDischarge.lean} (commit \texttt{d599936}), namely
\[
\text{ch}_2 \;\le\; 1 - \exp(-\Phi_{\text{IIT}}/2)
\]
with equality on uniform-Schmidt states. Under the Lean form the table entries should be $\text{ch}_2(\Phi=3.5) \approx 0.826$, $\text{ch}_2(2.1) \approx 0.650$, $\text{ch}_2(2.8) \approx 0.753$, $\text{ch}_2(0.8) \approx 0.330$, $\text{ch}_2(0.4) \approx 0.181$, $\text{ch}_2(0.1) \approx 0.049$ — only the $(0.8, 0.35)$ and $(0.1, 0.05)$ rows match within 10\%. The \textbf{Lean-anchored corrected form} from \texttt{Ch2PhiBridgeDischarge} ($\text{ch}_2 \le 1 - \exp(-\Phi_{\text{IIT}}/2)$, equality on uniform-Schmidt) supersedes both the bare Table~\ref{tab:phi-ch2-correlation} entries and the power-law form of Thm 18.9 until the manuscript reconciles the two.
\end{remark}
