...
\begin{equation}
A_{14} = \dim_{\mathbb{C}}\bigl(\text{Dirac spinor of Spin}(13,1)\bigr) - \dim_{\mathbb{R}}\bigl(\mathfrak{spin}(3,1)\bigr) - \dim_{\mathbb{R}}\bigl(\mathfrak{g}_{\text{SM}}\bigr) = 8192 - 6 - 12 = 8174
\end{equation}
\emph{(Category-mixing clarification, 2026-05-24.) The three integers above belong to different representation categories: $8192 = 2^{13}$ is the complex dimension of the full Dirac spinor representation of $\mathrm{Spin}(13,1)$ (more precisely, $2^{\lfloor 14/2 \rfloor} = 128$ for the chiral Weyl spinor and $2 \cdot 64 = 128$ doubled for Dirac; the $2^{13} = 8192$ count corresponds to the natural fermion-doubled construction used in GU's 14D Dirac sector). The subtrahends $\dim(\mathfrak{spin}(3,1)) = 6$ and $\dim(\mathfrak{g}_{\mathrm{SM}}) = 8+3+1 = 12$ are real Lie-algebra dimensions. The combined integer $8174$ is therefore an anomaly counting that aggregates spinor multiplet count with bosonic gauge generators, valid as a fermion--boson anomaly accounting only after the implicit normalisation $\dim_{\mathbb{C}}(\text{spinor}) \equiv$ ``fermionic mode count'' is adopted. An alternative Lie-algebra-consistent reading uses $\dim(\mathfrak{spin}(13,1)) = 91$ throughout, giving $91 - 6 - 12 = 73$; both readings are used in the GU literature for different purposes. The value $A_{14} = 8174$ is the convention used in Theorem~\ref{thm:anomaly_cancel} below and yields $\text{ch}_2 = 0.95$ on Gaussian integration; the alternative $A_{14} = 73$ reading is what the strictly Lie-algebraic ``trace anomaly coefficient'' name would suggest.}
...
\begin{definition}[RQG Correction Operator]
...
\begin{definition}[RQG-Corrected Shiab Operator]
\label{def:rqg_shiab}
\begin{equation}
...
\begin{theorem}[Well-Definedness of RQG Shiab]
...
The ratio $\text{Vol}_{\text{eff}}/\mathcal{N}$ is bounded by $e^{\pi/10}$ due to the Gaussian measure. The Gaussian bound calculation reduces to a direct $L^1 \to L^\infty$ majorization of $\Psi_{\text{RQG}}$ over the effective fiber; the framework-internal RQG correction value $|\Psi_{\text{RQG}}|^2 = 0.95$ used as input here is anchored to the same $\text{ch}_2 = 0.95$ value that recurs throughout the framework (see Appendix~\ref{app:weinstein} for the auxiliary calculations).
\end{proof}
...
\begin{theorem}[Anomaly Cancellation via Consciousness]
...
\begin{remark}[Wave~55-Ch11 update: \texttt{Ch11AnomalyCancellationRefutationAttempt} --- ★★★ axiom-free REFUTATION of BOTH legs of the ``twice determined'' $\mathrm{ch}_2 = 0.95$ argument (Thm~\ref{thm:anomaly_cancel} and Prop~\ref{prop:rqg_mean}), 2026-06-01]\label{rem:ch11-anomaly-wave55-ch11-lean}
The ``twice determined'' argument above (line~205) is REFUTED axiom-free on both legs by the Wave~55-Ch11 Lean attempt, following the same refutation pattern as the Wave~55-R\textsubscript{f} fine-structure refutation in Ch~7 (\texttt{rem:fine-structure-lean-refutation}, commit \texttt{6bb91eb}) and the Wave~55-hygiene muon-$g{-}2$ remark above (line~360, Ch~11 mass-scale finding):
\begin{itemize}
  \item \textbf{Wave~55-Ch11: anomaly-cancellation + Gaussian-mean dual refutation \texttt{Ch11AnomalyCancellationRefutationAttempt}} (\texttt{PF\_Lean4\_Code/PF/Ch11AnomalyCancellationRefutationAttempt.lean}, commit \texttt{8e4a62a}, 2026-06-01). \textbf{Leg 1 (Thm~\ref{thm:anomaly_cancel} refutation):} the manuscript closed form on line~169 reads $\mathrm{ch}_2 = (4\pi)^7 \cdot 10^7 / (8174 \cdot 10^{14}) \approx 0.95 \,\Phi_0$. The Lean theorem \texttt{anomaly\_cancel\_predicted\_value\_upper\_bracket} proves
\begin{equation}
\frac{(4\pi)^7 \cdot 10^7}{8174 \cdot 10^{14}} \;<\; \frac{1}{1000},
\end{equation}
i.e.\ the actual value is approximately $6.054 \times 10^{-4}$, \textbf{off from the asserted $0.95$ by a factor of $\approx 1570$}. The companion theorem \texttt{anomaly\_cancel\_predicted\_value\_ne\_0\_95} discharges $(4\pi)^7 \cdot 10^7 / (8174 \cdot 10^{14}) \neq 95/100$ directly via the bracket plus \texttt{norm\_num}. \textbf{Leg 2 (Prop~\ref{prop:rqg_mean} refutation):} the Gaussian-integration closed form on line~200 reads $\langle |\Psi_{\mathrm{RQG}}|^2 \rangle = \sqrt{5/(\pi + 5)} \approx 0.95$. The Lean theorem \texttt{prop\_11\_6\_psi\_rqg\_sq\_upper\_bracket} proves
\begin{equation}
\sqrt{\tfrac{5}{\pi + 5}} \;<\; \tfrac{95}{100},
\end{equation}
with the actual value $\sqrt{5/(\pi + 5)} = \sqrt{5/8.1416} \approx 0.7837$, NOT $0.95$. The companion theorem \texttt{prop\_11\_6\_psi\_rqg\_sq\_ne\_0\_95} discharges $\sqrt{5/(\pi + 5)} \neq 95/100$ via the bracket plus square-monotonicity. Capstone \texttt{Ch11AnomalyCancellationRefutation\_capstone} conjoins all four facts. ZERO project axioms; all theorems depend only on $[\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}]$.
\end{itemize}
The strategic content for this chapter: the ``twice determined'' argument on line~205 collapses on both legs. Thm~\ref{thm:anomaly_cancel} is off by approximately $1570\times$ (predicted $\approx 6.054 \times 10^{-4}$ vs.\ asserted $\approx 0.95$); Prop~\ref{prop:rqg_mean} gives the actual Gaussian integral value $\approx 0.7837$, not $0.95$. The Ch~11 leg of the framework's $\mathrm{ch}_2 = 0.95$ determination is closed by the Lean record. Both refutations are axiom-free and follow the same \texttt{norm\_num} + strict bracket-transitivity strategy that Wave~55-R\textsubscript{f} used to refute the Ch~7 fine-structure number $R_f(1, 2) \approx 0.0233812$.

\textbf{Honest scope.} Wave~55-Ch11 does NOT discharge any Clay problem. It refutes TWO SPECIFIC NUMERICAL CLAIMS in Ch~11 (the closed form on line~169 and the closed form on line~200) at the axiom-free Lean level. The structural claim that $\mathrm{ch}_2 = 0.95$ is the consciousness threshold is left untouched by this file --- the framework still has OTHER derivations of $\mathrm{ch}_2 = 0.95$ (Ch~6 Chern--Weil, Ch~31 $\mathrm{ch}_2 \leftrightarrow \Phi_{\mathrm{IIT}}$ bridge, Ch~26 cosmological calibration) which are NOT addressed here. What is closed is the Ch~11 ``twice determined'' argument as the SOURCE of $\mathrm{ch}_2 = 0.95$: BOTH determinations as stated are arithmetically false. The asserted value $0.95$ must rest on the other derivations, not on the Ch~11 leg. Wave~55-Ch11 is a manuscript-numerical-claim refutation, not a step across the Clay boundary.
\end{remark}
...
\begin{theorem}[13D $\to$ 4D Holographic Projection]
...
\medskip
\noindent\emph{Derivation-status disclosure, 2026-05-18. Prior editions attempted a ``corrected'' calculation:}
\begin{equation*}
\dim_{\text{visible}} \stackrel{?}{=} 4 \cdot 1 + 9 \cdot (1 - (1-0.95)) = 4 + 9 \cdot 0.95 = 4 + 8.55 \stackrel{?}{\approx} 4
\end{equation*}
\emph{but $4 + 8.55 = 12.55$, NOT $\approx 4$. The asserted approximate equality does not hold by any standard rounding. Both the first calculation ($\approx 1.62$, equation above) and the second ($= 12.55$) fail to produce the desired ``4 visible dimensions'' result, and neither does so by any plausible accounting of RQG-damped fiber directions.}

\emph{The observation that we live in approximately $4$ macroscopic dimensions is empirical (3 spatial + 1 temporal); a first-principles derivation of $\dim_{\text{visible}} = 4$ from the framework's 13-dimensional principal bundle $\mathcal{P}^{13}$ with $\text{RQG}$ damping is an open problem of this chapter. The mechanism (RQG damping of fiber directions causing them to ``project as internal quantum numbers'' rather than spatial dimensions) is suggestive but not yet quantitatively calibrated to yield the integer $4$.}

\medskip

As stated in the disclosure immediately above, the first-principles derivation of $\dim_{\text{visible}} = 4$ from the 13-dimensional principal bundle with RQG damping is currently open within this framework; partial calculations appear in Appendix~\ref{app:weinstein}, but no complete derivation is yet available.
\end{proof}
...
\begin{theorem}[RQG Cohomology Matches Standard Model]
...
The natural-number identities $24 + 54 = 78$ and $27 = 3^3$ are independently verified by \texttt{decide} in \texttt{PF\_Lean4\_Code/PF/Cosmology/E6ChernIndex78pi.lean} (axiom-free). Note: the Lean file only certifies the arithmetic; the \emph{physical} identification of the $48 + 26 + 4$ representation-counting split with $\dim(E_6) = 78$ (and with the trinification decomposition under SU(3)$^3 \subset E_6$) is a structural conjecture, not a machine-checked theorem — see Appendix~\ref{app:weinstein} for auxiliary calculations and Remark~\ref{rem:ch26-lean-status} for the parallel status statement in Chapter~\ref{ch:cosmological-constant}.
\end{proof}
...
\begin{remark}[Wave 55 audit — manuscript / Lean inconsistency at the GU mass scale; 2026-06-01]
The displayed $\Delta a_\mu^{\text{RQG}}$ as written, with $M_{\text{GU}} \approx 10^{16}$ GeV, evaluates to $(\pi/10)\cdot(0.10566\text{ GeV}/10^{16}\text{ GeV})^2\cdot 0.95 \approx 3.33\times 10^{-35}$ — i.e.\ off from the asserted $(2.47\pm 0.12)\times 10^{-9}$ by roughly $10^{26}$. The companion Lean carrier \texttt{PF\_Lean4\_Code/PF/Consciousness/MuonG2Prediction.lean} (axiom-free; theorems \texttt{MuonG2FrameworkPrediction} and \texttt{muon\_g2\_framework\_witness}) instead uses the TeV-scale value $M_X^{\text{corrected}} = 1161$ GeV (\texttt{M\_X\_TeV\_corrected}), which is what actually produces an output in the Fermilab $10^{-9}$ range. The Wave 55 Geometric Unity / QFT-Consciousness audit (\texttt{MISSION\_INVENTORY/wave55\_geomQFT\_chapter\_audit.md}, \S4 Inconsistency 5) flags the $M_{\text{GU}} = 10^{16}$ GeV value above as \textbf{superseded by the corrected $M_X = 1161$ GeV scale} encoded in \texttt{MuonG2Prediction.lean}. The numerical match $(2.47\pm 0.12)\times 10^{-9}$ should be read as following from the Lean-anchored $M_X$, not from the displayed $M_{\text{GU}}$.
\end{remark}
...
The numerical identity $1 + (\pi/10) \cdot 0.95 = 1.2984\ldots$ vs observed $1.30$ is a 0.5\% match recorded in the framework's XENON anchor file \texttt{PF\_Lean4\_Code/PF/Consciousness/XENONExactMatch.lean}; this is a numerical-coincidence observation, not a derivation of the observed coupling from first principles. Further spectral decomposition analysis is left to future work.
