...
\textbf{Status (rev 2)}: The closed-form identities $\lambda_0(H_P) = \pi/(10\sqrt{2})$ and $\lambda_0(H_{NP}) = \pi/(10(\varphi + 1/4))$ are now machine-checked Lean~4 theorems to 10-digit precision (\texttt{lambda\_0\_P\_precise}, \texttt{lambda\_0\_NP\_precise}), as is the fractal-dimension separation $\sqrt{2} < \varphi + 1/4$ (\texttt{alpha\_sep\_greek}) and the spectral-gap positivity (\texttt{spectral\_gap\_positive}). Three BOOK-CORE claims remain as named Lean axioms --- the operator--complexity bridge (\texttt{turingTimeComplexity}, \texttt{p\_eq\_np\_spectrum\_collapse}, \texttt{operator\_collapse\_hypothesis}) --- under which the main chain $\mathrm{ClassP} = \mathrm{ClassNP} \Rightarrow \lambda_0(H_P) = \lambda_0(H_{NP})$ becomes a theorem, delivering P $\neq$ NP conditional on those three. See the remark immediately after Conjecture~\ref{thm:main-p-neq-np} and \texttt{AXIOM\_AUDIT.md} at the repository root for details. The polylogarithm-modulation conjecture and the fractal-analytic-continuation mechanism remain genuinely open and are not part of the 8 remaining Lean axioms.
...
\emph{Conditional theorem status (rev 2)}: the Lean~4 formalization makes the
chain $\mathrm{ClassP} = \mathrm{ClassNP} \Rightarrow \lambda_0(H_P) = \lambda_0(H_{NP})$
a direct consequence of three named axioms (\texttt{operator\_collapse\_hypothesis},
\texttt{p\_eq\_np\_spectrum\_collapse}, \texttt{turingTimeComplexity}).
Combined with the machine-checked theorem \texttt{P\_neq\_NP\_from\_spectral\_gap}
(which uses only the ALREADY-proven \texttt{spectral\_gap\_positive}
$\lambda_0(H_P) - \lambda_0(H_{NP}) > 0$), the chain delivers P $\neq$ NP
\emph{conditional on} the three axioms. The remark below enumerates which
claims are thereby machine-checked theorems and which are axiomatized.
\end{conjecture}

\begin{remark}[Formalization status, rev 2]
The Lean 4 formalization of this chapter's main argument is complete modulo three named axioms, each tagged \textbf{BOOK-CORE} in the repository's \texttt{AXIOM\_AUDIT.md}:
\begin{itemize}
\item \texttt{operator\_collapse\_hypothesis} (\texttt{PF/P\_NP\_Complete\_Proof.lean:175}): formalizes the bridge $(\forall L\in\mathrm{NP},\ L\in\mathrm{P}) \Rightarrow \alpha_{\mathrm{NP}} = \alpha_{\mathrm{P}}$ —  that is, if certificates become redundant, the spectral coefficients collapse.
\item \texttt{p\_eq\_np\_spectrum\_collapse} (\texttt{PF/TuringEncoding/Operators.lean:191}): the derived implication $\mathrm{ClassP} = \mathrm{ClassNP} \Rightarrow \lambda_0(H_P) = \lambda_0(H_{NP})$.
\item \texttt{turingTimeComplexity} (\texttt{PF/TuringEncoding/Complexity.lean:57}): the time-cost function signature for TM2 machines, retained as an abstract signature rather than a concrete step-counter (defining it as a constant would make $\mathrm{ClassP} = \mathrm{ClassNP}$ provable and destroy the whole argument; the correct path is to parameterize \texttt{InClassP}/\texttt{InClassNP} over an abstract time-cost function or to construct the counter from \texttt{TM2}'s stepping semantics).
\end{itemize}
Everything \emph{downstream} of these three axioms is machine-checked: the fact that $\alpha_{\mathrm{NP}} > \alpha_{\mathrm{P}}$ (\texttt{alpha\_sep\_greek}), that $\lambda_0(H_P) > \lambda_0(H_{NP})$ (\texttt{spectral\_gap\_positive}), that a strict spectral gap implies \textbf{not} $\mathrm{ClassP} = \mathrm{ClassNP}$ (\texttt{P\_neq\_NP\_from\_spectral\_gap}), and that the closed-form values $\pi/(10\sqrt{2})$ and $\pi/(10(\varphi + 1/4))$ match to $10^{-10}$ precision (\texttt{lambda\_0\_P\_precise}, \texttt{lambda\_0\_NP\_precise}) are all Lean 4 theorems in the current revision.
\end{remark}
