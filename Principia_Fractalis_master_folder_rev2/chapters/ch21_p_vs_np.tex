...
\begin{remark}[Verification status of $\alpha_{NP} = \varphi + 1/4$ derivation, 2026-04-27]
The closed-form values $\alpha_P = \sqrt{2}$ and $\alpha_{NP} = \varphi + 1/4$ are matched numerically to high precision by the spectral computations cited above, but the author's audit document \texttt{DERIVATION\_ANALYSIS\_alpha\_NP.md} (2025-11-30, repository root) explicitly catalogs that the \emph{first-principles derivation} of these values is incomplete in the manuscript text. Quoting the audit's executive summary: ``The claimed value $\alpha_{NP} = \varphi + 1/4$ is \textbf{ASSERTED but NOT RIGOROUSLY DERIVED} in Principia Fractalis. \dots The value $\alpha_{NP} = \varphi + 1/4$ is a CONJECTURE supported by numerical evidence, not a derived result.'' Specifically, four gaps are identified: no explicit derivation of the modified generating function $G_{NP}(z)$, the reality condition is not solved from first principles, the $1/4$ correction has no derivation, and the $\varphi$-from-certificate-trees connection is heuristic. The author's companion derivation script \texttt{Evidence\_and\_Data\_for\_GitHub/alpha\_sqrt2\_derivation.py} (also 2025-11-30) uses the finite generating function $(1+z+z^2)^N$ rather than the divergent infinite product transcribed at line~286 above; aligning the chapter text with the author's derivation script is one of the pending revision tasks. The Lean 4 formalization above is unaffected (the spectral gap positivity is established arithmetically, not via the closed-form derivation under question), but a referee reading this chapter will need the author's audit catalog as canonical context.
\end{remark}
