...
\textbf{Formal verification (rev 2).} The monotonicity lemma $Q[b] \geq Q[b+1]$ for all integer $b \geq 4$ has been machine-checked in Lean 4, discharging what was previously an axiom in \texttt{PF/IntervalArithmetic.lean} (name: \texttt{Q\_decreasing\_from\_4}). The proof applies Mathlib's \texttt{Real.log\_div\_self\_antitoneOn}, which establishes antitonicity of $x \mapsto (\log x)/x$ on $\{x \mid \exp 1 \leq x\}$, combined with $\exp 1 < 2.7182818286 < 4$ from \texttt{Real.exp\_one\_lt\_d9}.
\end{proof}
