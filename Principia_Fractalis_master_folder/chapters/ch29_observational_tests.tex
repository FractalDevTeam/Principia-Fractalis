...
\subsection{The Standard Model: \texorpdfstring{$\Lambda$}{Lambda}CDM}
...
\subsection{\texorpdfstring{$H(z)$}{H(z)} vs \texorpdfstring{$d_A(z)$}{d\_A(z)}}
...
\empiricalclaim{Combined SNe Ia (Union2.1 / Pantheon
\cite{suzuki2012,scolnic2018}), SDSS BAO \cite{sdss2018}, and
Planck 2018 CMB \cite{planck2018}; $\chi^2_{\Lambda\text{CDM}}
= 687.3$, $\chi^2_{\text{mod}} = 354.2$, $\Delta\chi^2 = 333.1$
across 588 d.o.f. The Lean stack does NOT verify the
$\chi^2$-improvement number; it verifies only the dark-energy
density bracket (theorem
...
Referees seeking to reproduce the 94.3\% figure must rerun the
$\chi^2$ analysis against the primary catalogs.}
\end{theorem}
...
\noindent\emph{Honest scope (2026-07-23): the parameter values in Table~\ref{tab:parameter-constraints}, and the ``match'' between the fitted $f_{\mathcal{C}},z_*$ and their predicted values, are \textbf{empirical results not independently reproduced} and \textbf{not machine-verified}. They come from the same in-house analysis that produced the $\chi^2$ numbers; an independent MCMC on the primary catalogs (Research Problem~1) has not been carried out here.}
...
Held to the standard of the machine-checked corpus, the evidentiary claims of this
chapter are empirical and, as presented, not independently reproduced:
\begin{itemize}
\item \textbf{EMPIRICAL --- not machine-verified, not independently reproduced} ---
  the headline $94.3\%$ / $\Delta\chi^2=333.1$ improvement over $\Lambda$CDM
  (Thm~\ref{thm:global-fit}) and every component $\chi^2$, F-test, BIC, and
  $p<10^{-50}$ figure. As the \texttt{empiricalclaim} note records, the Lean stack
  verifies \emph{only} the dark-energy density bracket
  (\texttt{darkEnergyDensity\_in\_bracket}); the goodness-of-fit numbers must be
  rerun against the primary Pantheon/SDSS/Planck data (Research Problem~1) to be
  trusted.
\item \textbf{DEFINITIONAL, not predictive} --- the recovered
  $H_0=69.82$, $\Omega_\Lambda\approx0.68$, and related constants. Where the corpus
  ``confirms'' these it does so through bracket inequalities that hold because the
  constants are defined within the cited ranges; this is arithmetic, not an
  observational determination.
\item \textbf{CIRCULAR until reproduced} --- the agreement between fitted
  ($f_{\mathcal{C}}=0.082$, $z_*=0.48$) and predicted ($0.08$, $0.5$) consciousness
  parameters (Table~\ref{tab:parameter-constraints}). Both sides originate in the
  framework; independent re-analysis is required for this to count as evidence.
\item \textbf{EMPIRICAL HYPOTHESIS --- untested} --- all forecasts for Euclid, LSST,
  Roman, CMB-S4, and SKA. These are the genuine falsifiable predictions of the
  framework and remain to be tested; a proof assistant cannot establish them.
\end{itemize}
