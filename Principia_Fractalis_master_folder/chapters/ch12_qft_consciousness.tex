...
\item Renormalization group equations and asymptotic freedom\cite{gross1973,politzer1973reliable}
...
\noindent\emph{Honest scope (2026-07-23): this Lagrangian, and the entire quantum field theory built from it, is an EMPIRICAL HYPOTHESIS --- a posited construction. No consciousness field has been detected and none of its predictions has been experimentally confirmed; see the Verification status ledger in the Conclusion.}
...
Quantum field theories have couplings that "run" with energy scale\cite{peskin1995,weinberg1995}. Electromagnetism gets stronger at high energy (QED). The strong force gets weaker (QCD)\cite{gross1973,politzer1973reliable}. What about consciousness?
...
\noindent\emph{Honest scope (2026-07-23): this is an internal-consistency result for a posited field theory; the proof below assumes the consciousness field and the Timeless-Field extension of the state space. It does not establish that consciousness exists or behaves unitarily in nature --- that remains an untested empirical hypothesis.}
...
\item FMRI BOLD signal oscillations\cite{logothetis2001neurophysiological,ogawa1990}
...
\begin{itemize}
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED / NOT INDEPENDENTLY VALIDATED} --- that consciousness is a quantized rank-2 tensor field with the Lagrangian $\mathcal{L}_C$ (Definition~\ref{def:consciousness-lagrangian}), and every physical prediction that follows: the consciousness propagator, Feynman rules, asymptotic freedom / running couplings, the consciousness ``crystallization'' phase transition, ``psychon'' production at colliders, and quantum-consciousness interference. No such field, particle, or effect has been observed; these are theoretical constructions, not experimentally confirmed physics. A proof assistant cannot establish any of them.
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED} --- the unitarity and microcausality theorems (Theorem~\ref{thm:consciousness-unitarity}, Theorem~\ref{thm:microcausality}). These are internal consistency statements about a \emph{posited} field theory whose existence is unestablished; their proofs assume the consciousness field and the Timeless-Field extension of Hilbert space, so they do not establish that consciousness behaves this way in nature.
\item The Comparative-Alignment ``Status Markers'' ($\checkmark$/$\otimes$/$\triangle$) in this chapter mark consistency-with, or simulation-of, external experiments (Bell tests, Casimir effect); they are \textbf{not} independent confirmations of the consciousness QFT.
\end{itemize}
...
\begin{enumerate}
\item \textbf{(Lattice QFT)} Formulate consciousness QFT on a spacetime lattice\cite{wilson1974confinement,creutz1980monte}. Can you compute the consciousness mass $m_C$ non-perturbatively?
