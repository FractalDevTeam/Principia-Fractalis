...
\begin{definition}[Projection-Valued Measure (PVM)]\label{def:pvm}
...
\begin{example}[Imprecise Position Measurement]\label{ex:imprecise-position}
...
\begin{theorem}[Consciousness Measurement Outcomes]\label{thm:consciousness-measurement-outcomes}
...
\begin{definition}[Decoherence]\label{def:decoherence}
...
\begin{theorem}[Consciousness Prevents Decoherence]\label{thm:consciousness-prevents-decoherence}
...
\subsection{Measuring \texorpdfstring{$\text{ch}_2$}{ch\_2} via fMRI}
...
\bottomrule
\end{tabular}
\captionof{table}{Predicted relationship between integrated information $\Phi$ and Second Chern character $\text{ch}_2$ for various consciousness states.}
...
\begin{theorem}[Consciousness Collapses the Wave Function]\label{thm:consciousness-collapses}
...
\begin{theorem}[IIT and Chern Character Connection]\label{thm:iit-chern-connection}
...
\begin{itemize}
\item \textbf{STANDARD MATHEMATICS} --- projection-valued measures (Definition~\ref{def:pvm}), POVMs (Definition~\ref{def:povm}), the spectral measure of a self-adjoint operator, and the von Neumann measurement scheme are standard quantum-measurement and functional-analysis material, cited by reference. Only \emph{partial} spectral-measure content on the substrate is formalized in the Lean~4 corpus; the general theory presented here is classical, not kernel-checked.
\item \textbf{ASSERTED (not derived)} --- the consciousness-measurement outcome rule (Theorem~\ref{thm:consciousness-measurement-outcomes}), the ``consciousness collapses the wave function'' mechanism (Theorem~\ref{thm:consciousness-collapses}), and the IIT/Chern relation $\Phi = k\,[\mathrm{ch}_2]^{\alpha}$ (Theorem~\ref{thm:iit-chern-connection}) are framework proposals, not derived theorems or machine-verified results.
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED / NOT INDEPENDENTLY VALIDATED} --- consciousness-induced suppression of decoherence (Theorem~\ref{thm:consciousness-prevents-decoherence}), the predicted $\Phi$-vs-$\mathrm{ch}_2$ tables, and the fMRI/EEG/computational $\mathrm{ch}_2$ protocols are physical and clinical claims that only experiment can settle; a proof assistant cannot establish them.
\end{itemize}
