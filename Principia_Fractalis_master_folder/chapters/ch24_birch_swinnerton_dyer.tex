...
\item \textbf{PROVEN (kernel-verified), narrow scope} --- the Lean stack verifies only the rank-blind concordance on a $\mathrm{Fin}\,6$ LMFDB-restricted encoding and the rank-1 Heegner cascade on $E_{37\mathrm{a}1}$/$E_{43\mathrm{a}1}$ (\texttt{BSD\_HeegnerRank1Proof} / \texttt{...E43a1}), and those are \emph{conditional on Gross--Zagier and Kolyvagin}. It does \emph{not} verify the empirical 100\% success rate across the Cremona sample.
