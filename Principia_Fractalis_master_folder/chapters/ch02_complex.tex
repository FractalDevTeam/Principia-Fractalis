...
\begin{example}[Simply Connected Domains]
\begin{itemize}
...
\begin{definition}[Principal Logarithm]\index{principal logarithm}\label{def:principal-log}
...
\begin{definition}[Principal Argument]\index{principal argument}
...
\begin{definition}[Holomorphic Function]\index{holomorphic function}
...
\begin{theorem}[Cauchy–Goursat]\index{Cauchy-Goursat theorem}\label{thm:cauchy-goursat}
...
\begin{intuitive}[Why Holomorphic Integrals Vanish]
...
\begin{theorem}[Cauchy Integral Formula (CIF)]\index{Cauchy integral formula}\label{thm:CIF}
...
\begin{keyidea}[Rigidity of Holomorphic Functions]
...
\begin{theorem}[Morera]\index{Morera's theorem}\label{thm:morera}
...
\begin{theorem}[Liouville]\index{Liouville's theorem}\label{thm:liouville}
If $f$ is holomorphic on all of $\C$ (entire) and bounded, then $f$ is constant.
\end{theorem}

\subsection{Maximum Modulus Principle}

\begin{theorem}[Maximum Modulus]\index{maximum modulus principle}\label{thm:maximum-modulus}
...
\begin{theorem}[Identity Theorem]\index{identity theorem}\label{thm:identity}
...
\begin{definition}[Germ]\index{germ}\label{def:germ}
...
\begin{definition}[Analytic Continuation Along a Path]\index{analytic continuation}\label{def:analytic-continuation}
...
\begin{theorem}[Monodromy Theorem]\index{monodromy theorem}\label{thm:monodromy}
...
\begin{definition}[Branch]\index{branch}
...
\begin{example}[Principal Branch of $\Log$]
...
\begin{definition}[Principal Branch of $\Log$, reprise]\label{def:Log}
...
\begin{definition}[Fractional Power]\index{fractional power}\label{def:fractional-power}
...
\subsection{Winding Action on \texorpdfstring{$\Log$}{Log}}
...
\begin{definition}[Polylogarithm]\index{polylogarithm}\label{def:polylog}
...
\subsection{Singular Expansion Near \texorpdfstring{$z = 1$}{z = 1}}

This expansion is referenced in the P vs NP monodromy analysis.

\begin{theorem}[Singular Expansion Near $z = 1$]\index{polylogarithm!singular expansion}\label{thm:Li-expansion}
...
\subsection{Logarithmic Case: \texorpdfstring{$s \in \mathbb{N}$}{s in N}}
...
\begin{theorem}[Jonquières Expansion, Statement]\index{Jonquières expansion}\label{thm:jonquieres}
...
\begin{theorem}[Abel's Theorem]\index{Abel's theorem}\label{thm:abel}
...
\item \textbf{Chapter~\ref{ch:p-vs-np}:} Monodromy nonlinearity (Lemma~\ref{lem:frac-nonlinear}),\linebreak[1] polylogarithm expansions (Theorem~\ref{thm:Li-expansion})
...
The key insight: \textit{nonlinearity under winding} (Lemma~\ref{lem:frac-nonlinear}) is not a technical detail—it is the mathematical reason why nondeterministic computation cannot be efficiently simulated by deterministic algorithms. \emph{(Honest scope: this is a statement of the framework's programme, not a theorem established here or in the machine-checked corpus; see the Verification status note below and Chapter~\ref{ch:p-vs-np}.)}

\paragraph{Verification status (2026-07-23).}
To hold this chapter to the standard of the machine-checked corpus:
\begin{itemize}
\item \textbf{STANDARD MATHEMATICS} --- every theorem in this chapter (Cauchy--Goursat, the Cauchy integral formula, Morera, Liouville, Schwarz, the identity and monodromy theorems, the winding action $\Log z \mapsto \Log z + 2\pi i m$, the nonlinearity of $w^{s-1}$ under winding for $s\notin\Z$ (Lemma~\ref{lem:frac-nonlinear}), the polylogarithm singular expansion (Theorem~\ref{thm:Li-expansion}) and its monodromy (Corollary~\ref{cor:Li-monodromy}), and Abel's theorem) is classical complex analysis. These are correct, standard results proved or cited by reference; none is part of --- nor needs to be part of --- the project's Lean~4 kernel corpus.
\item \textbf{CONDITIONAL REDUCTION / ASSERTED} --- the interpretive claim that this analytic nonlinearity ``is the mathematical reason'' that $P\neq NP$ is \emph{not} established in this chapter and does not follow from these lemmas alone. The lemmas are true; the bridge from ``fractional powers are nonlinear under winding'' to ``nondeterministic computation cannot be efficiently simulated'' is the open content of Chapter~\ref{ch:p-vs-np}, where it rests on unproven framework hypotheses. Treat the winding/complexity link as an asserted programme, not a proof.
\end{itemize}
