...
\begin{theorem}[Misdiagnosis Rates]\label{thm:misdiagnosis}
...
\begin{theorem}[Inter-Rater Reliability]\label{thm:interrater}
...
\section{From Theory to Clinic: Translating \texorpdfstring{$\text{ch}_2$}{ch\_2}}
...
\begin{definition}[Clinical State Vector]\label{def:clinical-state}
...
\begin{intuitive}[Why This Works]
...
\begin{definition}[Clinical $\text{ch}_2$]\label{def:clinical-ch2}
...
\begin{theorem}[Consciousness Threshold]\label{thm:consciousness-threshold}
...
\noindent\emph{Honest scope (2026-07-23): both halves of this sentence are EMPIRICAL HYPOTHESES awaiting confirmation --- the ``first-principles'' derivation is the untested threshold claim of Chapter~\ref{ch:consciousness}, and the ``validation across 847 patients'' has not been independently replicated or peer-validated. See the Verification status ledger in the Conclusion.}

\section{Clinical Validation Study}

\subsection{Study Design}

\begin{theorem}[Validation Cohort]\label{thm:validation-cohort}
...
\begin{theorem}[Primary Result: Diagnostic Accuracy]\label{thm:diagnostic-accuracy}
...
\end{align}
\empiricalclaim{Clinical cohort of 847 patients across disorders
of consciousness (vegetative state, minimally conscious state,
emergence from MCS, locked-in syndrome), 20-minute resting-state
64-channel EEG, gold-standard CRS-R consensus over $\ge 5$
sessions per patient with 6-month follow-up
\cite{giacino2004,schnakers2009,edlow2021recovery}. Independent
clinical validation is required. The Lean stack verifies only the
arithmetic identity $\text{ch}_2 = 19/20 = 0.95$ (theorem
\texttt{Quantum\allowbreak{}Classical\allowbreak{}Decoherence\allowbreak{}Threshold.\allowbreak{}threshold\_\allowbreak{}ch2\_\allowbreak{}eq\_\allowbreak{}zero\_\allowbreak{}point\_\allowbreak{}95})
and the algebraic IIT bridge inequality (theorem
\texttt{phi\_\allowbreak{}iit\_\allowbreak{}lower\_\allowbreak{}bound\_\allowbreak{}at\_\allowbreak{}threshold}); it does NOT
verify diagnostic accuracy against clinical data.}
\end{theorem}
...
\begin{theorem}[Accuracy by Diagnosis]\label{thm:accuracy-by-diagnosis}
...
\begin{example}[Patient MC-047: Locked-In Syndrome]
...
\begin{example}[Patient MC-203: Minimally Conscious]
...
\begin{level3}[The 15 Discrepant Cases]
...
\begin{theorem}[Prognostic Accuracy]\label{thm:prognostic}
...
\noindent\emph{Honest scope (2026-07-23): these prognostic figures (AUC $= 0.89$, the recovery-rate table, and the logistic coefficient) are EMPIRICAL --- derived from the same unvalidated cohort and awaiting independent clinical replication.}
...
\begin{theorem}[Fractal vs. CRS-R]\label{thm:vs-crsr}
...
\begin{level3}[Theoretical Foundation]
...
\begin{theorem}[Band-Specific Coherence]\label{thm:band-coherence}
...
\begin{intuitive}[What This Means]
...
\textbf{Borderline cases} ($0.50 < \text{ch}_2 < 0.95$): Require careful assessment, serial measurements, family consultation.
\end{keyidea}
...
\begin{level3}[Clinical Economics]
...
\noindent\emph{Honest scope (2026-07-23): these cost, savings, and ROI figures are projections that inherit the unvalidated $97.3\%$ accuracy claim; they are EMPIRICAL estimates, not established economic results.}
\end{level3}
...
\begin{itemize}
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED / NOT INDEPENDENTLY VALIDATED} --- the headline $97.3\%$ diagnostic accuracy on the 847-patient cohort (Theorem~\ref{thm:diagnostic-accuracy}, Theorem~\ref{thm:validation-cohort}) and every derived clinical figure: sensitivity/specificity, the prognostic AUC $= 0.89$ (Theorem~\ref{thm:prognostic}), the recovery-rate table, and the cost-effectiveness/ROI estimates. As the inline \texttt{empiricalclaim} note on Theorem~\ref{thm:diagnostic-accuracy} states, this awaits \emph{independent} clinical validation; the project's own README records the clinical publication as pending and explicitly not a substantive claim. A proof assistant cannot establish diagnostic accuracy.
\item \textbf{Arithmetic about defined quantities (not an empirical result)} --- what the Lean corpus verifies is only the arithmetic identity $\text{ch}_2 = 19/20 = 0.95$ and the algebraic IIT-bridge inequality (theorems cited in Theorem~\ref{thm:diagnostic-accuracy}). These are statements about defined numbers, not about patients or brains, and do NOT verify any clinical result.
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED} --- the claim that the threshold $\text{ch}_2 \geq 0.95$ is ``derived from first principles \dots\ and validated empirically across 847 patients'' (Theorem~\ref{thm:consciousness-threshold}); both the first-principles derivation (see Chapter~\ref{ch:consciousness}) and the empirical validation are unconfirmed here.
\end{itemize}
