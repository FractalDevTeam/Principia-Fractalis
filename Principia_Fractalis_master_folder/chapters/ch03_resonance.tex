...
\hline
\end{tabular}
\caption{Canonical resonance values of $R_f(\alpha, s)$ per millennium problem (corrected 2026-05-18 to match the Per-Millennium $\alpha$-Dictionary; prior NS=$5/3$, BSD=$\varphi+1/3$, Hodge=$\pi/2$ did not match the canonical values used in Ch~\ref{ch:navier-stokes}, Ch~\ref{ch:birch-swinnerton-dyer}, Ch~\ref{ch:hodge-conjecture}).}
...
\item \textbf{Algebraic geometry} ($\alpha = \varphi, 3\pi/4$): Hodge and BSD conjectures (canonical values; prior $\pi/2, \phi+1/3$ corrected 2026-05-18)
\end{itemize}
...
\begin{align}
\text{At } \alpha = \sqrt{2}: \quad &\Delta_P = 0.0891219046 \\
\text{At } \alpha = \phi + 1/4: \quad &\Delta_{NP} = 0.1337359842
\end{align}
...
\section{The Universal \texorpdfstring{$\pi/10$}{pi/10} Factor}
...
\begin{theorem}[Polylogarithm Evaluation]\index{polylogarithm}
...
\begin{itemize}
\item \textbf{STANDARD MATHEMATICS} --- absolute convergence of $R_f(\alpha,s)$ for $\text{Re}(s)>1$ (Theorem~\ref{thm:rf-convergence}) and the identity $R_f(0,s)=\zeta(s)$ are correct and elementary (comparison with the $p$-series). The claimed analytic continuation to $\mathbb{C}\setminus\{1\}$ is stated by analogy with $\zeta$; a complete continuation/functional-equation proof is not given here.
\item \textbf{ASSERTED (not derived)} --- the canonical resonance values in Table~\ref{tab:resonance-values} (e.g.\ $\alpha_P=\sqrt2$, $\alpha_{NP}=\varphi+\tfrac14$, $\alpha=3/2$ for RH) are \emph{chosen/assigned}, not derived from first principles. Per the framework's own \texttt{DERIVATION\_ANALYSIS\_alpha\_NP.md}, $\alpha_{NP}=\varphi+\tfrac14$ is a conjecture supported by numerics: the golden-ratio term is heuristic and the additive $1/4$ is unexplained. The Lean corpus proves only \emph{algebraic identities among these defined constants}, not that the constants solve the Millennium problems.
\item \textbf{CONDITIONAL / DEFERRED} --- the RH resonance equivalence (Theorem~\ref{thm:rh-resonance}) and the P-vs-NP spectral-gap claim (Theorem~\ref{thm:complexity-gap}) are stated with proofs deferred to later chapters, where they are conditional reductions resting on open framework hypotheses, not settled theorems. The quoted gap digits $\Delta_P,\Delta_{NP}$ are numerical outputs, not a proof of $P\neq NP$.
\item \textbf{EMPIRICAL / OPEN} --- the ``universal $\pi/10$ factor'' is an empirical observation; the text itself notes its ``theoretical derivation from first principles is an open problem.'' It is not machine-verified.
\end{itemize}

\paragraph{Verification update (2026-08-06): Theorem~\ref{thm:rh-resonance} is
FALSE as stated --- and the same digit structure yields a real theorem in its
place.}
The 2026-07-23 ledger above filed the RH resonance equivalence under
\textbf{CONDITIONAL / DEFERRED}. That was too generous. The claim has now been
computed and it does not hold.

\begin{itemize}
\item \textbf{REFUTED} --- Theorem~\ref{thm:rh-resonance} asserts
$R_f(3/2,\,1/2+it)=0 \Leftrightarrow \zeta(1/2+it)=0$. At $\alpha=3/2$ the
phase is $(-i)^{D_3(n)}$, and the exact digit-block identity
$\sum_{n<3^k}\omega^{D_3(n)}=(1+\omega+\omega^2)^k$ gives partial sums of
modulus~$1$; the abscissa of convergence is therefore $0$ and direct summation
\emph{on} the critical line is valid. Evaluated at the first five zeta zeros,
$|R_f(3/2,\tfrac12+i\gamma)| = 0.6187,\ 1.5462,\ 1.7966,\ 1.1359,\ 1.6266$
for $\gamma = 14.1347,\ 21.0220,\ 25.0109,\ 30.4249,\ 32.9351$, stable across
$n<3^{13}$ and $n<3^{15}$. None is zero. Conversely $R_f(3/2,\cdot)$ has its
own near-zeros on the line near $t \approx 2.75,\ 11.0,\ 19.75,\ 29.0,\ 31.75$,
and none of these is a zeta ordinate. So neither direction of the claimed
equivalence survives. $R_f$ is not a deformation of $\zeta$.
In Lean the statement was never discharged: it is a bare \texttt{Prop} at
\texttt{PF/Consciousness/FractalResonance.lean:323}.
Script: \texttt{codex/rf\_falsification\_checks\_2026-08-06.py}.
\item \textbf{CONSEQUENCE} --- the chain
ch03 $\to$ ch09 (\S\ref{ch:spectral_unity}) $\to$ ch20 rests on this theorem.
Chapter~9's ``spectral unity'' bridge is therefore not merely unproved; its
stated form is refuted. Nothing in the machine-checked corpus depended on it.
\item \textbf{NEW, and kernel-verified (r212)} --- the same digit-block
identity that refutes the theorem also \emph{proves} something. Since
$1+e^{i\pi\alpha}+e^{2i\pi\alpha}=e^{i\pi\alpha}(1+2\cos\pi\alpha)$, the
abscissa of convergence of $R_f(\alpha,\cdot)$ is
\[
  \sigma(\alpha) \;=\; \log_3\bigl|1+2\cos(\pi\alpha)\bigr|.
\]
Checks: $\sigma(0)=1$ (the pole of $\zeta$), $\sigma(2)=1$, $\sigma(3/2)=0$.
This is the framework's first \emph{non-circular} $\alpha$-selection
mechanism: it is derived from the base-3 digit structure rather than assigned.
Run against the nine canonical values, it derives exactly the three
\emph{rational} ones --- $\sigma(1)=0$, $\sigma(3/2)=0$, $\sigma(2)=1$ --- and
provably misses all six irrational ones, because $\sigma(\alpha)\in\{0,1\}$
forces $\alpha$ rational. So $\sqrt2$, $\varphi$, $\varphi+\tfrac14$,
$\sqrt{2\pi}$, $3\pi/4$ and $3\pi/2$ cannot arise this way. This complements
r123, which refutes the substrate trace / K-theory route to $\alpha$: the
digit route works, and reaches exactly one third of the table.
Lean: \texttt{PF/SigmaAbscissa\_r212.lean}, 31 declarations, axioms exactly
$[\texttt{propext},\texttt{Classical.choice},\texttt{Quot.sound}]$.
\item \textbf{GUARD RAIL} --- $\sigma(\varphi)=0.4962$, which is within $0.8\%$
of $\tfrac12$. This is a \emph{coincidence} and must not be read as a
derivation of $\alpha_{\mathrm{Hodge}}$: the exact solution of
$\sigma(\alpha)=\tfrac12$ is $\alpha=1.6192817$, not $\varphi=1.6180340$, and
they differ by $1.25\times10^{-3}$. The separation is kernel-verified
(\texttt{sigma\_goldenRatio\_ne\_half}), not merely numerical.
\end{itemize}

\noindent Record: \texttt{codex/AUDIT\_RESPONSE\_2026-08-06.md} \S2.1, \S3.
