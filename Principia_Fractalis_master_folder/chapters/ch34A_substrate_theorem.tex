...
\begin{warning}[Honest Scope, audited per axis at HEAD \texttt{3457d56}]
...
\texttt{PrincipiaTractalis.RiemannHypothesis}$, which is the
standard critical-strip statement on mathlib's
\texttt{riemannZeta}: $\forall s, 0 < \Re s < 1, \zeta(s) = 0
\implies \Re s = 1/2$. Discharge conditional on Mayer 1991 \S 3
spectral correspondence and the Hilbert--P\'olya program as
typed Lean propositions (currently named, not yet formalised
as Lean theorems).
\item \textbf{NS} (C4) --- \texttt{PF\_NS3DEncodingV4.Velocity}
$:= \texttt{SchwartzMap (Fin 3 → \ensuremath{\mathbb{R}}) (Fin 3 → \ensuremath{\mathbb{R}})}$:
mathlib's standard Schwartz spacetime velocity type. Discharge
axiom-free on the encoding. The framework's $H^{s}_{\sigma}$
inner-product scaffold and Leray projection scaffold are
substrate-PROVEN
(\texttt{hsSigmaInnerProductScaffoldAtSubstrate},
\texttt{lerayProjectionScaffoldAtFiniteRank}), not
\texttt{Prop := True} placeholders.
\item \textbf{BSD} (C3) --- \texttt{PF\_BSDEncodingV4.EllipticCurve}
$:= \texttt{WeierstrassCurve \ensuremath{\mathbb{Q}}}$: mathlib's
standard elliptic-curve type. Discharge axiom-free with
framework rank projections (\texttt{algebraicRankV4 = analyticRankV4}
by \texttt{rfl} on shared \texttt{manuscriptRankV4}). The
bridge to mathlib's \texttt{MordellWeilRank} is named:
\texttt{UniversalBridge\_MordellWeilRank\_eq\_algebraicRankV4}
(typed proposition; formalisation pending).
\end{itemize}
...
$:= \texttt{GeneralSmoothQuintic}$ (framework-restricted to
general smooth quintics, not arbitrary smooth projective
complex varieties). Discharge axiom-free on the encoding;
Voisin~2007 obstruction on the generic non-CM smooth quintic
names the precise open gap.
