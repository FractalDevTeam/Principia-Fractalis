...
companion file
\texttt{PF/Referee/MinimalSubstrateRigidityIndependence.lean}
...
\begin{proof}[Proof (machine-checked, kernel-only axioms)]
By the Lean theorem
\texttt{minimal\_invariants\_are\_strictly\_independent}. The
...
(\texttt{PF.IBMPeaksGaloisPair.IBM\_peaks\_are\_Galois\_conjugates\_in\_Q\_sqrt5},
...
The companion file
\texttt{PF/Referee/MinimalRigidityForcesIBMGaloisPair.lean} elevates
...
\begin{proof}[Proof (machine-checked, kernel-only axioms)]
By the Lean theorem
\texttt{unified\_minimal\_forces\_IBM\_Galois\_pair\_structure}.
The argument composes
\texttt{unified\_alpha\_skeleton\_forced\_by\_minimal\_invariants}
...
\subsection{Non-Clay $\alpha$-value reach}
...
(\texttt{PF/Referee/MinimalRigidityForcesNonClayAlphas*.lean})
...
\texttt{PF.Consciousness.QuantumClassicalDecoherence\-Threshold}, the
theorem \texttt{phi\_iit\_lower\_bound\_at\_threshold} establishes
...
\begin{proof}[Proof (machine-checked, kernel-only axioms)]
By the Lean theorem
\texttt{unified\_minimal\_forces\_iit\_phi\_threshold\_in\_alpha\_NP\_terms}.
...
master theorem in \texttt{PF/Referee/SubstrateRigidityMasterCapstone.lean}:
...
\begin{proof}[Proof (machine-checked, kernel-only axioms)]
By the Lean theorem
\texttt{substrate\_rigidity\_master\_capstone}, composing
\texttt{unified\_alpha\_skeleton\_forced\_by\_minimal\_invariants}
(M1),
\texttt{unified\_minimal\_forces\_IBM\_Galois\_pair\_structure} (M2),
\texttt{unified\_minimal\_forces\_Hermitian\_realization} (M3), and
\texttt{unified\_minimal\_forces\_iit\_phi\_threshold\_in\_alpha\_NP\_terms} (M4).
Strict minimality is established by
\texttt{minimal\_invariants\_are\_strictly\_independent},
\texttt{positivity\_hypotheses\_are\_strictly\_necessary}, and
\texttt{perelman\_anchor\_is\_strictly\_necessary}.
\end{proof}

\subsection{Substrate forces spectral gap, $H_3$ icosahedral geometry, cosmological $\Lambda$ suppression}
...
\texttt{substrate\_rigidity\_ultimate\_master\_capstone}), demonstrating
...
\begin{proof}[Proof (machine-checked, kernel-only axioms)]
By the Lean theorem
\texttt{substrate\_rigidity\_ultimate\_master\_capstone}. Each clause
composes the unified $\alpha$-skeleton forcing with:
\begin{itemize}
\setlength{\itemsep}{0pt}
\item (M6): \texttt{unified\_minimal\_forces\_spectral\_gap\_content\_capstone}
in \texttt{PF/Referee/MinimalRigidityForcesSpectralGapContent.lean}.
\item (M7): \texttt{unified\_minimal\_forces\_sin\_pi\_div\_ten\_eq\_inv\_two\_a\_Hodge}
in \texttt{PF/Referee/MinimalRigidityForcesH3CoxeterGeometry.lean}.
\item (M8): \texttt{unified\_minimal\_forces\_H3\_combinatorial\_structure\_capstone}
in \texttt{PF/Referee/MinimalRigidityForcesH3CombinatorialStructure.lean}.
\item (M9): \texttt{cosmological\_suppression\_substrate\_capstone}
in \texttt{PF/Referee/MinimalRigidityForcesCosmologicalSuppression.lean}.
\end{itemize}
\end{proof}
...
The framework's
\texttt{PF/PerelmanBackwardUnifiedAttack.lean} proves AXIOM-FREE that
...
(\texttt{W\_alpha\_limit\_eq\_alpha\_times\_three}).
\end{itemize}
...
\begin{proof}[Proof (machine-checked, kernel-only axioms)]
By the Lean theorem
\texttt{perelman\_w\_entropy\_substrate\_scaling\_capstone}
in
\texttt{PF/Referee/MinimalRigidityForcesPerelmanWEntropyScaling.lean}.
...
from \texttt{unified\_alpha\_skeleton\_forced\_by\_minimal\_invariants}.
\end{proof}
...
\begin{enumerate}
\item \texttt{MinimalRigidityForcesParticlePhysicsCapstone} ---
...
\item \texttt{MinimalRigidityForcesCrossDomainExperimentalWins} ---
...
\item \texttt{MinimalRigidityForcesQCMaxSpeedup} --- the framework's
...
\item \texttt{MinimalRigidityForcesConsciousnessQuantification} ---
...
\item \texttt{SubstrateRigidityCrossDomainSuperCapstone} ---
single-citation theorem composing (1)--(4) under one set of
13-condition substrate-rigidity hypotheses.

\item \texttt{MinimalRigidityForcesAlphaArchitecturalIdentities} ---
...
\item \texttt{MinimalRigidityForcesCrossMillenniumSharedInvariants}
--- the 11-clause typed bundle of baseline algebraic invariants
relating the 9 $\alpha$-instances, all lifted parametrically.

\item \texttt{MinimalRigidityForcesGraphIsomorphismPrediction} ---
...
\item \texttt{MinimalRigidityForcesAlphaBasisDecomposition} ---
the 9 substrate-forced $\alpha$-values expressed over the
4-element minimal basis $\{1, \pi, \phi, \sqrt{2}\}$.

\item \texttt{MinimalRigidityForcesPiRationalSubstructure} ---
...
\item \texttt{MinimalRigidityForcesHodgeGroundStateClean} ---
...
\item \texttt{MinimalRigidityForcesBSDDistinguishedEigenvalue} ---
the Ch~24 distinguished BSD eigenvalue
$\phi/e = u.\mathrm{sector2}.a_{Hodge}/e \approx 0.595$ parametric.

\item \texttt{MinimalRigidityForcesPerelmanAnchoredCascade} ---
...
\item \texttt{MinimalRigidityForcesH3UnifiedAlgebraicStructure}
...
\item \texttt{MinimalRigidityForcesCrossMillenniumMoreInvariants}
...
\item \texttt{MinimalRigidityForcesPolylogResonanceAtGaloisPair}
...
\item \texttt{MinimalRigidityForcesBSDConcordance} --- rank-blind
...
\item \texttt{MinimalRigidityForcesIBMSearchRange} --- 8 of the
...
(Lean: \texttt{PF/Referee/UnifiedClayClosureLinkage.lean}), the
...
\item via \texttt{unified\_clay\_closure\_via\_substrate\_linkage},
...
(\texttt{MinimalRigidityForcesPerelmanAnchoredCascade}) tethers
every clause back to $\alpha_{Poincar\acute{e}} = 1$, and
\texttt{unified\_clay\_closure\_via\_substrate\_linkage} reduces
...
\texttt{unified\_clay\_closure\_via\_substrate\_linkage}; what
