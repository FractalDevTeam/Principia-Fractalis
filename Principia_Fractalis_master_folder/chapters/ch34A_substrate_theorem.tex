...
\subsubsection*{The $\alpha$-Substrate Reference Table (post-2026-09-25)}
\label{subsubsec:alpha-substrate-reference-table}

Under the two external anchors landed on 2026-09-25 (Perelman
$\alpha_{\text{Poincar\'e}} = 1$; OpenAI-corroborated
$\alpha_{\text{NS}} = 3\pi/2$; see
\S\ref{sec:two-anchor-cascade}), the framework's nine $\alpha$-values
...
\begin{tabular}{lccl}
\toprule
\textbf{Constant} & \textbf{Value} & \textbf{Anchor status} & \textbf{Refutable by} \\
\midrule
$\alpha_{\text{Poincar\'e}}$ & $1$                       & \emph{External} (Perelman 2003) & \textit{settled} \\
$\alpha_{\text{NS}}$         & $3\pi/2$                  & \emph{External} (OpenAI 2026, corroborating) & (open Clay NS) \\
$\alpha_{\text{YM}}$         & $2$                       & Forced by I7 \& Perelman & Yang-Mills mass gap \\
$\alpha_{\text{RH}}$         & $3/2$                     & Forced by I9 \& $\alpha_{\text{YM}} = 2$ & Riemann hypothesis \\
$\alpha_{\text{BSD}}$        & $3\pi/4$                  & Forced by I6 \& $\alpha_{\text{NS}}$ & Birch--Swinnerton-Dyer \\
$\alpha_{\text{QG}}$         & $\sqrt{2\pi}$             & Forced by QG-bridge \& Perelman & (open QG; no Clay) \\
$\alpha_{\text{Hodge}}$      & $\varphi = (1+\sqrt{5})/2$ & Forced by I8 \& positivity & Hodge conjecture \\
$\alpha_P$                   & $\sqrt{2}$                & Forced by Wave-22 \& $\alpha_{\text{YM}}$ & P vs NP \\
$\alpha_{\text{NP}}$         & $\varphi + 1/4$           & Forced by Hodge-NP link & P vs NP (co-refuted with $\alpha_P$) \\
\bottomrule
\end{tabular}
\end{center}

\noindent The two \emph{External} rows are established by
independently verified proofs outside Principia Fractalis. The seven
\emph{Forced} rows are consequences of the two external anchors plus
the structural identities I6, I7, I8, I9, QG-bridge, Wave-22, and the
Hodge-NP link (each proved kernel-clean in the Lean corpus). Any
future externally-verified Millennium result whose $\alpha$-classification
disagrees with the fourth-column entry refutes at least one anchor or
identity; this is the content of
\texttt{framework\_falsifiable\_at\_five\_points} in
\texttt{PF/FiveWayFalsifiability.lean}.
...
\paragraph{Verification update (2026-08-08): the implication carries no
information about the antecedents, so ``determine'' does not follow. The Lean
source says this; the gloss above does not.}

The Lean file is explicit. \texttt{PrincipiaFractalisSubstrateTheorem.lean:374-377}:

\begin{quote}\small
``\emph{The Lean implication is therefore vacuously true}: it does not derive the
consequences FROM the antecedents in any deductive sense; it merely records that
both are inhabitable in the same file.''
\end{quote}

\noindent and the proof sketch of
Theorem~\ref{thm:pf-substrate-theorem} says the same: the antecedents ``are not
\emph{consumed} \dots\ they are passed in as a record and discarded as
\texttt{\_h\_antecedents}''. \textbf{This is not concealment.} The defect is that
the theorem's one-line gloss and the KeyIdea above outran the disclosures
already sitting beneath them.

\begin{itemize}
\item \textbf{THE LOGIC.} Let $A =$ \texttt{PFSubstrateAntecedents} and
$C =$ \texttt{PFSubstrateConsequences}. The corpus proves $C$ outright
(\texttt{PrincipiaFractalisSubstrateConsequences\_holds\_unconditionally}).
Once $C$ is a theorem, $A \to C$ is a theorem \emph{for every} $A$ --- for
\texttt{True}, for $1=1$, for any unrelated proposition. So the implication
distinguishes the framework's five antecedents from nothing at all. The word
``Equivalently'' at the head of Theorem~\ref{thm:pf-substrate-theorem} is
therefore incorrect: $A \to C$ is not equivalent to ``$A$ determines $C$'', and
in this instance it is strictly weaker than $C$ alone.
\item \textbf{THE KEYIDEA CLAIM DOES NOT FOLLOW.} A referee who accepts the
implication form has accepted $A \to C$, which they could have derived from $C$
without ever reading $A$. They have accepted nothing about determination. Read
the sentence ``the framework's headline is \emph{both}'' as superseded: the
currently-proved content is the unconditional form only, exactly as the second
bullet of that KeyIdea states.
\item \textbf{THE BAR FOR A REAL DERIVATION, stated so it cannot be met
trivially.} The obvious test --- ``one consequence whose proof stops
typechecking when the antecedent hypothesis is removed'' --- is mechanical, but
it has a loophole, and the loophole is already inhabited. Several antecedent
fields \emph{are} consequence fields: A3 and C7 are the same proposition, so
\texttt{fun h => h.perelmanAlphaPoincareEqOne} consumes the antecedent
syntactically while conveying nothing, since C7 is separately true by
definitional unfolding. Recorded in the kernel as
\texttt{C7\_from\_A3\_by\_projection}
(\texttt{PF/SubstrateTheoremContent\_r216.lean}); the same trivial route exists
for C20 from A5.

The bar that is not met is therefore: \textbf{one consequence that is not
itself an antecedent field, whose proof fails to elaborate when the antecedent
hypothesis is deleted.} At present \textbf{zero of the twenty-five} meet it.
This is not residual formalization labour --- it is a design question about
whether the substrate does any deductive work.
\item \textbf{NOW KERNEL-VISIBLE (r216).} The statements above are no longer
prose. \texttt{PF/SubstrateTheoremContent\_r216.lean} proves, axioms exactly
$[\texttt{propext},\texttt{Classical.choice},\texttt{Quot.sound}]$:
\texttt{consequences\_hold\_under\_any\_hypothesis}, i.e.\ $P \to C$ for
\emph{every} proposition $P$ --- witnessed concretely by
\texttt{consequences\_hold\_under\_an\_unrelated\_hypothesis}, which derives all
twenty-five consequences from $2+2=4$; and
\texttt{substrate\_implication\_from\_conclusion}, i.e.\ the implication follows
from its own conclusion. Both directions of the arrow typecheck vacuously
(\texttt{converse\_implication\_holds\_by\_discarding\_its\_hypothesis}), so
neither carries information.
\item \textbf{FIELD CLASSIFICATION.} Of the twenty-five fields:
one is an existential witnessed by the zero object (C4, Navier--Stokes);
about seven are definitional or arithmetic;
five are restricted carriers;
about six are content-bearing;
six are bundles that re-package the others. Specifics worth naming:
\begin{itemize}
\item \textbf{C4} --- the typed NS predicate contains no time derivative, no
$\mathbf{u}\cdot\nabla\mathbf{u}$, no pressure, no Laplacian and no viscosity;
its existential is witnessed by the zero field at the zero datum. Now
kernel-visible: \texttt{PF/NavierStokesTypedContent\_r215.lean}. See the
2026-08-08 entry in Ch.~\ref{ch:navier-stokes}.
\item \textbf{C7} --- $\alpha_{\text{Poincar\'e}} = 1$, against
\texttt{CrossMillenniumSharedInvariants.lean:64}, which \emph{defines}
$\alpha_{\text{Poincar\'e}}$ to be the real number $1$. A definitional
unfolding, not a derived fact.
\item \textbf{C17} --- $78 = 48+26+4$. Arithmetic. No BRST complex and no
$H^2$ computation stands behind it.
\item \textbf{C20, which is also antecedent A5} --- the ``143-problem''
dataset is \texttt{List.replicate 72 (canonicalEntry .P) ++ List.replicate 71
(canonicalEntry .NP)} (\texttt{HundredFortyThreeProblems.lean:132-142}), and
\texttt{canonicalEntry} sets \texttt{alphaMeasured := canonicalAlpha c}
(\texttt{:127}). So ``every measured $\alpha$ is $\sqrt2$ or $\varphi+1/4$'' is
true by construction from 143 copies of two hard-coded rows. It carries no
empirical content, and \emph{the same tautology appears on both sides of the
implication}.
\item \textbf{C3, C5, C2} --- restricted carriers: BSD on
\texttt{EllipticCurve := Fin 6}; Hodge with \texttt{isAlgebraic} reduced to
three existentials, two of which are trivially inhabited; YM as one
eigenvector of one map on $\ell^2$, with no gauge group. These are already
recorded in Remark~\ref{rem:framework-first-honest-scope}'s 2026-08-02
correction, which stands.
\end{itemize}
\item \textbf{A CORRECTION TO AN EARLIER AUDIT OF THIS CHAPTER.} An external
audit reported that fields of \texttt{PFSubstrateConsequences} are
\texttt{Prop := True} markers. That is wrong: the record has \textbf{zero}
such fields. The \texttt{True} fields live in the adjacent
\texttt{...HonestScope} record at
\texttt{PrincipiaFractalisSubstrateTheorem.lean:496-516}, where four of four
are \texttt{: True}, discharged by
$\langle\texttt{trivial},\dots\rangle$. The distinction matters: the
consequences record is populated by real propositions, several of them weak,
none of them literally trivial.
\item \textbf{WHAT IS NOT AFFECTED.} Not every field is hollow ---
\texttt{phi\_iit\_lower\_bound},
$\forall \Phi,\ \tfrac{19}{20} \le 1-e^{-\Phi/2} \to 2\log 20 \le \Phi$, is
genuine real analysis. And the operator-algebra substrate is untouched by any
of the above: the r102--r113 arc proves \texttt{UHF\_trace\_faithful} and
\texttt{substrate\_completion\_simple\_unconditional} unconditionally,
kernel-only, zero project axioms. Those results do not depend on this theorem
and are not weakened by it. They are the part of the substrate programme a
referee can check end to end today.
\end{itemize}

\noindent Record: \texttt{codex/AUDIT\_RESPONSE\_2026-08-06.md} \S4.2.
...
\texttt{[propext, Classical.choice, Quot.sound]}, all built into the
...
\noindent\emph{Correction (2026-08-02): the paragraph above overstates, and Ch.~34's master ledger --- not this remark --- is authoritative. Three qualifications. (1) ``Follow by construction'' describes the framework's \emph{encodings}: three of the six axes discharge on substrate-restricted carriers (YM on $\ell^2(\mathbb{R})$, not a compact simple gauge group; Hodge with \texttt{isAlgebraic} trivialised where the hard content lives; BSD's V4 rank equality holding by \texttt{rfl} because both ranks are the same definition). (2) The ``residual mathlib formalization work'' includes, per this chapter's own per-axis tags and Appendix~K's residual list, items that are the open problems themselves (the HP program for RH; the vortex-stretching bound for NS; the continuum Wightman lift for YM), so it is not merely instantiation labour. (3) Ch.~34's ledger states the machine-verified scientific scope is the operator-algebra substrate together with the unconditional number-theoretic results; where this remark and that ledger disagree, the ledger governs.}
\end{remark}

\begin{remark}[Operator-algebra foundation now fully formalized, 2026-07-23]
Independently of the per-axis Clay discharges discussed above, the
\emph{operator-algebraic} substrate on which $\mathcal{T}_\infty$ rests is
now machine-verified without residuals. The July 2026 development arc
r102--r112 established, kernel-only and with zero project axioms, that the
substrate UHF $C^*$-algebra $\mathcal{T}_\infty = M_{3^\infty}$ carries a
canonical \emph{faithful} tracial state (\texttt{UHF\_trace\_faithful})
and is $C^*$-simple (\texttt{substrate\_completion\_simple\_unconditional},
the Glimm 1960 theorem), via a formalized trace-preserving conditional
expectation. This closes what had previously been a named residual
(\texttt{SubstrateUHFCompletionPositiveFaithfulnessSubstrateConjecture})
and is documented in full in Chapter~\ref{ch:timeless-field} and the
companion paper \emph{Formalizing a Faithful Tracial State on the UHF
Algebra $M_{3^\infty}$}. This is a self-contained operator-algebra result
and does not itself constitute a Clay-statement discharge.
\end{remark}
...
\section{Two-Anchor Cascade and Five-Way Falsifiability (2026-09-25)}
\label{sec:two-anchor-cascade}

\subsection{From One External Anchor to Two}

Prior to September 2026, the substrate cascade rested on a single
external anchor: Grigori Perelman's Ricci-flow-with-surgery proof of
the Poincar\'e conjecture ($\alpha_{\text{Poincar\'e}} = 1$,
community-verified 2003--2006, Clay awarded 2010, declined by Perelman).
Under this single anchor, the framework's structural identities
(I6, I7, I8, I9, QG-bridge, Wave-22 $\alpha_P^2 = \alpha_{\text{YM}}$,
Hodge-NP link) derive every remaining $\alpha$-value---but the
derivation is internal to the framework.

On September~8, 2026, OpenAI's Lean-verified formalization of Navier-Stokes
and Euler blow-up results
(Chapter~\ref{ch:navier-stokes}, \S\ref{sec:openai-2026-corroboration})
provided a second externally-verified data point on the Navier-Stokes/Euler
axis. Under the honest scoping of that section---Clay has \emph{not}
accepted the OpenAI claim, and the specific result addresses (C)/(D)
alternatives rather than the primary Clay conjecture---the Lean
formalization is nevertheless a genuine externally-verified analytic
result. Combined with the framework's derived $\alpha_{\text{NS}} = 3\pi/2$,
this yields a second corroborating external anchor.

\subsection{The Forced Values}

\begin{keyidea}
Under the two external anchors and the framework's structural
identities, the remaining seven $\alpha$-values in the substrate
are \emph{uniquely forced}:
\begin{align*}
\alpha_{\text{Poincar\'e}} &= 1 &&\text{(anchored: Perelman 2003)} \\
\alpha_{\text{NS}} &= 3\pi/2 &&\text{(anchored: OpenAI 2026, corroboration)} \\
\alpha_{\text{YM}} &= 2 &&\text{(forced by I7)} \\
\alpha_{\text{RH}} &= 3/2 &&\text{(forced by I9 with $\alpha_{\text{YM}}=2$)} \\
\alpha_{\text{BSD}} &= 3\pi/4 &&\text{(forced by I6)} \\
\alpha_{\text{QG}} &= \sqrt{2\pi} &&\text{(forced by QG-bridge)} \\
\alpha_{\text{Hodge}} &= \varphi = (1+\sqrt{5})/2 &&\text{(forced by I8 + positivity)} \\
\alpha_P &= \sqrt{2} &&\text{(forced by Wave-22 + I7)} \\
\alpha_{\text{NP}} &= \varphi + 1/4 &&\text{(forced by Hodge-NP link)}
\end{align*}
This is proved axiom-free in
\texttt{PF/FiveWayFalsifiability.lean}, theorem
\texttt{five\_way\_falsifiability}, kernel-clean with only
\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}.
\end{keyidea}

\subsection{Falsifiability at Five Open Millennium Points}

The companion theorem
\texttt{framework\_falsifiable\_at\_five\_points} is the direct
contrapositive of the above: any future externally-verified Millennium
result yielding a value inconsistent with the framework's forced
$\alpha_X$ for $X \in \{\text{RH}, \text{YM}, \text{Hodge},
\text{BSD}, \text{P/NP}\}$ refutes at least one anchor or one
structural identity of the substrate cascade.

The five open predictions are:
\begin{enumerate}
\item $\alpha_{\text{RH}} = 3/2$ --- refutable by a solved Riemann
Hypothesis attack yielding a different $\alpha$-classification.
\item $\alpha_{\text{YM}} = 2$ --- refutable by a Yang-Mills mass
gap resolution.
\item $\alpha_{\text{Hodge}} = \varphi$ --- refutable by a Hodge
conjecture proof.
\item $\alpha_{\text{BSD}} = 3\pi/4$ --- refutable by a
Birch--Swinnerton-Dyer resolution.
\item $\alpha_P = \sqrt{2}$ (with $\alpha_{\text{NP}} = \varphi + 1/4$)
--- refutable by a P vs NP resolution yielding a different $\alpha$
signature.
\end{enumerate}

This is not a proof of the five open Millennium Problems. It is a
\emph{prediction machine}: given the two established external anchors
and the framework's substrate structure, the five open
$\alpha$-values are pinned to specific numerical or algebraic forms.
The framework is scientifically falsifiable at five distinct points.

\subsection{Lean Files Landed 2026-09-25}

Kernel-clean, all with only
\texttt{[propext, Classical.choice, Quot.sound]}:
\begin{itemize}
\item \texttt{PF/TwoAnchorCascadeCapstone.lean} ---
\texttt{two\_anchor\_cascade} and \texttt{two\_anchor\_zero\_dof}.
Derives $\alpha_{\text{YM}}$, $\alpha_{\text{RH}}$, $\alpha_{\text{BSD}}$,
$\alpha_{\text{QG}}$ from the two anchors + I6, I7, I9, QG-bridge.
\item \texttt{PF/HodgeQuadraticFromTwoAnchors.lean} ---
\texttt{hodge\_from\_two\_anchors}. Derives $\alpha_{\text{Hodge}} =
\varphi$ from I8 + positivity.
\item \texttt{PF/PvsNPAnchorReduction.lean} ---
\texttt{alpha\_P\_from\_two\_anchors},
\texttt{alpha\_NP\_from\_two\_anchors},
\texttt{pvsnp\_from\_two\_anchors}. Derives $\alpha_P = \sqrt{2}$ and
$\alpha_{\text{NP}} = \varphi + 1/4$.
\item \texttt{PF/FiveWayFalsifiability.lean} ---
\texttt{five\_way\_falsifiability} (all 7 forced) and
\texttt{framework\_falsifiable\_at\_five\_points} (contrapositive).
\end{itemize}

For the manuscript-adjacent write-up including scope disclaimers on the
OpenAI result, see
\texttt{codex/TWO\_ANCHOR\_STRATEGIC\_UPDATE\_2026-09-25.md}.
