...
\textbf{Build state at HEAD commit \texttt{85d6bae}:}
...
verified by Cao--Zhu 2006~\cite{caozhu2006}, Morgan--Tian
2007~\cite{morgantian2007}, and Kleiner--Lott
2008~\cite{kleinerlott2008}, fixes the rigidity skeleton's anchor
value; the framework records this as a substrate-level postulate.
...
axiom-free, at HEAD commit \texttt{85d6bae}, without consuming
...
\item \textbf{RH typed-bridge V2 (added in V2.1.1, HEAD \texttt{85d6bae}).}
...
\item \textbf{P vs NP carrier V3 (added in V2.1.1, HEAD \texttt{85d6bae}).}
...
preserved \emph{verbatim} at HEAD~\texttt{85d6bae}; V2 does NOT edit
...
\subsection{Beyond Clay: substrate-level treatment of 16 non-Clay
open problems}
\label{sec:beyond-clay-reach}

The substrate that yields the six Clay axes as coupled sub-stories
also yields a structurally analogous joint treatment of sixteen
\emph{non-Clay} open problems. The pattern is the same: each axis
receives a framework-attack module in the Lean tree at the path
\texttt{PF/NumberTheory/<problem>FrameworkAttack.lean}; the module
exposes the literal published-conjecture encoding, concrete
axiom-free witnesses on small cases, the
$\alpha$-skeleton bridge, the framework's $3^{k}$ substrate match,
and the named published partial results. All sixteen modules are
indexed by a single record,
\texttt{PF.Referee.FrameworkUniversalReach.}
\texttt{framework\_universal\_reach\_realized}
(\texttt{FrameworkUniversalReach.lean}, line~153 at
HEAD~\texttt{85d6bae}), which bundles the sixteen non-Clay axes
together with the seven Clay axes as a single citation point
($23 = 7 + 16$).

\medskip

\noindent\textbf{Bundle-field discipline (load-bearing honest scope).}
Of the 17 fields in the
\texttt{FrameworkUniversalReach} record, \textbf{15 are typed as}
$\mathtt{:\,True}$ (i.e.\ provenness tags carrying no mathematical
content at the record-level aggregation; their substantive content
lives in the per-axis framework-attack modules referenced by the
record file header). The only two fields that carry non-trivial
framework $\mathtt{Prop}$s at the record level are
\texttt{brocard\_attack\_realized}~:~$\mathtt{BrocardFrameworkAttack}$
and
\texttt{hadwiger\_nelson\_attack\_realized}~:~$\mathtt{Hadwiger\\
NelsonFrameworkAttack}$.
The honest-scope reading is therefore: \emph{the record is a single
citation point indexing the per-axis modules; only Brocard and
Hadwiger--Nelson are bundled with non-trivial framework Props at the
record level itself}. The companion paper
\texttt{Papers/clay\_substrate\_reformulation\_v1.tex} \S6 records
the same bundle-field discipline and is the model for the
honest-scope reading; the present subsection mirrors that paper's
prose at chapter level.

\medskip

\noindent\textbf{The sixteen non-Clay axes and their module paths.}
In the order recorded in
\texttt{PF.Referee.FrameworkUniversalReach}:
...
\begin{tabular}{ll}
\toprule
\textbf{Non-Clay axis} & \textbf{Lean module path} \\
\midrule
abc conjecture &
  \texttt{PF/NumberTheory/abcConjectureFrameworkAttack.lean} \\
Beal conjecture &
  \texttt{PF/NumberTheory/BealConjectureFrameworkAttack.lean} \\
Brocard's problem &
  \texttt{PF/NumberTheory/BrocardProblemFrameworkAttack.lean} \\
Collatz conjecture &
  \texttt{PF/NumberTheory/CollatzConjectureFrameworkAttack.lean} \\
Erd\H{o}s discrepancy &
  \texttt{PF/NumberTheory/ErdosDiscrepancyFrameworkAttack.lean} \\
Erd\H{o}s--Straus &
  \texttt{PF/NumberTheory/ErdosStraussFrameworkAttack.lean} \\
Goldbach conjecture &
  \texttt{PF/NumberTheory/GoldbachConjectureFrameworkAttack.lean} \\
Hadwiger--Nelson &
  \texttt{PF/NumberTheory/HadwigerNelsonFrameworkAttack.lean} \\
Inverse Galois problem &
  \texttt{PF/NumberTheory/InverseGaloisProblemFrameworkAttack.lean} \\
Lonely Runner &
  \texttt{PF/NumberTheory/LonelyRunnerFrameworkAttack.lean} \\
Odd perfect numbers &
  \texttt{PF/NumberTheory/OddPerfectNumberFrameworkAttack.lean} \\
Polignac's conjecture &
  \texttt{PF/NumberTheory/PolignacConjectureFrameworkAttack.lean} \\
Twin Prime conjecture &
  \texttt{PF/NumberTheory/TwinPrimeConjectureFrameworkAttack.lean} \\
Singmaster's conjecture &
  \texttt{PF/NumberTheory/SingmastersConjectureFrameworkAttack.lean} \\
Pillai / generalised Catalan &
  \texttt{PF/NumberTheory/CatalanGeneralizedFrameworkAttack.lean} \\
Andrews--Curtis conjecture &
  \texttt{PF/NumberTheory/AndrewsCurtisFrameworkAttack.lean} \\
\bottomrule
\end{tabular}
\end{center}
...
\noindent\textbf{Honest scope (foregrounded).}
The substrate's reach across these sixteen non-Clay open problems
is \emph{suggestive} --- it documents that the same framework
machinery (the $\alpha$-skeleton, the $3^{k}$ substrate, the
$\mathtt{*FrameworkAttack}$ module pattern) applies across a broad
set of open problems, and that the framework's identification of
each problem's substrate signature is internally consistent across
the sixteen modules. \textbf{It is NOT a discharge of any of these
sixteen problems at the literal published-mathematics tier.} The
fourteen non-Clay axes that are \emph{not} Brocard or
Hadwiger--Nelson are inhabited at the record level as
$\mathtt{:\,True}$ placeholders; the substantive structural content
lives in their per-axis modules, cited above by module path. The
substrate-level reading of this subsection is the same as the
honest-scope reading of the six Clay axes elsewhere in this
chapter and at \S\ref{sec:substrate-honest-scope}: the framework
exhibits a single substrate-level mechanism that applies across a
wide span of open problems, and the framework's claim is that the
mechanism is structurally illuminating, \emph{not} that the
underlying problems are discharged at the tier their respective
fields require for resolution.

\section{Honest Scope}
\label{sec:substrate-honest-scope}

\begin{warning}[Honest Scope, Foregrounded]
The Principia Fractalis Substrate Theorem is a
\emph{substrate-level} meta-theorem. It is \textbf{NOT} a literal
...
\texttt{surjectivity} predicate in
\texttt{PF/Referee/RHCapstoneTypedBridge.lean}.
\item \textbf{YM} (C2) --- finite-dimensional $2 \times 2$
mass-gap witness (Wave~55C) plus an infinite-dimensional
$\ell^2$ witness with a toy Hamiltonian (Wave~58+); not the
full Wightman QFT continuum.
\item \textbf{BSD} (C3) --- $\text{Fin}\,6$ LMFDB-restricted
concordance; rank-1 cascade on $E_{37.\text{a}1}$ conditional
on Gross-Zagier and Kolyvagin (both cited, not formalised in
mathlib at HEAD~\texttt{85d6bae}).
...
codim~$\ge 2$ on the general smooth quintic outside the Dwork
locus remains the named Voisin~2007 obstruction.
\item \textbf{P vs NP} (C6) --- enum-level conditional on
\texttt{PolylogEigenvalueConjecture}; the Razborov-Rudich
natural-proofs and Aaronson-Wigderson algebrisation barriers
are preserved.
\end{itemize}
What the meta-theorem \emph{does} establish: the seven Clay
axes plus the cosmology, consciousness, Weinstein-GU rescue,
and counter-rotating-vortex content are NOT seven (plus
$N$) independent objects~--- they are sub-stories of \emph{one}
framework anchored on \emph{one} substrate (the Timeless Field
+ $\alpha$-rigidity + Perelman + IBM + 143). At the framework's
current verification level, each of the twenty-five
consequences is independently axiom-free.
...
checkout of the repository at HEAD commit \texttt{85d6bae}:
\begin{enumerate}
