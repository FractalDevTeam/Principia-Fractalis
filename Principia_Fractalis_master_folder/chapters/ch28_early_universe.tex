...
\emph{Honest scope (2026-07-23): that the CMB and BBN match \textbf{standard} physics is an observational fact independent of this framework; the framework merely \emph{postulates} $\text{ch}_2\approx0$ in the early universe, which then adds nothing to those epochs. The genuinely new content --- late-time consciousness signatures --- is an empirical hypothesis still untested. See the Verification status ledger at the end of the chapter.}
\end{intuitive}
...
\bottomrule
\end{tabular}
\captionof{table}{Cosmic timeline with consciousness evolution. Note: ch$_2 \approx 0$ until complex life emerges around $t \sim 9$ Gyr.}
...
\begin{theorem}[Inflationary Cosmology]\label{thm:inflation}
...
\begin{theorem}[BBN with Consciousness]\label{thm:bbn-consciousness}
...
\begin{theorem}[CMB Acoustic Peaks]\label{thm:cmb-peaks}
...
\textbf{2. Gravitational Lensing}\cite{lewis2000}: CMB photons are lensed by intervening structure at $z < 10$. Consciousness-enhanced structure growth (Chapter \ref{ch:dark-energy-expansion}) increases lensing:
\begin{equation}
...
\begin{theorem}[Linear Perturbation Growth]\label{thm:linear-growth}
...
\begin{theorem}[Late-Time Phase Transition]\label{thm:consciousness-phase-transition}
...
This is the temperature of Earth's biosphere—not a coincidence! \emph{Honest scope (2026-07-23): this order-of-magnitude coincidence is \textbf{asserted, not derived} --- the input rates ($10^{15}$ bits/s, $10^{10}\,k_B$) are themselves rough estimates, and no independent test establishes a consciousness ``critical temperature.''}
...
Held to the standard of the machine-checked corpus, the cosmological claims of this
chapter are empirical, not derived facts:
\begin{itemize}
\item \textbf{EMPIRICAL HYPOTHESIS --- untested / not independently validated} ---
  the late-time consciousness phase transition at $z\sim0.5$
  (Thm~\ref{thm:consciousness-phase-transition}) and its predicted signatures
  (ISW enhancement, CMB-lensing excess, growth-factor kink, discontinuity in
  $w_{\text{DE}}$). These await Roman/LSST/CMB-S4 data; none is confirmed, and none
  is machine-verified.
\item \textbf{CONSISTENCY, not confirmation} --- the ``successful predictions'' that
  BBN (Thm~\ref{thm:bbn-consciousness}) and the primary CMB carry no consciousness
  correction. These follow trivially from the \emph{postulate} $\text{ch}_2\approx0$
  at early times; they test standard physics, not the consciousness field, so they
  provide no positive evidence for the framework.
\item \textbf{ASSERTED (numerology / contested input)} --- the consciousness
  ``critical temperature'' $T_c\sim10^5$ K equated with the biosphere temperature,
  and the IIT threshold $\Phi_c\approx3.0$ bits. Both are asserted from rough
  estimates and rest on the (empirically contested) integrated-information theory of
  consciousness.
\item \textbf{No kernel-verified physics here} --- nothing in this chapter is part
  of the machine-checked substrate; the only formalized cosmological statements in
  the corpus are the definitional constant brackets cited in
