...
\begin{intuitive}[What Does "Timeless" Mean?]
...
\begin{keyidea}[The Shift from Objects to Algebra]
...
\begin{level2}[The Right Mathematical Language]
...
\subsection{Level-\texorpdfstring{$k$}{k} Hilbert Spaces}
...
\begin{intuitive}[Understanding the Scaling]
Why $3^k$ dimensions?
\begin{itemize}
...
\begin{example}[Explicit Basis at Level 2]
...
\begin{intuitive}[What Does This Algebra Encode?]
...
\begin{level2}[Minimal vs Maximal Tensor Products]
...
\begin{intuitive}[What Do Connecting Morphisms Do?]
...
\begin{level3}[Existence and Uniqueness Theorem]
...
\end{equation}
This limit exists due to compatibility and defines a trace. \textbf{Faithfulness of $\tau$ --- that $\tau(x^*x) = 0$ forces $x = 0$ --- is \emph{not} automatic}: earlier drafts of this chapter asserted it in passing, but it is in fact the deep part of the statement, equivalent to the $C^*$-simplicity of $\mathcal{T}_\infty$ (Glimm 1960). As of July 2026 both are \textbf{machine-verified, unconditional theorems} in the project's Lean~4 corpus: \texttt{UHF\_trace\_faithful} and \texttt{substrate\_completion\_simple\_unconditional} (development arc r102--r112), kernel-checked with zero project axioms. The proof constructs a trace-preserving conditional expectation $E_k: \mathcal{T}_\infty \to A_k$ (the tail partial trace), establishes its $\|\cdot\|_2$- and operator-norm contractivity, and concludes $\tau(x^*x)=0 \Rightarrow \forall k\, E_k x = 0 \Rightarrow x=0$. See the companion paper \emph{Formalizing a Faithful Tracial State on the UHF Algebra $M_{3^\infty}$} (\texttt{Papers/uhf\_faithful\_trace\_glimm\_2026-07-23}) for the full development.
...
\paragraph{Machine-verification status of Theorem~\ref{thm:existence-uniqueness} (updated 2026-07-23).}
To keep this chapter to the same standard as the formal corpus, here is the honest ledger of which parts of the theorem are kernel-verified in Lean~4 and which rest on classical literature:
\begin{itemize}
\item \textbf{Formalized, unconditional} (Lean~4, mathlib \texttt{v4.24.0-rc1}, kernel axioms $[\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}]$ only): the construction of $\mathcal{T}_\infty$ as inductive limit + completion with a \texttt{CStarAlgebra} instance; the canonical trace $\tau$ with additivity, unitality, 1-Lipschitz continuity, traciality, Hermiticity, positivity; \textbf{faithfulness of $\tau$} (\texttt{UHF\_trace\_faithful}); \textbf{$C^*$-simplicity} of $\mathcal{T}_\infty$ --- every two-sided ideal is trivial (\texttt{substrate\_completion\_simple\_unconditional}, the Glimm 1960 theorem); the trace-preserving conditional expectations $E_k$ with their contraction properties.
\item \textbf{Classical, by reference --- not formalized}: nuclearity of $\mathcal{T}_\infty$ (and everything downstream of it in Lemma~\ref{lem:nuclear-structure}: tensor uniqueness, amenability, Connes embedding); uniqueness of $\mathcal{T}_\infty$ up to isomorphism (the Glimm classification of $M_{3^\infty}$); uniqueness of the tracial state. These are standard results we cite, not results we have machine-checked; formalizing them is future work.
\end{itemize}
\end{level3}
...
\noindent\emph{Verification note (2026-07-23): this lemma is classical-by-reference. Nuclearity of $\mathcal{T}_\infty$ and its consequences are \textbf{not} part of the machine-verified corpus; the kernel-checked properties of $\mathcal{T}_\infty$ are those listed in the verification ledger following Theorem~\ref{thm:existence-uniqueness} --- notably the faithful tracial state and $C^*$-simplicity, which \emph{are} formalized.}

\begin{keyidea}[Why Nuclearity Matters]
...
\begin{level2}[Interpreting K-Theory]
...
\end{thm}\manuscriptcorrection{This construction collapses, and the collapse is machine-checked. $\text{Aut}_0$ is not a proper subgroup: theorem \texttt{trace\_preserved} (file \texttt{PF/SubstrateAutomorphismQuotientCollapse.lean}, r333, commit \texttt{15c5c4df}) proves that \emph{every} continuous unital $\mathbb{C}$-algebra endomorphism of $\mathcal{T}_\infty$ preserves the canonical trace --- because r113 (\texttt{substrate\_UHF\_trace\_unique}) had already established that $\tau_{\text{UHF}}$ is the \emph{unique} tracial state on $\mathcal{T}_\infty$. Uniqueness is exactly what makes the trace useless here. Hence $\text{Aut}_0=\text{Aut}$ and the quotient is a single point: \texttt{inducedTraces\_encard\_eq\_one} computes its cardinality as $1$, and \texttt{no\_two\_distinct\_inducedTraces} rules out even two classes. This is not the group being trivial. \texttt{trace\_blind\_to\_nontrivial\_endo} (r334, commit \texttt{20c0a587}) exhibits a non-identity inner automorphism --- conjugation by the unipotent unit $1+E_{01}$, an instance of the very ``unitary conjugation'' listed in the box below --- which induces exactly the same trace functional. The trace is blind, not vacuous, and the quotient therefore discards structure $\mathcal{T}_\infty$ demonstrably has. The obvious repair, replacing the trace by the $K$-theoretic trace range on projections, fails the same way: \texttt{nontrivial\_endo\_fixes\_idemTraces} (r335, commit \texttt{d74395ee}) proves that same automorphism fixes the idempotent trace range setwise. All three results are axiom-free ([\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}]). This theorem must be withdrawn as stated; no replacement is claimed. The quotient by the \emph{inner} automorphisms, $\text{Out}(\mathcal{T}_\infty)=\text{Aut}/\text{Inn}$, is untouched by these results and remains the live candidate --- note that the ``Open Problem: Explicit Automorphism Classification'' note later in this chapter already lists that classification of $\text{Aut}(\mathcal{T}_\infty)$ as open, which is inconsistent with using it to compute a dimension here.}

\begin{intuitive}[Understanding Automorphisms]
...
\begin{level2}[Why Four Dimensions?]
The dimension count comes from:
\begin{equation}
\dim(\mathcal{M}^4) = \dim(\text{Aut}(\mathcal{T}_\infty)) - \dim(\text{Aut}_0(\mathcal{T}_\infty))
\end{equation}\manuscriptcorrection{This subtraction is void. By r333 (see the correction on Theorem~\ref{thm:spacetime-emergence}), $\text{Aut}_0=\text{Aut}$, so the right-hand side is a quantity minus itself. The text on the following line concedes that $\dim\text{Aut}(\mathcal{T}_\infty)$ is infinite, so the expression is $\infty-\infty$ in any reading. Neither the 3 spatial dimensions nor the 1 time dimension is derived here; both are asserted and then attributed to the quotient. Separately, the third example in the box above, $\alpha_\theta(a)=e^{i\theta}ae^{-i\theta}$, is the identity map for every $\theta$, since scalars are central in $\mathcal{T}_\infty$; it is not a symmetry and cannot contribute a dimension.}
...
\begin{keyidea}[Forces Are Symmetries]
...
\begin{example}[Electromagnetism from $U(1)$]
...
\begin{intuitive}[The Holographic Principle]
...
\begin{keyidea}[Consciousness as Geometry]
...
\begin{level3}[Open Problem: Explicit Automorphism Classification]
