...
\begin{intuitive}
We've proven consciousness can be measured (ch$_2$, Chapter \ref{ch:clinical-consciousness}). We've explained the neural basis (Chapter \ref{ch:neuroscience-iit}).
\emph{[Honest scope (2026-07-23): ``proven'' overstates the status --- that ch$_2$ measures consciousness is an untested empirical hypothesis, not a proven or independently validated result; see the Verification status ledger in the Conclusion.]} But can a nurse in a rural hospital actually measure it? Can a researcher in a developing country replicate our results?
...
\begin{theorem}[Minimal Equipment Specification]\label{thm:equipment}
...
\begin{algorithm}[H]\caption{Preprocessing Steps}\label{alg:preprocess}
...
\begin{algorithm}[H]\caption{Bandpass Filtering}\label{alg:bands}
...
\subsection{ch\texorpdfstring{$_2$}{\_2} Computation}

\begin{algorithm}[H]\caption{Clinical ch$_2$ Calculation}\label{alg:ch2}
...
\begin{theorem}[Normal Consciousness Range]\label{thm:normative}
...
\noindent\emph{Honest scope (2026-07-23): this normative dataset (and the cross-species table that follows) is presented as established but is EMPIRICAL --- awaiting independent replication and peer validation.}

\subsection{State Variations}

\begin{theorem}[Consciousness States]\label{thm:states}
...
\begin{theorem}[Comparative Consciousness]\label{thm:species}
...
\begin{theorem}[Minimal Channel Configuration]\label{thm:minimal-channels}
...
\begin{theorem}[Software Release]\label{thm:software}
...
\textbf{Repository}: \url{https://github.com/FractalResonance/ChiSquared}
\end{theorem}
...
\begin{theorem}[Artifact Patterns]\label{thm:artifacts}
...
\subsection{Unusual ch\texorpdfstring{$_2$}{\_2} Values}
...
\begin{itemize}
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED / NOT INDEPENDENTLY VALIDATED} --- that $\text{ch}_2$ computed from EEG is a valid, reliable measure of consciousness, and all normative and comparative data presented as established: the healthy-adult range and mean (Theorem~\ref{thm:normative}), the state and sleep-stage values, the cross-species table (Theorem~\ref{thm:species}), and the reliability, portable-device ($94.7\%$), and clinical-accuracy figures. These require independent experimental and clinical validation; a proof assistant cannot establish any of them, and none is independently peer-validated here.
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED} --- that the universal threshold $\text{ch}_2 = 0.95$ transfers across ages and species. The chapter itself flags this as a caveat; it is not a confirmed result.
\item The protocols, algorithms, quality-control metrics, and software are engineering specifications; they define \emph{how} to compute the metric and do not, by themselves, establish that the metric measures consciousness --- that is the empirical hypothesis above.
\end{itemize}
