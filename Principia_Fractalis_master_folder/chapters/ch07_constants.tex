...
\section{The Spectral Gap \texorpdfstring{$\Delta = 0.0891219046...$}{Delta = 0.0891219046...}}
...
The spectral gap $\Delta = 0.0891$ is the \textit{minimum energy advantage} that Hiker NP has over Hiker P. It's the irreducible cost of quantum superposition---the price nature pays for parallel exploration of possibilities.
\end{intuitive}
...
\begin{equation}
\boxed{\Delta = \lambda_1^{\mathbf{NP}} - \lambda_1^{\mathbf{P}} = 0.0891219046...}
\end{equation}
where $\lambda_1$ denotes the first non-zero eigenvalue of the respective transfer operators.
\end{thm}

\begin{level2}[Detailed Calculation]
...
\subsection{Critical \texorpdfstring{$\alpha$}{alpha} Values}
...
\section{The Consciousness Threshold \texorpdfstring{$\text{ch}_2 = 0.95$}{ch\_2 = 0.95}}
...
\subsection{Fine Structure Constant \texorpdfstring{$\alpha_{\text{EM}}$}{alpha\_EM}}
...
\end{equation}
where $R_f(1,2)$ is the resonance function evaluated at linear resonance ($\alpha=1$) and dimension $s=2$.\manuscriptcorrection{The Lean closed-form identity \texttt{R\_f\_odd\_eq\_neg\_eta} (file \texttt{PF/RfIntegerAlphaDichotomy.lean}, Wave~55, commit \texttt{496193c}) proves $R_f(1,s) = -\eta(s)$ for every $s\in\mathbb{C}$, where $\eta$ is the Dirichlet eta function. Specialising to $s=2$ gives $R_f(1,2) = -\eta(2) = -\pi^2/12 \approx -0.8225$ — \emph{negative} and roughly $35\times$ larger in magnitude than the heuristic value $\approx 0.02338$ reported below. The companion theorem \texttt{R\_f\_one\_two\_ne\_manuscript\_value} establishes the inequality axiom-free. The fine-structure identification on the displayed line therefore cannot hold with the literal definition of $R_f$ used elsewhere in the framework; recovering $\alpha_{\text{EM}}^{-1}\approx 137.036$ requires either a different definition of $R_f$ at $(\alpha=1,s=2)$ (e.g.\ a regularised or alternating variant) or a different combination of the two factors. The original numerical paragraph below is preserved verbatim for historical reference.}
\end{thm}
...
\subsection{Gravitational Constant \texorpdfstring{$G$}{G}}
...
\begin{itemize}
\item \textbf{ASSERTED (not derived)} --- the universal factor $\pi/10$ (Theorem~\ref{thm:pi-ten-scaling}) and the ``sacred'' resonance values $\alpha_P=\sqrt{2}$, $\alpha_{NP}=\varphi+\tfrac14$, $\alpha=3/2$, etc.\ (Theorem~\ref{thm:sacred-necessity}, Table~\ref{tab:sacred-alpha}). These are \emph{chosen/definitional} constants of the framework, not consequences of first principles; the ``derivation from polylogarithms'' and the necessity arguments are heuristic. See \texttt{codex/DERIVATION\_ANALYSIS\_alpha\_NP.md}: the value $\varphi+1/4$ is a conjecture supported by numerics, with four named derivation gaps still open.
\item \textbf{ASSERTED (not derived)} --- the spectral gap $\Delta = 0.0891219046\ldots$ (Theorem~\ref{thm:p-np-gap}). It is arithmetic about the two asserted values $\alpha_P,\alpha_{NP}$; it does \emph{not} establish $\mathbf{P}\neq\mathbf{NP}$, which remains an open Millennium problem (the identification of $\Delta$ with a complexity separation is a conditional reduction; see Ch~\ref{ch:spectral_unity}).
\item \textbf{PROVEN-as-arithmetic (only)} --- base-3 radix optimality (Theorem~\ref{thm:base-3-optimal}): $Q[b]=(\ln b)/b$ is maximised at $b=e$ and, among integers, at $b=3$. This is an elementary calculus fact and is true. It is \emph{not} a proof that physical reality is ternary; that reading is framework-internal.
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED / NOT INDEPENDENTLY VALIDATED} --- the consciousness threshold $\text{ch}_2=0.95$ and its ``four independent derivations.'' The percolation route is admitted in-text to require a sign reinterpretation, the information-theoretic and spectral routes assume the conclusion, and the EEG figure awaits independent replication. No proof assistant can settle this.
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED} --- the fine-structure identity $\alpha_{\text{EM}}=R_f(1,2)\cdot\pi/10$ (Theorem~\ref{thm:fine-structure}); see the inline \texttt{manuscriptcorrection} note, where the Lean identity $R_f(1,s)=-\eta(s)$ makes the literal formula fail. The Hubble-tension and quasicrystal ``comparative alignments'' are self-labelled \emph{Predicted}/\emph{Proposed} and await DESI~DR3 / archival diffraction data.
\end{itemize}
\emph{None of the constants in this chapter are kernel-verified physics; the only machine-checked scientific content in the corpus is the operator-algebra substrate underlying $\mathcal{T}_\infty$ (Ch~\ref{ch:timeless-field}).}
...
% Third tier
\node[ellipse, draw, fill=red!10, align=center] (PHYS) at (10,1) {Physical\\Constants};
\node[ellipse, draw, fill=red!10, align=center] (CONSC) at (10,-1) {Consciousness\\Emergence};
