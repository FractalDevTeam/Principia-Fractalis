...
\begin{intuitive}[Why Do We Use Ten Fingers?]
...
\begin{equation}
11011_2 = 1\times 16 + 1\times 8 + 0\times 4 + 1\times 2 + 1\times 1 = 27
\end{equation}
...
\begin{equation}
1000_3 = 1\times 27 + 0\times 9 + 0\times 3 + 0\times 1 = 27
\end{equation}

\begin{intuitive}[Understanding Any Base]
...
\begin{keyidea}[The Magic of Three]
...
\begin{example}[Converting 27 to Base-3]
\textbf{Method:} Repeatedly divide by 3 and track remainders:

\begin{align}
27 \div 3 &= 9 \text{ remainder } \boxed{0} \\
9 \div 3 &= 3 \text{ remainder } \boxed{0} \\
3 \div 3 &= 1 \text{ remainder } \boxed{0} \\
1 \div 3 &= 0 \text{ remainder } \boxed{1}
\end{align}
...
\textbf{Verification:}
\begin{equation}
1\times 3^3 + 0\times 3^2 + 0\times 3^1 + 0\times 3^0 = 1\times 27 + 0 + 0 + 0 = 27 \checkmark
\end{equation}
\end{example}

\begin{tryit}[Your Turn]
...
\begin{definition}[Digital Sum in Base-3]
...
\begin{example}[Computing $D_3(27)$]
...
\begin{example}[Computing $D_3(10)$]
...
\begin{intuitive}[What Do You Notice?]
...
\begin{historicalnote}[Leibniz and Binary]
...
\begin{intuitive}[What Do You See?]
...
\begin{keyidea}[Fractal Self-Similarity]
...
\begin{example}[Running the Code]
...
\begin{theorem}[Self-Similarity Property]
...
\begin{theorem}[Addition Property]
...
\begin{theorem}[Modular Property]
...
\begin{historicalnote}[Casting Out Nines]
...
\begin{advanced}[Fractal Dimension]
...
\begin{advanced}[Connection to $p$-adic Numbers]
...
\subsection{Divisibility by 2 Using \texorpdfstring{$D_3(n)$}{D\_3(n)}}
...
\begin{intuitive}[The Parity Rule]
Look at the pattern:
\begin{itemize}
...
\begin{theorem}[Digital Sum Modulo $(b-1)$]
...
\begin{theorem}[Base-3 Parity Rule]
...
\begin{example}[Using the Parity Rule]
...
\begin{keyidea}[Divisibility Rules in Different Bases]
\begin{itemize}
...
\begin{intuitive}[The Missing Rule]
...
for n in range(1, 21):
    d3 = D3(n)
    match = "OK" if (n % 2 == d3 % 2) else "X"
    print(f"{n}\t{d3}\t{n%2}\t{d3%2}\t{match}")
\end{lstlisting}

\textbf{Output:} All 20 cases show \texttt{OK}, confirming $n \equiv D_3(n) \pmod{2}$.
...
\begin{intuitive}[The Pattern Repeats]
...
\begin{theorem}[Scaling Law for $D_3$]
...
\begin{example}[Scaling in Action]
...
\begin{intuitive}[Fractals in Nature and Mathematics]
...
\begin{keyidea}[Logarithmic Growth]
...
\begin{example}[Recursive Computation]
...
\paragraph{Verification update (2026-08-05): the Hausdorff dimension of the
Cantor set is now kernel-certified (arc r205--r209).}
The value $\log 2/\log 3$ quoted informally in this chapter --- and in the
box-counting remark of Section~\ref{sec:digital-sum-patterns} --- is, for the Cantor
set itself, now a machine-checked theorem rather than a citation. The record
is \texttt{codex/FRACTAL\_DIMENSION\_2026-08-05.md}. Every result below is
...
\begin{itemize}
\item \textbf{The Moran/Falconer upper bound (r205).} A general covering
engine \texttt{dimH\_le\_of\_bounded\_covers}, with no dynamics in it, and
above it the classical iterated-function-system bound: for a bounded set
covered by the images of its contracting Lipschitz branches,
$\sum_j \mathrm{Lip}(\varphi_j)^d \le 1$ implies $\dim_H \le d$. mathlib has
\texttt{dimH} but no IFS, Moran, or thermodynamic-formalism theory at the pin;
that layer was built here.
\item \textbf{The mass distribution principle and H\"older transfer (r206).}
\texttt{le\_dimH\_of\_massDistribution} restates mathlib's
\texttt{le\_hausdorffMeasure} with a constant, and
\texttt{le\_dimH\_of\_holder\_surj} gives $r \cdot \dim_H F \le \dim_H E$ for a
H\"older-$r$ surjection $E \to F$. The arithmetic identity
$(1/3)^{\log_3 2} = 1/2$ makes the Moran sum for the Cantor set exactly $1$,
so the upper bound sits precisely at the classical value, with no slack.
\item \textbf{The result (r207).}
\[
\texttt{dimH\_cantorSet} \;:\; \dim_H(\mathcal{C}) = \log_3 2
\approx 0.63092975357,
\]
on mathlib's own \texttt{cantorSet}. To our knowledge the Hausdorff dimension
of the Cantor set --- of any nontrivial fractal --- had not previously been
formalized in a proof assistant. The lower half required the \emph{Cantor
function}, which mathlib does not have; it is constructed here as a
telescoping sum of clamped approximants, proved continuous, monotone, and to
satisfy both functional equations, then shown globally H\"older with exponent
$\log_3 2$, and its image proved to be exactly $[0,1]$. The whole argument is
measure-free: no Cantor--Lebesgue measure, no Frostman lemma, no product
measures. A side result mathlib also lacks, $1 \in \mathcal{C}$, was proved
along the way.
\item \textbf{Certified enclosures for the continued-fraction sets
(r208, r209).} For $E_K$, the set of reals in $[0,1]$ whose continued-fraction
digits are all at most $K$, the branches are M\"obius rather than
similarities, so the dimension is the root of no finite Moran equation and
only enclosures are available. Level-2 refinement gives the upper bounds, and
a Bernoulli weighted-address map --- built by the same clamped-telescoping
technique as the Cantor function, with no measure constructed --- gives the
lower bounds:
\[
0.54 \le \dim_H(E_3) \le 0.77,
\qquad
0.39 \le \dim_H(E_2) \le 0.58 .
\]
\end{itemize}

\noindent \textbf{What is not proved.} The Gauss enclosures do not close. The
true values, $\dim_H(E_3) = 0.7056609$ and $\dim_H(E_2) = 0.5312805$
(Jenkinson--Pollicott), lie strictly inside the certified intervals, and the
certified endpoints must not be quoted as approximations to them. Closing the
gap requires the equilibrium state --- Ruelle--Perron--Frobenius theory, which
exists in no proof assistant --- and that work is not started. Nothing in this
arc bears on the framework's wider claims: it is a dimension computation for
two specific self-similar sets, and it says nothing about $D_3(n)$, whose
box-counting statement above remains a citation.
...
\begin{exercise}
\textbf{(Iterated Function System)} Show that the sequence $(D_3(n))_{n \geq 1}$ can be generated by an iterated function system (IFS) with three contractive maps. Describe these maps explicitly. For background on IFS, see \cite{barnsley1988,hutchinson1981fractals}.
\end{exercise}
...
To hold this chapter to the standard of the machine-checked corpus, here is the honest status of its principal claims:
\begin{itemize}
\item \textbf{STANDARD MATHEMATICS} --- the scaling law $D_3(3^k n) = D_3(n)$ (Theorem~\ref{thm:d3-scaling}), the recursive/addition identities, and the base-$b$ digit-sum congruence $n \equiv D_b(n) \pmod{b-1}$ with its base-3 parity specialization (Theorem~\ref{thm:digital-sum-modular}, Theorem~\ref{thm:parity-rule}) are classical elementary number theory. The proofs given here are correct and complete; they are ``standard-by-reference'' rather than part of the project's Lean~4 kernel corpus, which formalizes the operator-algebra substrate (Chapter~\ref{ch:timeless-field}), not base-3 digit arithmetic.
\item \textbf{STANDARD MATHEMATICS} --- the self-similar/``fractal'' description of $D_3(n)$ and its logarithmic growth $\max_{m\le n} D_3(m)\sim 2\log_3 n$ are elementary consequences of the base-3 representation; the fractal language is descriptive, not a claim requiring machine verification.
\item \textbf{ASSERTED (framework interpretation)} --- the previewed claim that $D_3$ is the ``seed'' of a resonance function that ``solves the Millennium Problems'' is a thesis about later chapters, not a result of this chapter; its status is treated where those claims are made (see the corresponding chapters' verification ledgers).
\end{itemize}
...
\begin{theorem}[Divisibility by 2 via $D_3$]
...
\begin{definition}[Digital Sum Hash]
