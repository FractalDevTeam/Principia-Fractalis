...
\section{The 14D Trace Anomaly and \texorpdfstring{$|\Psi_{\text{RQG}}|^2 = 0.95$}{|Psi\_RQG|\^{}2 = 0.95}}
...
\end{equation}
where $\langle \cdot \rangle$ denotes expectation value over $\mathcal{P}^{13}$.\manuscriptcorrection{Direct arithmetic with $(4\pi)^7 \approx 4.9518\times10^7$, $A_{14}=8174$, $\langle\Delta\Phi\rangle\approx -10^7\Phi_0$, and $\langle R^2\rangle\approx 10^{14}$ yields $\bigl|(4\pi)^7\cdot 10^7\bigr|/(8174\cdot 10^{14}) \approx 6.05\times 10^{-4}\Phi_0$, NOT $0.95\,\Phi_0$ — i.e.\ the stated closed form is off by a factor of $\approx 1570$. Theorem \texttt{anomaly\_cancel\_predicted\_value\_ne\_0\_95} (file \texttt{PF/Ch11AnomalyCancellationRefutationAttempt.lean}, Wave~55, commit \texttt{8e4a62a}) proves this inequality axiom-free by bracketing the predicted value below $1/1000$. The threshold $\text{ch}_2=0.95$ does have other independent derivations in the framework (Ch~6 Chern--Weil; Ch~31 $\text{ch}_2\leftrightarrow\Phi$ bridge), so the structural claim survives even though THIS particular 14D anomaly route fails its stated numerical close.}
\end{theorem}
...
\end{equation}\manuscriptcorrection{The Gaussian integral evaluates exactly to $\sqrt{5/(\pi+5)} = \sqrt{5/8.1416\ldots} \approx 0.7837$, NOT $0.95$. Theorem \texttt{prop\_11\_6\_psi\_rqg\_sq\_ne\_0\_95} (file \texttt{PF/Ch11AnomalyCancellationRefutationAttempt.lean}, Wave~55, commit \texttt{8e4a62a}) proves this inequality axiom-free: squaring the alleged equality would force $5/(\pi+5) = 9025/10000$, contradicting $\pi > 3.14$. Therefore the ``twice-determined'' overdetermination argument on line~204 fails: BOTH the anomaly leg (off by $\approx 1570\times$) AND the Gaussian leg (gives $0.7837$, not $0.95$) miss their stated targets. The threshold $\text{ch}_2=0.95$ should be read as set by the Ch~6 Chern--Weil derivation, not by Ch~11's $\Psi_{\text{RQG}}$ Gaussian integral.}
\end{proposition}
...
This yields $\approx 1.6$, not the required $4$.

\noindent\emph{Derivation-status disclosure, 2026-05-18: Prior editions attempted a ``corrected'' calculation: $\dim_{\text{visible}} = 4\cdot 1 + 9\cdot 0.95 = 4 + 8.55 \approx 4$. The arithmetic does not work: $4 + 8.55 = 12.55$, not $\approx 4$. Both the first calculation ($\approx 1.62$, equation above) and the second ($= 12.55$) fail to produce the desired ``4 visible dimensions'' result. The observation that we live in $\approx 4$ macroscopic dimensions is empirical; a first-principles derivation from the $13$-dimensional principal bundle remains an open problem of this chapter.}

Complete rigorous proof attempted in \cite{cohen2025weinstein}, Section 4 (status as open derivation per above disclosure).
\end{proof}
...
This first count gives $63$, not the $78 = \dim E_6$ required by the BRST theorem above; the discrepancy is resolved by counting physical degrees of freedom (polarizations and chiralities) rather than particle species. \emph{(Derivation-status note, 2026-08-01: the target value $78$ was fixed in advance by the $\dim E_6$ identification, and the counting scheme below was selected to reach it. The DOF count is arithmetically correct as stated, but the \emph{choice} of scheme --- species for the first count, polarizations for the second --- is made after the fact. An earlier draft recorded this step verbatim as a mid-derivation course correction; that trace is preserved in the repository history.)} Recounting by degrees of freedom:
\begin{align}
...
\begin{itemize}
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED} --- the entire RQG completion of Geometric Unity: the regularised shiab operator (Theorem~\ref{thm:rqg_shiab_welldefined}), the holographic $13\text{D}\to4\text{D}$ projection (Theorem~\ref{thm:holographic_projection}), and the BRST count $\dim H^2=78$ (Theorem~\ref{thm:rqg_cohomology}). These are framework-internal constructions, not kernel-verified; the projection's own proof carries a 2026-05-18 disclosure that the dimension arithmetic does not yield 4 and that the 4D count is empirical input.
\item \textbf{ASSERTED --- refuted at the stated numerical close} --- the two routes claiming $\text{ch}_2=0.95$: the 14D anomaly cancellation (Theorem~\ref{thm:anomaly_cancel}) and the Gaussian $\langle|\Psi_{\text{RQG}}|^2\rangle$ (Proposition~\ref{prop:rqg_mean}). Both carry inline \texttt{manuscriptcorrection} notes with axiom-free Lean refutations: the anomaly route misses by $\approx1570\times$ and the Gaussian route yields $\approx0.7837$, not $0.95$. The threshold survives only via its independent Ch~6 Chern--Weil route, not via this chapter.
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED / NOT INDEPENDENTLY VALIDATED} --- the anomaly ``resolutions'' (muon $g-2$, Hubble tension, ANITA events, primordial lithium, XENON $^{127}$Xe). Several are back-fitted so the RQG correction reproduces the measured value; the agreement claimed here is not independent confirmation.
\item \textbf{ASSERTED (not derived)} --- $\pi/10$, $\text{ch}_2=0.95$, and $R_f$ throughout inherit their definitional status from Ch~\ref{ch:constants}.
\end{itemize}
\emph{Nothing in this chapter is kernel-verified physics; the machine-checked corpus covers only the $\mathcal{T}_\infty$ operator-algebra substrate (Ch~\ref{ch:timeless-field}).}
...
\subsection{The \texorpdfstring{$\pi/10$}{pi/10} Signature Across Scales}
...
\section{Photonic Frame-Dragging and Temporal Curvature: The Mallett--\texorpdfstring{$\Phi$}{Phi} Correspondence}
