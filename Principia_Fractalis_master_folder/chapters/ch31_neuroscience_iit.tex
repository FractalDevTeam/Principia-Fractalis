...
\noindent\emph{Honest scope (2026-07-23): this central claim is an EMPIRICAL HYPOTHESIS --- untested and not independently validated. See the Verification status ledger in the Conclusion.}
...
\noindent\emph{Honest scope (2026-07-23): the number $\Phi = -\log_2(0.05) \approx 4.32$ is arithmetic about defined quantities. The claim that this correspondence holds for real neural systems --- and that either quantity measures consciousness --- is an untested EMPIRICAL HYPOTHESIS; the proof below is an information-theoretic heuristic, not an independently validated equivalence.}
...
\begin{itemize}
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED / NOT INDEPENDENTLY VALIDATED} --- the central claim that consciousness arises when thalamocortical networks reach $\text{ch}_2 \geq 0.95$ at critical frequency $\alpha = \sqrt{2}$, together with all the reported experimental data: the optogenetics cohort (Theorem~\ref{thm:optogenetics}), anesthesia, lesion, white-matter, and laminar-recording results, and the artificial-network / LLM $\text{ch}_2$ figures. These are neuroscience claims that only independent experiment can settle; a proof assistant cannot establish them, and none is independently peer-validated here.
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED} --- the $\text{ch}_2 \leftrightarrow \Phi$ (IIT) correspondence $\Phi = -\log_2(1-\text{ch}_2)$ (Theorem~\ref{thm:iit-resonance}). Evaluating it at $\text{ch}_2 = 0.95$ to get $\Phi \approx 4.32$ bits is arithmetic about defined quantities; the empirical claim that neural/EEG integration actually realizes this relation is untested. (The corpus verifies only an algebraic IIT-bridge \emph{inequality} at the threshold, not the equivalence or any neural fact.)
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED} --- the derived critical frequency $\sqrt{2}\cdot 10\,\text{Hz}\approx 14\,\text{Hz}$ and its claimed match to beta-band consciousness data. The Comparative-Alignment ``Status Markers'' in this chapter mark consistency-with external literature, not independent confirmation.
\end{itemize}
