...
All code in this chapter is available in the companion repository at \url{https://github.com/pablocohen/fractal-resonance-code}.
...
\begin{definition}[ADM Variables]\label{def:adm-variables}
...
\begin{theorem}[ADM Evolution with Consciousness]\label{thm:adm-consciousness}
...
\begin{definition}[BSSN Variables]\label{def:bssn-variables}
\begin{align}
...
\begin{definition}[Finite Difference Approximations]\label{def:finite-differences}
...
\begin{definition}[Fourier Spectral Method]\label{def:fourier-spectral}
...
\begin{definition}[Convergence Order]\label{def:convergence-order}
...
\begin{itemize}
\item \textbf{PROVEN-as-arithmetic / standard numerics} --- the underlying numerical machinery (ADM and BSSN formulations, finite-difference and spectral discretisations, Runge--Kutta integration, Metropolis Monte Carlo, convergence testing) is established computational physics, correct on its own terms. It is not project-specific Lean-verified content, and its \emph{correctness as code} is independent of whether consciousness gravity is real.
\item \textbf{EMPIRICAL HYPOTHESIS --- UNTESTED} --- everything these methods are applied to that is consciousness-specific: the ADM evolution with consciousness source terms (Theorem~\ref{thm:adm-consciousness}), the ``binary consciousness merger'' waveform, and the stress-energy template $C^{\mu\nu}=\text{ch}_2\cdot R_f\cdot T^{\mu\nu}_{\text{template}}$. These inherit the unverified physics of Ch~\ref{ch:field-equations}; the predicted signal ratios ($h_C/h_{\text{GR}}\sim10^{-60}$) are unobservable and unconfirmed.
\item \textbf{ASSERTED (not derived)} --- the ternary-computing ``comparative alignment'' rests on the base-3 radix-economy fact (true arithmetic, Ch~\ref{ch:constants}); the claimed energy-per-operation advantage is self-labelled \emph{Predicted} and awaits hardware verification.
\end{itemize}
\emph{No consciousness-coupled result computed here is kernel-verified physics; only the operator-algebra substrate of $\mathcal{T}_\infty$ (Ch~\ref{ch:timeless-field}) is machine-checked.}
