...
% Clay-standard encoding shorthand used in ch34A's per-axis honest-scope list.
\newcommand{\Cstd}[1]{C^{\mathrm{std}}_{\mathrm{#1}}}
% \captionof for tables inside tcolorbox environments (floats are illegal there).
\usepackage{capt-of}
% Some TeX Live versions predefine \rank (same failure mode as \Tr above).
\let\rank\relax
