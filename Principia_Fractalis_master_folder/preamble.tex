...
% Fonts and Unicode support -- engine-aware.
% pdflatex: needs inputenc (utf8) + fontenc (T1) + \DeclareUnicodeCharacter
%   declarations to map UTF-8 source chars (×, α, π, ✓, emoji etc.) to
%   LaTeX commands.
% lualatex / xelatex: read UTF-8 natively via fontspec; \DeclareUnicodeCharacter
%   from inputenc is not applicable and errors out. We therefore guard the
%   inputenc + declaration block behind \ifPDFTeX.
\usepackage{iftex}
\ifPDFTeX
  ...
  \usepackage{lmodern}
\else
  % lualatex / xelatex path: fontspec replaces fontenc+inputenc.
  \usepackage{fontspec}
  \usepackage{unicode-math}
\fi

% The \DeclareUnicodeCharacter calls below are pdflatex-only.
% Under lualatex/xelatex they would error; wrap entire block in \ifPDFTeX.
\ifPDFTeX
...
\DeclareUnicodeCharacter{039B}{\ensuremath{\Lambda}} % Lambda
\DeclareUnicodeCharacter{03A9}{\ensuremath{\Omega}} % Omega
\DeclareUnicodeCharacter{2713}{\checkmark} % Heavy check mark (used in version_history)
\DeclareUnicodeCharacter{2714}{\checkmark} % Heavy check mark variant
\DeclareUnicodeCharacter{2718}{\ensuremath{\times}} % Heavy ballot X (✘)
\DeclareUnicodeCharacter{2715}{\ensuremath{\times}} % Multiplication X (✕)
\DeclareUnicodeCharacter{2716}{\ensuremath{\times}} % Heavy multiplication X (✖)
% CJK characters that appear in chapter prose -- map to romanized fallback so
% pdflatex doesn't require CJK fonts. The romanization is approximate.
\DeclareUnicodeCharacter{87BA}{[luo]}    % 螺 (spiral, first char of 螺旋)
\DeclareUnicodeCharacter{65CB}{[xuan]}   % 旋 (spiral, second char of 螺旋)
...
\DeclareUnicodeCharacter{2248}{\ensuremath{\approx}} % approximately equal
\DeclareUnicodeCharacter{2245}{\ensuremath{\cong}} % congruent
\DeclareUnicodeCharacter{2026}{\ldots} % horizontal ellipsis
% (U+2192 declared above as rightwards arrow)
% Subscripts and superscripts
% Subscripts and superscripts (\ensuremath so they're safe in either mode)
...
% Operators and binary signs (\ensuremath so they work in math context too)
\DeclareUnicodeCharacter{2717}{\ensuremath{\times}} % Ballot X
\DeclareUnicodeCharacter{2192}{\ensuremath{\to}} % Rightwards arrow
\DeclareUnicodeCharacter{2190}{\ensuremath{\leftarrow}} % Leftwards arrow
\DeclareUnicodeCharacter{2194}{\ensuremath{\leftrightarrow}} % Left-right arrow
\DeclareUnicodeCharacter{2261}{\ensuremath{\equiv}} % identical to
...
\DeclareUnicodeCharacter{00B5}{\ensuremath{\mu}} % micro / mu
...
\DeclareUnicodeCharacter{2286}{\ensuremath{\subseteq}} % subset or equal
\DeclareUnicodeCharacter{2229}{\ensuremath{\cap}} % intersection
\DeclareUnicodeCharacter{222A}{\ensuremath{\cup}} % union
\DeclareUnicodeCharacter{2205}{\ensuremath{\emptyset}} % empty set
\DeclareUnicodeCharacter{221E}{\ensuremath{\infty}} % infinity
\DeclareUnicodeCharacter{2200}{\ensuremath{\forall}} % for all
\DeclareUnicodeCharacter{2203}{\ensuremath{\exists}} % exists
\DeclareUnicodeCharacter{2202}{\ensuremath{\partial}} % partial derivative
\DeclareUnicodeCharacter{2207}{\ensuremath{\nabla}} % nabla
\DeclareUnicodeCharacter{2211}{\ensuremath{\sum}} % summation
\DeclareUnicodeCharacter{220F}{\ensuremath{\prod}} % product
\DeclareUnicodeCharacter{221A}{\ensuremath{\sqrt{}}} % square root sign
% Greek letters (\ensuremath so they work inline in body text AND in math)
\DeclareUnicodeCharacter{03B1}{\ensuremath{\alpha}} % alpha
\DeclareUnicodeCharacter{03B2}{\ensuremath{\beta}} % beta
\DeclareUnicodeCharacter{03B3}{\ensuremath{\gamma}} % gamma
\DeclareUnicodeCharacter{03B4}{\ensuremath{\delta}} % delta
\DeclareUnicodeCharacter{03B5}{\ensuremath{\varepsilon}} % epsilon
\DeclareUnicodeCharacter{03B6}{\ensuremath{\zeta}} % zeta
\DeclareUnicodeCharacter{03B7}{\ensuremath{\eta}} % eta
\DeclareUnicodeCharacter{03B8}{\ensuremath{\theta}} % theta
\DeclareUnicodeCharacter{03BB}{\ensuremath{\lambda}} % lambda (lowercase)
\DeclareUnicodeCharacter{03C0}{\ensuremath{\pi}} % pi
\DeclareUnicodeCharacter{03C3}{\ensuremath{\sigma}} % sigma
\DeclareUnicodeCharacter{03C4}{\ensuremath{\tau}} % tau
\DeclareUnicodeCharacter{03C6}{\ensuremath{\varphi}} % phi
\DeclareUnicodeCharacter{03C8}{\ensuremath{\psi}} % psi
\DeclareUnicodeCharacter{03C9}{\ensuremath{\omega}} % omega
\DeclareUnicodeCharacter{0394}{\ensuremath{\Delta}} % Delta
\DeclareUnicodeCharacter{03A3}{\ensuremath{\Sigma}} % Sigma
\DeclareUnicodeCharacter{03A8}{\ensuremath{\Psi}} % Psi
\DeclareUnicodeCharacter{03A6}{\ensuremath{\Phi}} % Phi
\fi % end \ifPDFTeX guard around all \DeclareUnicodeCharacter calls
...
% `@` in command names is normally non-letter; the \makeatletter/\makeatother
% block makes it a letter for the duration of these definitions and their
% downstream usage inside the \newtcolorbox bodies below.
\makeatletter
...
% Several of these (e.g. \Tr, \rank, \diag, \im, \Real) are also provided
% by the `physics` package, `amsmath`, or `mathtools` -- using plain
% \DeclareMathOperator would error with "Command \X already defined".
% \pfDeclareOp installs the operator robustly: if already defined it
% renews; otherwise it declares fresh. Both paths typeset upright with
% proper math spacing via \operatorname.
\newcommand{\pfDeclareOp}[2]{%
  \expandafter\providecommand\csname #1\endcsname{}%
  \expandafter\renewcommand\csname #1\endcsname{\operatorname{#2}}%
}
\pfDeclareOp{Log}{Log}      % principal complex logarithm
\pfDeclareOp{Arg}{Arg}      % principal argument
\pfDeclareOp{Li}{Li}        % polylogarithm
\pfDeclareOp{Spec}{Spec}    % spectrum
\pfDeclareOp{Tr}{Tr}        % trace (also from physics package)
\pfDeclareOp{sgn}{sgn}      % sign function
\pfDeclareOp{rank}{rank}    % matrix rank
\pfDeclareOp{diag}{diag}    % diagonal matrix
% Arithmetic-geometry / BSD / Hodge notation used across Ch 24, 25, 21
\pfDeclareOp{ord}{ord}      % order of vanishing
\pfDeclareOp{im}{im}        % image
\pfDeclareOp{Hdg}{Hdg}      % Hodge classes
\pfDeclareOp{Alg}{Alg}      % algebraic classes
\pfDeclareOp{Real}{Re}      % real part (long form)
\pfDeclareOp{cl}{cl}        % cycle class map
\pfDeclareOp{Reg}{Reg}      % regulator
\pfDeclareOp{encode}{enc}   % encoding function (P vs NP chapter)
