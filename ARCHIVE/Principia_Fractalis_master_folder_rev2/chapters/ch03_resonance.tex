...
\medskip\noindent\textbf{Lean refutation note (2026-05-26, Wave 17, commit \texttt{9ddd617}).} The operator-side identity~\eqref{eq:lambda0-operator} -- $\lambda_0(H_\alpha) = \pi/(10\alpha)$ as a literal ground-state eigenvalue -- has now itself been refuted on the framework's own arxiv operator. The Lean~4 theorem \texttt{arxivHalpha\_spectral\_reading\_refuted} in \texttt{PF\_Lean4\_Code/PF/PolylogViaHilbertSchmidtCompactness.lean} (commit \texttt{9ddd617}) proves axiom-free that for every $\alpha \geq \sqrt{2}$ (covering both $\alpha = \sqrt{2}$ and $\alpha = \varphi + 1/4$, specialisations \texttt{spectral\_reading\_refuted\_at\_sqrt2}, \texttt{spectral\_reading\_refuted\_at\_phi\_quarter}; and applying also at $\alpha = \varphi$ used in Ch~\ref{ch:hodge-conjecture}), a two-vector Rayleigh-quotient witness $\langle\psi_{\pi/4}, \texttt{arxivHalpha}\,\alpha\,\psi_{\pi/4}\rangle$ is strictly less than $\pi/(10\alpha)$ (and strictly negative). By the variational principle, $\pi/(10\alpha)$ is therefore not a literal ground-state eigenvalue of \texttt{arxivHalpha}\,$\alpha$. The Lean theorems \texttt{lambda\_0\_P\_precise} and \texttt{lambda\_0\_NP\_precise} certify only the real-arithmetic numerical match between $\pi/(10\alpha)$ and the empirical extrapolations; they do not certify the operator-spectral interpretation in~\eqref{eq:lambda0-operator}. \textbf{Reframe.} $\pi/(10\alpha)$ is the algebraic resonance value at $\alpha$, identified with: (i) the empirical ground-state extrapolations $0.2221441469$, $0.1681764182$ to $10^{-10}$ precision; (ii) the IBM hardware peaks $\alpha_{\mathrm{RH}} = 3/2$ and $\alpha_{\mathrm{NP}} \approx 1.868$; (iii) the B-clean monodromy phase identity $\pi/(10\alpha) = (1/5)\arg^*(1-e^{i\pi/\alpha})$ (Ch~\ref{ch:p-vs-np}, Theorem~\ref{thm:b_clean_phase_identity}); (iv) the H$_3$ Coxeter half-angle structure (Ch~\ref{ch:p-vs-np}, Theorem~\ref{thm:h3_coxeter_origin}). The framework's spectral content is captured by the reformulated \texttt{PolylogResonanceConjecture}: $\pi/(10\alpha)$ is the algebraic-resonance value, not a Rayleigh-Ritz minimum of \texttt{arxivHalpha}.
