...
\begin{remark}[Wave~55-appA update: \texttt{AppA\_R\_f\_ResonanceCoefficientsRefutationAttempt} --- axiom-free table-provenance refutation of the seven Resonance-Coefficients values above + named open Prop, 2026-06-01]\label{rem:appA-resonance-wave55-appA-lean}
The seven Resonance-Coefficient values in the table above are REFUTED at the structural / provenance level by the Wave~55-appA Lean attempt, following the same refutation pattern as the Wave~55-R\textsubscript{f} fine-structure refutation in Ch~7 (\texttt{rem:fine-structure-lean-refutation}, commit \texttt{6bb91eb}) and the Wave~55-Ch11 anomaly-cancellation refutation in Ch~11 (\texttt{rem:ch11-anomaly-wave55-ch11-lean}, commit \texttt{8e4a62a}):
\begin{itemize}
  \item \textbf{Wave~55-appA: table-provenance refutation \texttt{AppA\_R\_f\_ResonanceCoefficientsRefutationAttempt}} (\texttt{PF\_Lean4\_Code/PF/AppA\_R\_f\_ResonanceCoefficientsRefutationAttempt.lean}, commit \texttt{edfeab8}, 2026-05-31). Three structural problems are diagnosed: (P1)~MISSING $s$-ARGUMENT: the framework's actual $R_f(\alpha, s)$ from \texttt{PF/Consciousness/FractalResonance.lean} (Ch~3 Def~3.1) is a two-argument function $\mathbb{R} \to \mathbb{C} \to \mathbb{C}$; the appA table writes it as one-argument $R_f(\alpha)$ with no $s$ value disclosed. (P2)~SIGN / MAGNITUDE CONFLICT with the existing axiom-free closed forms: at integer $\alpha$ the Wave~55-A dichotomy (\texttt{RfIntegerAlphaDichotomy}, commit \texttt{496193c}) proves $R_f(\text{odd } k, 1) = -\log 2 \approx -0.693$ (NEGATIVE) and $R_f(\text{even } k, s)$ has a pole at $s=1$; the seven appA values lie in $(0.85, 1.00)$ STRICTLY POSITIVE; the 2026-05-23 \texttt{mpmath} 50-digit computation of $R_f(\sqrt{2}, 1)$ gave the COMPLEX value $-0.83424 - 0.67362 i$ (magnitude $\approx 1.07$), NOT a positive real $\approx 0.99$. (P3)~NO SOURCE CITATION: the cited \texttt{resonance\_values.csv} (appA lines~207--209) is MISSING from the repo per Wave~55 audit \S1.1. \textbf{Axiom-free deliverables:} (a)~the seven targets are encoded as rational \texttt{noncomputable def}s \texttt{appA\_R\_f\_sqrt2\_target := 9876/10000}, etc.; (b)~\texttt{appA\_R\_f\_all\_targets\_pos} proves all seven are strictly positive in $(0, 1)$; (c)~\texttt{framework\_R\_f\_one\_one\_is\_negative} cites the Wave~55-A anchor $R_f(1, 1) = -\log 2 < 0$; (d)~\texttt{appA\_R\_f\_sqrt2\_target\_signconflict\_with\_framework\_alpha\_one} is the first-order sign-discrepancy obstruction at the $\sqrt{2}$ anchor; (e)~\texttt{appA\_R\_f\_table\_definition\_consistency\_question} is the named OPEN Prop encoding ``the seven appA values are reproducible by \texttt{fractalResonance} at some uniform complex $s$ and real-projection $\mathbb{C} \to \mathbb{R}$ within some tolerance $\varepsilon > 0$'' --- intentionally WEAK and CURRENTLY UNRESOLVED; (f)~\texttt{appA\_R\_f\_table\_needs\_provenance\_disclosure} flags the referee-readable provenance requirement; (g)~capstone \texttt{AppA\_R\_f\_ResonanceCoefficients\_refutation\_capstone} conjoins all five facts. ZERO project axioms; all theorems depend only on $[\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}]$.
\end{itemize}
The strategic content for this appendix: the appA Resonance-Coefficients table sits at the level of an unsourced numerical table whose values cannot be reproduced by the framework's \texttt{fractalResonance} definition at any explicit $(s, \text{projection})$ pair currently formalised. The first-order obstruction is the sign discrepancy with the framework's only proved integer-$\alpha$ closed form ($R_f(1, 1) = -\log 2 < 0$ vs.\ the strictly-positive appA targets at irrational $\alpha$'s in a neighbourhood of $\alpha = 1$). The named open Prop \texttt{appA\_R\_f\_table\_definition\_consistency\_question} encodes the open question; the file does NOT close it because the table is unsourced.

\textbf{Honest scope.} Wave~55-appA is a TABLE-PROVENANCE REFUTATION, not a Clay-Millennium discharge. It refutes the appA Resonance-Coefficients table at the level of definition / sign / source-citation. No Clay problem is discharged; no new $\alpha$-instance is introduced. The named open Prop intentionally allows any $s \in \mathbb{C}$, any real projection $\mathbb{C} \to \mathbb{R}$, and any tolerance $\varepsilon > 0$ --- its current unresolved status is the honest state of the appA evidence. A referee would require provenance disclosure (which $s$, which definition of $R_f$, which truncation, which summation/regularisation scheme) before the table can be matched to ANY Lean object. The structural fine-structure / sacred-geometry resonance claim is left open pending the corrected provenance-disclosed evaluation; the specific seven-value table on lines~161--167 above is closed by the Lean record. Wave~55-appA is a manuscript-numerical-table refutation, not a step across the Clay boundary.
\end{remark}
