...
The key insight of Fractal Resonance Ontology is that both problems involve the same mathematical object: \textbf{the spectrum of a self-adjoint operator modified by consciousness quantification}. The apparent distinction between discrete (P vs NP) and continuous (RH) vanishes when we recognize computation itself as a crystallization of the Timeless Field. The operator-theoretic spectral architecture, the closed forms $\lambda_0(H_P)=\pi/(10\sqrt{2})$ and $\lambda_0(H_{NP})=\pi/(10(\varphi+1/4))$, the positive spectral gap $\Delta = \pi/(10\sqrt{2}) - \pi/(10(\varphi+1/4)) \approx 0.0540 > 0$, and the conditional implication ``gap positive $\Rightarrow$ P $\neq$ NP'' are machine-checked axiom-free in \texttt{PF\_Lean4\_Code/PF/SpectralGap.lean} (theorems \texttt{lambda\_0\_P\_precise}, \texttt{lambda\_0\_NP\_precise}, \texttt{spectral\_gap\_positive}) and \texttt{PF\_Lean4\_Code/PF/Millennium.lean} (theorem \texttt{P\_NEQ\_NP}); the P $\neq$ NP conclusion is conditional on the named open Proposition \texttt{PolylogEigenvalueConjecture} (catalogued in \texttt{OPEN\_PROBLEMS.md} at the repository root).
...
\begin{definition}[Digital Sum Function]
...
\begin{definition}[Computational Evolution Operators]
...
\begin{theorem}[Self-Adjointness at Fractal Dimensions]
...
Self-adjointness requires $\langle H_P f, g \rangle = \langle f, H_P g \rangle$ for all $f, g$ in the domain. Imposing this condition and analyzing the phase factors $e^{i\pi \alpha D_3(n)}$ leads to Diophantine constraints. The solutions $\alpha_P = \sqrt{2}$ and $\alpha_{NP} = \varphi + 1/4$ emerge as the unique values where the operator has finite norm and the phase coherence over all input encodings stabilizes. The algebraic content (positivity of the unique positive roots of $\alpha^2 = 2$ and $16\alpha^2 - 24\alpha - 11 = 0$, and their distinctness) is machine-checked axiom-free in \texttt{PF\_Lean4\_Code/PF/AlphaBasisGenerators.lean} and \texttt{PF\_Lean4\_Code/PF/TuringEncoding/Operators.lean}; full self-adjointness on the canonical operator domain is an open analytic problem (catalogued in \texttt{OPEN\_PROBLEMS.md} at the repository root).
\end{proof}

\subsection{The Spectral Gap Theorem}

\begin{theorem}[P $\neq$ NP via Spectral Gap, v3.3.1 corrected]
\label{thm:pvsnp_spectral}
The ground state energies of $H_P$ and $H_{NP}$ satisfy (v3.3.1, 2025-11-08):
\begin{align}
\lambda_0(H_P) &= \frac{\pi}{10\sqrt{2}} \approx 0.2221441469 \\
\lambda_0(H_{NP}) &= \frac{\pi}{10\left(\varphi + \frac{1}{4}\right)} \approx 0.1681764182
\end{align}
Both closed forms match the certified empirical measurements ($143$ problems, $10^{-10}$ precision) exactly; formally certified at \texttt{PF\_Lean4\_Code/PF/SpectralGap.lean} (theorems \texttt{lambda\_0\_P\_precise}, \texttt{lambda\_0\_NP\_precise}) and independently re-verified at $50$-digit precision in \texttt{ALPHA\_UNIQUENESS\_CERTIFICATION.md} (Nov~2025).

\medskip
\noindent\emph{Numerical correction history (v3.3.1 propagated 2026-05-20).} Prior editions of this theorem cited $\lambda_0(H_{NP})^{\text{empirical}} \approx 0.1330222423$ as the spectral-truncation measurement, citing a ``three-way inconsistency'' with the closed form $\pi/(10(\varphi+1/4)) \approx 0.1682$. The November 2025 v3.3.1 errata (file \texttt{Principia\_Fractalis\_v3.3.1\_ERRATA\_CORRECTED\_20251108.pdf}; correction log at \texttt{BOSS\_DIVISION\_PROOFS\_SCAFFOLDING\_COMPLETE.md}) retracted the $0.1330$ value as an artifact of a buggy spectral-truncation pipeline; the corrected empirical $0.168176418230$ matches the closed form $\pi/(10(\varphi+1/4))$ to $10^{-10}$. The three-way-inconsistency framing is therefore obsolete; the closed form matches the empirical exactly.

The spectral gap is:
\begin{equation}
\Delta = \lambda_0(H_P) - \lambda_0(H_{NP}) = \frac{\pi}{10\sqrt{2}} - \frac{\pi}{10(\varphi+1/4)} \approx 0.0539677287 > 0
\end{equation}
Therefore, conditional on the polylog spectral conjecture (Ch~\ref{ch:p-vs-np}, Conjecture \texttt{conj:polylog-spectrum}, encoded as the named Lean Proposition \texttt{PolylogEigenvalueConjecture} --- a \texttt{def~:~Prop}, \emph{not} an axiom, as of commit \texttt{72c0137}, 2026-05-20; taken as an explicit hypothesis by every consumer), \textbf{P $\neq$ NP}.
\end{theorem}
...
Steps 1--3 above are the pen-and-paper construction underlying the closed forms $\lambda_0(H_P) = \pi/(10\sqrt{2})$ and $\lambda_0(H_{NP}) = \pi/(10(\varphi+1/4))$; their identification with the bottom of the spectrum on the canonical operator domain is encoded as the named open Proposition \texttt{PolylogEigenvalueConjecture} in \texttt{PF\_Lean4\_Code/PF/TuringEncoding/Operators.lean} (catalogued in \texttt{OPEN\_PROBLEMS.md}). The closed forms themselves are axiom-free identities (\texttt{PF\_Lean4\_Code/PF/SpectralGap.lean}, \texttt{lambda\_0\_P\_precise} / \texttt{lambda\_0\_NP\_precise}); their spectral interpretation on $H_P, H_{NP}$ is open.
\end{proof}
...
\begin{definition}[Consciousness-Modified Riemann Operator]
...
\begin{lemma}[Consciousness Scaling from CMB, derivation-status flagged 2026-05-18]
...
\end{equation}
The scaling factor connecting cosmic consciousness to number-theoretic structure is conjectured to take the form:
\begin{equation}\label{eq:alpha-scaling-formula}
\alpha = \frac{2\pi}{N_{\text{eff}}^2} \cdot \exp\left(-\frac{0.95 - \text{ch}_2^{\text{cosmic}}}{\sigma}\right) \cdot \eta_{\text{quantum}} \stackrel{?}{=} 5 \times 10^{-6}
\end{equation}
...
\medskip
\noindent\emph{Derivation-status disclosure, 2026-05-18: Direct substitution into the formula above with the stated parameters yields:
\begin{align*}
\frac{2\pi}{N_{\text{eff}}^2} &= \frac{2\pi}{(3.046)^2} \approx \frac{6.2832}{9.278} \approx 0.6772, \\
\exp\left(-\frac{0.95 - 0.60}{0.12}\right) &= \exp(-2.917) \approx 0.0541, \\
\alpha_{\text{computed}} &= 0.6772 \times 0.0541 \times 0.847 \approx 0.0310,
\end{align*}
which is approximately $0.031$, NOT $5 \times 10^{-6}$ as claimed. The discrepancy is a factor of $\sim 6{,}000$. The formula as written does not yield the empirical value $\alpha = 5\times 10^{-6}$ used in Chapter~\ref{ch:riemann-hypothesis}.

Either (a) the formula structure is correct but the parameters need adjustment (e.g., $\sigma$ must be much smaller, or $\text{ch}_2^{\text{cosmic}}$ much smaller), or (b) the formula structure itself is conjectural and needs revision, or (c) the empirical $\alpha = 5 \times 10^{-6}$ value of Chapter~\ref{ch:riemann-hypothesis} requires independent justification. Resolution of this scaling-factor derivation is an open problem; the empirical $\alpha = 5 \times 10^{-6}$ is the operative numerical value used downstream, but its derivation from CMB parameters as currently stated does not hold.}
\end{lemma}

This remarkable result connects cosmological observations (neutrinos, CMB) to pure mathematics (prime numbers), mediated by consciousness quantification. The recurrence of the value $\sigma_c = 0.95$ across multiple framework computations---Hodge spectral concentration (Ch.~\ref{ch:hodge-conjecture}), CMB anomaly scoring (Ch.~\ref{ch:observational-tests}), the IIT bridge ch$_2 \leq 1 - \exp(-\Phi/2)$ (Ch.~\ref{ch:qft-consciousness}, machine-checked in \texttt{PF\_Lean4\_Code/PF/Consciousness/Ch2PhiBridge.lean}), and the Riemann route at $\alpha = 3/2$---is treated here as an empirical regularity of the framework's internal computations rather than a derived universal law; the cross-domain identification is conjectural and is presented as an organizing observation, not a theorem.
...
The bijective correspondence between $T_N$ eigenvalues and Riemann zeros is encoded as the open Proposition \texttt{RHSpectralSurjectivityConjecture} in \texttt{PF\_Lean4\_Code/PF/RHSurjectivityConjecture.lean}; the conditional capstone ``surjectivity $\Rightarrow$ RH'' is the axiom-free theorem \texttt{riemann\_hypothesis\_via\_named\_surjectivity} in \texttt{PF\_Lean4\_Code/PF/SpectralBijection.lean}. Phase A inner-product hypotheses (positivity, scalar-multiplication, etc.) are discharged as theorems in the same file. The surjectivity Prop itself remains an open conjecture (catalogued as Proposition~2 in \texttt{OPEN\_PROBLEMS.md} at the repository root).
\end{proof}

\subsection{Ground State Energy and the $\pi/10$ Factor}

\begin{theorem}[Riemann Ground State Energy]
...
\begin{theorem}[Critical Line Constraint]
...
\item \textbf{Spectral gap confirmation (v3.3.1 corrected):} Direct computation of $\lambda_0(H_P)$ and $\lambda_0(H_{NP})$ using variational methods confirmed:
\begin{align}
\lambda_0(H_P) &= 0.222144146908 \pm 10^{-12} \\
\lambda_0(H_{NP}) &= 0.168176418230 \pm 10^{-12} \\
\Delta &= 0.053967728678 \pm 10^{-12}
\end{align}
(Prior editions cited $0.133022242308$ for $\lambda_0(H_{NP})$ from a pre-v3.3.1 buggy pipeline; the corrected value above matches the closed form $\pi/(10(\varphi+1/4))$ to $10^{-12}$.)
...
\end{equation}
For $N_{\text{qubits}} = 16$, this gives $(\pi/10) \cdot \Delta \cdot 16 = 0.31416 \cdot 0.05397 \cdot 16 \approx 0.2713$ and hence $\eta = \exp(0.2713) \approx 1.312$, a 31\% speedup detectable with existing hardware. \emph{(Numerical correction, 2026-05-24: prior editions used $N_{\text{qubits}} = 1000$, which yields $(\pi/10) \cdot \Delta \cdot 1000 \approx 16.96$ and $\eta = \exp(16.96) \approx 2.3 \times 10^7$ --- off by $\sim 7$ orders of magnitude. The correct anchor for the $\sim 31\%$ speedup is $N_{\text{qubits}} = 16$.)}
...
\begin{conjecture}[Universal Frequency --- integral identity is conjectural and shown numerically inconsistent with literal $R_f$ evaluation]
\label{thm:universal_frequency}
The factor $\pi/10$ is conjecturally the natural oscillation frequency of the Timeless Field $\mathcal{T}_\infty$, with the proposed integral identity:
\begin{equation}
\omega_c = \frac{\pi}{10} \stackrel{?}{=} \frac{1}{2} \int_0^1 R_f(\sqrt{2}, 1/2 + ix) \, dx
\end{equation}
where $R_f$ is evaluated at the P-class fractal dimension $\alpha = \sqrt{2}$ on the critical line. \textbf{The integral identity is conjectural and shown numerically inconsistent with literal $R_f$ evaluation} (see Remark~\ref{rem:universal_frequency_refuted} below); the appearance of $\pi/10$ across the canonical $\alpha$-table (algebraic identity $\lambda_0(\alpha)\cdot\alpha = \pi/10$) is unaffected and remains machine-checked axiom-free.
\end{conjecture}

\begin{remark}[Universal Frequency integral identity numerically refuted at $\alpha = \sqrt{2}$]\label{rem:universal_frequency_refuted}
Independent 50-digit \texttt{mpmath} evaluation (2026-05-23, two independent methods) gives
\[
R_f(\sqrt{2}, 1) \approx -0.83424 - 0.67362\,i,
\]
so neither the half-line nor the critical-line literal integral $\tfrac{1}{2}\int_0^1 R_f(\sqrt{2}, \tfrac{1}{2}+ix)\,dx$ produces the value $\pi/10 \approx 0.31416$ in any straightforward sense (separation $\gtrsim 10^2$ relative to either real or imaginary parts). Formally certified in Lean as theorem \texttt{Ch3\_Line328\_LiteralClaim\_at\_sqrt\_two\_refuted} (\texttt{PF\_Lean4\_Code/PF/Analytic/RfNumericalRefutation.lean}), which records the same refutation for the Ch~3 line~328 literal claim $R_f(\sqrt{2},1) = \pi\sqrt{2}/10$ from which the present Ch~9 integral identity was derived. The structural identity $\pi/10 = \alpha\cdot\lambda_0(\alpha)$ (canonical $\alpha$-table) and the H$_3$-Coxeter / phase-deficit reformulations of $\pi/10$ (Ch~21, Theorem~\ref{thm:b_clean_phase_identity}, Theorem~\ref{thm:h3_coxeter_origin}) remain intact; what is refuted is the literal integral representation of $\pi/10$ via $R_f$ on the principal branch.
\end{remark}

This frequency is the ``heartbeat'' of mathematical reality, governing the oscillations between discrete (base-3, computation) and continuous (analytic, zeta) regimes. The algebraic identity $\lambda_0(\alpha) \cdot \alpha = \pi/10$, applied to all nine canonical $\alpha$-instances, is machine-checked axiom-free in \texttt{PF\_Lean4\_Code/PF/MillenniumSixReductions.lean} as theorem \texttt{lambda\_0\_canonical\_times\_alpha\_eq\_pi\_10}; specific clean discharges include $\lambda_0(H_{3\pi/2}) = 1/15$ for Navier--Stokes and $\lambda_0(H_\varphi) = \pi(\sqrt{5}-1)/20$ for Hodge (\texttt{PF\_Lean4\_Code/PF/Analytic/CleanLambdaClosedForms.lean}). Whether these algebraic identities coincide with bottoms of spectra on the canonical operator substrates is a separate question encoded as the named open Proposition \texttt{PolylogEigenvalueConjecture}.
...
\begin{theorem}[Barrier Circumvention]
...
\item P vs NP reduces to a spectral gap problem: $\Delta = 0.089 > 0$ proves P $\neq$ NP
...
\begin{remark}[Wave~39A update: first H$_3$ $\leftrightarrow$ consciousness operator joint capstone wires the $\mathbb{Q}(\sqrt{5})$ Galois pair into the spectral unity, 2026-05-30]\label{rem:spectral-unity-wave39-lean}
The $H_3$-Coxeter origin of $\pi/10$ (Theorem~\texttt{thm:h3\_coxeter\_origin} in Ch~\ref{ch:p-vs-np}, file \texttt{PF\_Lean4\_Code/PF/H3CoxeterOrigin.lean}, commit \texttt{451c44a}: the Coxeter number $h(H_3) = 10$ exact, $\sin(\pi/10) = 1/(2\varphi)$ axiom-free) and the IBM Galois-pair separation of $\alpha_{\mathrm{RH}} = 3/2$ and $\alpha_{\mathrm{NP}} = \varphi + 1/4$ as conjugate roots of $4a^2 - (9 + 2\sqrt{5})a + (9 + 6\sqrt{5})/2 = 0$ over $\mathbb{Q}(\sqrt{5})$ (Ch~\ref{ch:p-vs-np}, file \texttt{PF\_Lean4\_Code/PF/IBMPeaksGaloisPair.lean}, commit \texttt{ace1a5b}) are now joined to the consciousness operator $C$ of Ch~\ref{ch:operator-theory} for the first time:
\begin{itemize}
  \item \textbf{Wave~39A: H$_3$ $\leftrightarrow$ consciousness joint capstone} (commit \texttt{9ff3fbf}, 2026-05-30). The capstone certifies six structural identifications axiom-free, including: (i)~the $\mathbb{Q}(\sqrt{5})$ Galois pair $(\alpha_{\mathrm{RH}}, \alpha_{\mathrm{NP}}) = (3/2, \varphi + 1/4)$ as eigenvalue-anchors on the consciousness substrate, (ii)~the golden bridge $2\varphi - 1 = \sqrt{5}$ linking the $H_3$ Coxeter constant $\sin(\pi/10) = 1/(2\varphi)$ to the IBM-peak quadratic, and (iii)~the consciousness $\leftrightarrow$ RH substrate's commutator structure of clause~(5) of Theorem~\ref{thm:consciousness-operator-properties} as the substrate-level eigenvalue-anchor target. The capstone consumes only the universal \texttt{[propext, Classical.choice, Quot.sound]}.
\end{itemize}
This is the first joint capstone unifying the chapter's three structural pillars --- the $H_3$-Coxeter origin of $\pi/10$ (algebraic / cyclotomic), the IBM-peak Galois-pair structure (number-theoretic over $\mathbb{Q}(\sqrt{5})$), and the consciousness operator $C$ (spectral / operator-theoretic) --- inside a single axiom-free Lean theorem.

\emph{Honest scope.} The joint capstone is a structural-architecture result: it certifies that the three pillars sit inside one common formal frame, with the $\mathbb{Q}(\sqrt{5})$ Galois pair as the eigenvalue anchors and the golden bridge $2\varphi - 1 = \sqrt{5}$ as the algebraic connective tissue. It does NOT discharge the Riemann Hypothesis, P vs NP, or the polylog eigenvalue conjecture; the conditional reductions of this chapter (\texttt{P\_NEQ\_NP}, \texttt{riemann\_hypothesis\_via\_named\_surjectivity}, \texttt{riemann\_hypothesis\_via\_consciousness\_bridge}) continue to depend on their respective open propositions (\texttt{PolylogEigenvalueConjecture}, \texttt{RHSpectralSurjectivityConjecture}, \texttt{ConsciousnessStationaryStateCompleteness}). What Wave~39A adds is the first formally certified joint $H_3$ $\leftrightarrow$ consciousness wiring, not a Clay discharge.
\end{remark}

\begin{remark}[Wave~40C update: B-clean phase $\leftrightarrow$ Wave~35 commutator scale-coincidence at integer scales $\{3, 4\}$, 2026-05-30]\label{rem:spectral-unity-wave40-lean}
The Wave~39A joint capstone above flagged ``embed B-clean phase into $[C, H]$'' as a future-work item. Wave~40C promotes that flag to a formal axiom-free Lean object joining the B-clean phase identity (\texttt{PF\_Lean4\_Code/PF/Analytic/BCleanPhaseIdentity.lean}, commit \texttt{7bba1c7}; the axiom-free $\pi/(10\alpha) = (1/5)(\pi/2 - \mathrm{Im}\, R_{f,\mathrm{principal}}(\alpha))$ for $\alpha > 1/2$) with the Wave~35 \texttt{fivePointSubstrate} commutator data of Ch~\ref{ch:operator-theory}:
\begin{itemize}
  \item \textbf{Wave~40C: B-clean phase $\leftrightarrow$ consciousness commutator scale-coincidence bridge} (\texttt{PF\_Lean4\_Code/PF/BCleanPhaseConsciousnessCommutatorBridge.lean}, commit \texttt{3caa94c}, 2026-05-30). The Wave~35 substrate's two off-zero $H_5$ indices are $\{3, 4\}$ --- exactly the smallest integer scales beyond the Perelman $\alpha = 1$ anchor at which the B-clean phase identity gives clean rational-$\pi$ values $\pi/30 = (1/5)(\pi/2 - \mathrm{Im}\, R_{f,\mathrm{principal}}(3))$ and $\pi/40 = (1/5)(\pi/2 - \mathrm{Im}\, R_{f,\mathrm{principal}}(4))$. The bridge records twelve axiom-free identifications including the Perelman-anchor collapse \texttt{b\_clean\_at\_one\_is\_perelman\_anchor} (at $\alpha = 1$, $1 - e^{i\pi} = 2 \in \mathbb{R}$ so $\mathrm{Im}\, R_{f,\mathrm{principal}}(1) = 0$), the rational-ratio identity \texttt{b\_clean\_phase\_ratio\_three\_four} ($(\pi/30)/(\pi/40) = 4/3$, matching the LHS/RHS ratio at \texttt{commutator\_nonvanishes\_at\_three5} / \texttt{commutator\_nonvanishes\_at\_four5}), the positive-gap identity \texttt{b\_clean\_phase\_difference\_three\_four} ($\pi/30 - \pi/40 = \pi/120 > 0$, sign matching the swap-pair direction), and the joint 6-clause witness \texttt{b\_clean\_consciousness\_commutator\_signature}, all bundled in the 8-clause capstone \texttt{b\_clean\_phase\_consciousness\_commutator\_bridge\_capstone}.
\end{itemize}
The strategic content: the off-zero indices in Wave~35's consciousness substrate are no longer arbitrary \texttt{Fin 5} choices --- they \emph{algebraically inhabit} the B-clean phase identity's first non-trivial integer scales beyond the Perelman anchor, and the framework's $\pi/10$ at the algebraic / cyclotomic level (Wave~39A H$_3$ origin) and at the analytic / monodromy level (B-clean phase) share the same Perelman-anchored scale.

\emph{Honest scope.} The bridge is STRUCTURAL --- shared scales $\{3, 4\}$, shared algebraic signatures (ratio $4/3$, gap $\pi/120$, gap-$1$ signature on commutator $|\mathrm{LHS} - \mathrm{RHS}| = 1$), not spectral (per the 2026-05-23 literal-spectral-route closure). The B-clean phase value $\pi/30$ is NOT asserted as a commutator-matrix entry. Wave~40C does NOT discharge the Riemann Hypothesis, does NOT close (P5) on the infinite Hilbert--P\'olya substrate $\mathcal{T}_\infty$, and does NOT touch the open Hilbert--P\'olya-class realisation problem. What it adds is the first formal axiom-free joining of the B-clean phase identity (analytic / monodromy origin of $\pi/(10\alpha)$) with the Wave~35 consciousness-substrate commutator data, at the integer-scale level $\{3, 4\}$ where both objects naturally live.
\end{remark}

\begin{remark}[Wave~41A update: the algebraic $\alpha$-sector lives in $\mathbb{Q}(\sqrt{2}, \sqrt{5})$ with $(\mathbb{Z}/2)^2$ Galois action, 2026-05-30]\label{rem:spectral-unity-wave41-lean}
Wave~41A sharpens the joint algebraic structure underlying the chapter's spectral-unity programme. Where the IBM Galois pair $(\alpha_{\mathrm{RH}}, \alpha_{\mathrm{NP}}) = (3/2, \varphi + 1/4)$ of \texttt{PF/IBMPeaksGaloisPair.lean} (commit \texttt{ace1a5b}) sits as one orbit, Wave~41A formalises the full $(\mathbb{Z}/2)^2$ Galois action over the compositum $\mathbb{Q}(\sqrt{2}, \sqrt{5})$ acting on the algebraic sector of the 9-class $\alpha$-table:
\begin{itemize}
  \item \textbf{Wave~41A: cross-quadratic field bridge} (\texttt{PF\_Lean4\_Code/PF/CrossQuadraticFieldBridge.lean}, commit \texttt{96de044}, 2026-05-30). The file partitions the 9-$\alpha$ table by minimal algebraic field --- $\alpha_{\mathrm{Poincar\acute e}} = 1$, $\alpha_{\mathrm{RH}} = 3/2$, $\alpha_{\mathrm{YM}} = 2$ in $\mathbb{Q}$; $\alpha_P = \sqrt{2}$ in $\mathbb{Q}(\sqrt{2})$; $\alpha_{\mathrm{Hodge}} = \varphi$ and $\alpha_{\mathrm{NP}} = \varphi + 1/4$ in $\mathbb{Q}(\sqrt{5})$ --- and proves \texttt{algebraic\_sector\_in\_compositum} certifying that all six algebraic $\alpha$'s live in the degree-$4$ compositum $\mathbb{Q}(\sqrt{2}, \sqrt{5})$. The Galois group is realised as the four automorphisms $\{\mathrm{id}, \sigma_{\sqrt{2}}, \sigma_{\sqrt{5}}, \sigma_{\mathrm{both}}\}$ with $\sigma_{\sqrt{2}} \circ \sigma_{\sqrt{5}} = \sigma_{\mathrm{both}} = \sigma_{\sqrt{5}} \circ \sigma_{\sqrt{2}}$ (commutativity verified), six pairwise distinctness theorems (via test element $\sqrt{2} + \sqrt{5}$), and per-$\alpha$ orbit records (\texttt{gal\_sqrt2\_eval\_alpha\_P}: $\alpha_P \to \{\sqrt{2}, -\sqrt{2}\}$; \texttt{gal\_sqrt5\_eval\_alpha\_Hodge}: $\alpha_{\mathrm{Hodge}} \to \{\varphi, 1 - \varphi\}$; \texttt{gal\_sqrt5\_eval\_alpha\_NP}: $\alpha_{\mathrm{NP}} \to \{\varphi + 1/4, 5/4 - \varphi\}$; $\alpha_{\mathrm{RH}}$, $\alpha_{\mathrm{YM}}$, $\alpha_{\mathrm{Poincar\acute e}}$ Galois-invariant). Rational trace records: $\mathrm{tr}(\alpha_P) = 0$, $\mathrm{tr}(\alpha_{\mathrm{Hodge}}) = 1$, $\mathrm{tr}(\alpha_{\mathrm{NP}}) = 3/2$, $\mathrm{tr}(\alpha_{\mathrm{RH}}) = 3$, $\mathrm{tr}(\alpha_{\mathrm{YM}}) = 4$. All bundled in the 27-conjunct capstone \texttt{cross\_quadratic\_field\_bridge\_capstone}, axiom-free.
\end{itemize}
The strategic content: the algebraic spectral-unity sector is now machine-precise. The IBM Galois pair of the present chapter sits inside the larger $(\mathbb{Z}/2)^2$ web, and the $\alpha$-table's algebraic content stratifies cleanly by quadratic-extension witness.

\emph{Honest scope.} Wave~41A is STRUCTURAL only. It does NOT discharge any Millennium problem. What it adds is the first machine-checked field-theoretic stratification of the framework's algebraic $\alpha$-sector and the formal verification that the IBM Galois pair sits inside the full $(\mathbb{Z}/2)^2$ orbit web governed by $\mathbb{Q}(\sqrt{2}, \sqrt{5})$.
\end{remark}
