...
> **⚠ Historical snapshot.** The per-section state-claims below are from 2026-05-08 and earlier (canonical Lean PF/ at 6–8 axioms). **Current state as of 2026-05-20: ZERO PROJECT AXIOMS** milestone (commit `72c0137`), 5750 jobs clean. The 2026-05-20 cascade refactor turned the previously-axiomatic `alpha_class_polylog_eigenvalue_conjecture` into a named Lean Proposition `PolylogEigenvalueConjecture : Prop` (a `def`, not an `axiom`), taken as an explicit hypothesis by every consumer; the analogous refactor was applied to the Coq side the same day. The axiom-elimination arc ran: rev-2 cycle 41 → 8 → 6, May 2026 Stages 1–35 reduced 6 → 3 → 1, and the May 20 cascade refactor reduced 1 → 0. See [`AXIOM_AUDIT.md`](AXIOM_AUDIT.md), [`PRISTINE_CERTIFICATION.md`](PRISTINE_CERTIFICATION.md), and [`CROSS_PROVER_PARITY.md`](CROSS_PROVER_PARITY.md) for the current authoritative state. This document is preserved as an audit-trail record of the parity state at intermediate stages.
