...
theorem p_equals_np_implies_zero_gap : P_equals_NP → spectral_gap = 0 := by
  ...
  -- PROOF STRATEGY:
  -- P = NP ⟹ no certificates needed ⟹ E_NP = E_P ⟹ α_NP = α_P ⟹ Δ = 0

  -- Step 1: Under P = NP, every NP problem is also in P
  -- This means certificates become unnecessary (can decide directly)

  -- Step 2: Without certificate structure, E_NP collapses to E_P
  -- The certificate energy term ∑_i i·D₃(c_i) vanishes
  -- This is formalized via the axiom below (to be proven from operator theory)
  have h_same_energy : alpha_NP = alpha_P := by
    -- P = NP eliminates the nondeterministic branching that creates
    -- the certificate structure, forcing resonance frequencies to collapse
    exact p_eq_np_implies_equal_frequencies h_p_eq_np

  -- Step 3: Same resonance frequencies ⟹ same ground states
  -- From λ₀ = π/(10α): if α_NP = α_P then λ₀(NP) = λ₀(P)
  have h_same_lambda : lambda_0_NP = lambda_0_P := by
    unfold lambda_0_P lambda_0_NP
    rw [h_same_energy]

  -- Step 4: Same ground states ⟹ zero spectral gap
  unfold spectral_gap
  linarith
