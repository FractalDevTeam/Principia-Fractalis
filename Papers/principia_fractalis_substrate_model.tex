...
RH &
  ...
  Conditional on (B1) Mayer 1991 \S3 + (B2) Hilbert--P\'olya program as typed Lean Props. \code{PF\_RH\_capstone\_via\_Mayer1991\_T3sym} \\
\midrule
NS &
  \code{Velocity := SchwartzMap (Fin 3 → ℝ) (Fin 3 → ℝ)} (mathlib SchwartzMap). &
  \textbf{Standard Clay velocity type on mathlib SchwartzMap.} &
  AXIOM-FREE on encoding. The framework's $H^{s}_{\sigma}$ inner-product scaffold and Leray projection scaffold are PROVEN substrate-scope (\code{hsSigmaInnerProductScaffoldAtSubstrate}, \code{lerayProjectionScaffoldAtFiniteRank}); they are not $\mathrm{Prop} := \mathrm{True}$. \code{PF\_NS\_capstone\_yields\_Clay\_NavierStokes\_standardV4} \\
\midrule
BSD &
  \code{EllipticCurve := WeierstrassCurve ℚ} (mathlib standard). \code{algebraicRank := algebraicRankV4}, \code{analyticRank := analyticRankV4} are framework rank projections sharing \code{manuscriptRankV4}. &
  \textbf{Standard Clay elliptic-curve type on mathlib WeierstrassCurve}; framework rank projections rather than mathlib \code{MordellWeilRank}. &
  AXIOM-FREE on encoding (rfl-level equality of rank projections). Bridge to mathlib \code{MordellWeilRank} named: \code{UniversalBridge\_MordellWeilRank\_eq\_algebraicRankV4}. \code{PF\_BSD\_capstone\_yields\_Clay\_BSD\_standardV4} \\
\midrule
YM &
...
\midrule
Hodge &
  \code{SmoothProjectiveComplexVariety := GeneralSmoothQuintic} (framework-restricted to general smooth quintics, not all SPC varieties). \code{isAlgebraic := ∃ Z, QuinticAlgebraicCycleRealisesHodgeClass c Z}. &
  \textbf{Substrate-restricted}: general smooth quintics, not arbitrary smooth projective complex varieties. &
  AXIOM-FREE on encoding; Voisin 2007 obstruction on the generic non-CM quintic is the precise open gap. \code{pf\_hodgeEncoding\_FullGeneral\_clay\_substrate\_closure} \\
\midrule
$P \ne NP$ &
...
\paragraph{Three axes use mathlib's standard entry-point types
verbatim} (RH on \code{riemannZeta}; NS on
\code{SchwartzMap (Fin 3 → ℝ) (Fin 3 → ℝ)}; BSD on
\code{WeierstrassCurve ℚ}). The framework discharges the typed
Clay-Standard contracts on these standard types; the
``substrate-scope'' qualifier applies to internal scaffolds
(NS: framework-proven $H^{s}_{\sigma}$ inner-product and Leray
projection scaffolds; BSD: framework rank projections with
named bridge to mathlib \code{MordellWeilRank}; RH: named
published bridges Mayer 1991 + Hilbert--P\'olya as typed
propositions). \emph{This is closer to a Clay discharge in the
standard form than I had been claiming.}

\paragraph{Three axes use substrate-restricted encodings} (YM
on $\ell^{2}(\mathbb{R})$ not a compact simple gauge group;
Hodge on \code{GeneralSmoothQuintic} not all smooth projective
complex varieties; $P$ vs $NP$ on framework canonical
Cook--Karp types not mathlib's eventual Turing machinery). The
substrate-level discharge on these restricted encodings is real
math; the bridge to the full Clay scope is the named open work.
