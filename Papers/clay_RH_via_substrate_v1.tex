...
content is machine-verified in Lean~4 at HEAD \texttt{700bb29} of
...
\texttt{700bb29} of \url{https://github.com/FractalDevTeam/Principia-Fractalis}.
...
The referee-bridge module
\texttt{PF.Referee.RHCapstoneTypedBridgeV2}
strengthens the RH typed bridge used in the proof above by reducing
the V1 capstone's TWELVE-parameter signature
...
Nine of ten V1 hypotheses are concretely discharged axiom-free:
$\alpha := \alpha_{\mathrm{star\_empirical}}$, $K := 1$,
\texttt{hK := one\_pos}, eigenvalues $:= \texttt{evV2}$ (canonical
$1/(n{+}1)$ decay) with \texttt{evV2\_bound}, \texttt{evV2\_ne\_zero},
and \texttt{evV2\_distinct} (strict anti-monotone via
...
\texttt{hpos\_def\_LogWeightedL2}). The V2 module supplies the
V2-level capstone
\texttt{PF\_RH\_capstone\_yields\_Clay\_RH\_standardV2}
and the backward-compatibility bridge
\texttt{PF\_RH\_capstoneV2\_implies\_V1\_via\_pinned}.
The V2 refactor is conservative over V1: every existing
\texttt{PF\_RH\_capstone\_yields\_Clay\_RH\_standard}
consequence remains valid, and no axiom is added. Honest scope: the
\texttt{surjectivity} residual retains the full RH depth; the
encoding's \texttt{hev} field requires the compact-operator spectral
theorem for $T_3^{\mathrm{sym}}$ (mathlib gap unchanged).
