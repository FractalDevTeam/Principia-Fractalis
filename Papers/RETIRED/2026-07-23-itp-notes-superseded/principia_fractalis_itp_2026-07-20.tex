...
%% positive Hermitian tracial state, proved FAITHFUL at all tiers
%% (completion tier included), together with C*-simplicity of T_infty
%% (Glimm 1960) -- fully formalized, unconditional, zero project axioms.
...
%%
%% 2026-07-23 revision (arc r107a-r112): completion-tier faithfulness is
%% now UNCONDITIONAL. r108 TFAE (residual = faithfulness = simplicity);
%% r109 conditional expectation E_k = tail partial trace; r110 E_k
%% 2-norm contraction (elementary); r111 E_k operator-norm contraction
%% (isometry decomposition); r112 completion extension + close ->
%% UHF_trace_faithful and substrate_completion_simple_unconditional,
%% both [propext, Classical.choice, Quot.sound]. Glimm C*-simplicity of
%% the base-3 UHF algebra formalized (novel). Re-verified at HEAD
%% d5ad2881 (2026-07-23): lake build PF = 4471, full = 8928.
%%
...
\noindent Third, the r81--r101 arc (twenty-two commits, 2026-07-07/08) constructs a canonical $\mathbb{C}$-valued map $\tau_{\textsf{UHF}} \colon T_\infty \to \mathbb{C}$ as the unique uniformly-continuous linear extension of the level-$k$ normalised matrix traces $\tau_n(M) := \mathrm{trace}(M)/n$ (for $n = 3^k$) via \texttt{UniformSpace.Completion.extension}, and kernel-verifies additivity, unitality, 1-Lipschitz continuity, the tracial identity $\tau_{\textsf{UHF}}(xy) = \tau_{\textsf{UHF}}(yx)$, the Hermitian identity $\tau_{\textsf{UHF}}(x^*) = \overline{\tau_{\textsf{UHF}}(x)}$, and positivity $0 \le \tau_{\textsf{UHF}}(x^* x)$. Faithfulness $\tau_{\textsf{UHF}}(x^* x) = 0 \Rightarrow x = 0$ holds unconditionally at the level-$k$ and pre-trace tiers via \texttt{Matrix.trace\_conjTranspose\_mul\_self\_eq\_zero\_iff} composed through \texttt{DirectLimit.exists\_eq\_mk}. \textbf{Completion-tier faithfulness $\tau_{\textsf{UHF}}(x^* x) = 0 \Rightarrow x = 0$ holds unconditionally} (development arc r102--r112), and with it the $C^*$-simplicity of $T_\infty$ (Glimm 1960~\cite{glimm1960}): every two-sided ideal is $\bot$ or $\top$. The proof is kernel-only and comprises: a Cauchy--Schwarz inequality $\|\tau_{\textsf{UHF}}(y^*x)\|^2 \le \operatorname{Re}\tau_{\textsf{UHF}}(x^*x)\cdot\operatorname{Re}\tau_{\textsf{UHF}}(y^*y)$; the trace-null set as a two-sided ideal; algebraic simplicity of the pre-completion tower $\varinjlim_k M_{3^k}(\mathbb{C})$ via a Weyl clock--shift unitary-averaging engine $\tfrac{1}{n^2}\sum_{a,b}(C^aS^b)\,x\,(C^aS^b)^\dagger = (\operatorname{tr}x/n)\cdot 1$; a trace-preserving conditional expectation $E_k \colon T_\infty \to M_{3^k}(\mathbb{C})$ (the tail partial trace) shown to be a $\|\cdot\|_2$-contraction (elementary Cauchy--Schwarz, not Kadison--Schwarz) and a $C^*$-operator-norm contraction (isometry/Stinespring decomposition $E_k B = \tfrac{1}{3}\sum_t W_t^\dagger B\, W_t$); its extension to $T_\infty$ with $E_k x \to x$; and the closing implication $\tau_{\textsf{UHF}}(x^*x)=0 \Rightarrow \forall k\, E_k x = 0 \Rightarrow x=0$. All headline theorems, including \texttt{UHF\_trace\_faithful} and \texttt{substrate\_completion\_simple\_unconditional}, report kernel axioms $[\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}]$ and zero project axioms. To our knowledge this is the first machine-checked construction of a UHF $C^*$-algebra together with a canonical \emph{faithful} tracial state and a formalized Glimm-simplicity theorem.

\smallskip

\noindent Machine verification aggregates (verified at HEAD~\texttt{d5ad2881}, 2026-07-23; \texttt{lake build} and \texttt{\#print axioms} run same-day): 4{,}471 \texttt{lake build PF} jobs (canonical Lean~4 layer, mathlib \texttt{v4.24.0-rc1}), 8{,}928 \texttt{lake build} full-library jobs (with \texttt{PF\_Lean4Lean} re-elaboration). Kernel axiom set $[\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}]$ on every headline theorem. The corpus is public at \href{https://github.com/FractalDevTeam/Principia-Fractalis}{\texttt{github.com/FractalDevTeam/Principia-Fractalis}}.
\end{abstract}
...
Formalized operator-algebra content in Lean~4 is still uncommon: mathlib provides a $C^*$-algebra typeclass hierarchy, direct-limit machinery for ring structures, and \texttt{UniformSpace.Completion} for metric completion, but concrete UHF $C^*$-algebra constructions and canonical tracial states on such completions have not been systematically formalized. This paper describes a three-part formalization contribution built on a base-3 ternary matrix tower: (i) a self-adjointness identity for a modified transfer operator on the mathlib Hilbert space $L^2([0,1], dx/x)$; (ii) a UHF $C^*$-algebra construction via inductive limit + metric completion; (iii) a canonical positive Hermitian tracial state on that completion, proved \emph{faithful unconditionally} at all tiers --- including the completion tier --- together with the $C^*$-simplicity of $T_\infty$ (Glimm 1960). The completion-tier faithfulness proof (arc r102--r112) routes through a trace-preserving conditional expectation $E_k \colon T_\infty \to M_{3^k}(\mathbb{C})$ that is both a $\|\cdot\|_2$- and $C^*$-operator-norm contraction (\S\ref{sec:trace}); every headline theorem is kernel-clean with zero project axioms.
...
\item[HEAD reference (verified)] \texttt{d5ad2881} (2026-07-23); build + \texttt{\#print axioms} run same-day
\end{description}
