%% SUPERSEDED_BY: principia_fractalis_2026-07-10_v3.tex
%% ---------------------------------------------------------------------------
%% FROZEN HISTORICAL COPY. Superseded by principia_fractalis_2026-07-10_v3.tex.
%%
...
\paragraph{Typed falsifiers.} The substrate registers eight typed falsifiers with concrete numerical brackets:
\begin{itemize}[itemsep=1pt,leftmargin=2em]
\item \textbf{F1} — $\alpha_{\textsf{RH}}$ to $10^{-15}$ against $T_3^{\textsf{sym}}$'s theoretical/numerical self-adjointness precision at $N \le 40$.
\item \textbf{F2} — $c_2$ consciousness saturation constant bracketed to $[0.94, 0.96]$ against clinical PCI + IIT correspondence.
\item \textbf{F3} — $\Lambda_{\textsf{eff}}/\Lambda_0$ bracketed to the joint DESI BAO + Pantheon+ effective-parameter ratio.
\item \textbf{F4} — $H_0$ bracketed to the SH0ES + JWST parametric refinement window.
\item \textbf{F5} — 144th-problem graph-isomorphism $\alpha$-pin at $2 \times 10^{-3}$ (post-Path-B tolerance).
\item \textbf{F6} — $\Omega_\Lambda$ bracketed to the substrate $78\pi$ prefactor implied ratio.
\item \textbf{F7} — BRST $H^2 = 78 = \dim E_6$ against Cartan--Killing classification of exceptional Lie algebras.
\item \textbf{F8} — micro-macro scale bridge $k = 252$ against substrate ternary-lattice depth.
\end{itemize}
Zero falsifiers have triggered at current measurement precision.
