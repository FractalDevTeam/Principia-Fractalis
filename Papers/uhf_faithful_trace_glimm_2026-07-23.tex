...
Since two-sided ideals absorb each conjugate, $\operatorname{avg}_k(x) \in
I$ whenever $x \in I$. Suppose $I$ contains an $x$ with $c := \tuhf(x) \ne
0$. Theorem~\ref{thm:avgk} at $\varepsilon = \|c\|$ gives
$\|1 - c^{-1}\operatorname{avg}_k(x)\| < 1$, so $c^{-1}\operatorname{avg}_k(x)$
is invertible by the Neumann series. An invertible element of an ideal
forces $1 \in I$: multiply it by its inverse, which lies in the whole
algebra, and the product stays in $I$ because $I$ absorbs multiplication.
No closure of $I$ is used --- $\operatorname{avg}_k(x)$ is a finite average,
a genuine element of $I$ --- so the argument holds for algebraic, not only
closed, two-sided ideals. This proves
...
through Theorem~\ref{thm:tfae}. We write $\tau_{3^k}$ for the normalized
trace on $A_k = M_{3^k}(\mathbb{C})$.

\subsection[The partial trace E\_k (r109)]{The partial trace $E_k$ (r109)}\label{sec:Ek}

Under the identification
$M_{3^{k+1}}(\mathbb{C}) \cong M_{3^k}(\mathbb{C}) \otimes M_3(\mathbb{C})$
furnished by \lt{levelStepEquiv}, the single-step partial trace
$\mathrm{id} \otimes \tau_3 \colon A_{k+1} \to A_k$ is, entrywise,
\[
...
gives $\lt{partialTraceDown} \colon A_j \to A_k$ for $k \le j$.

These maps are compatible with the inductive system: $\mathcal{E}$
inverts a single embedding ($\mathcal{E}(A \otimes I_3) = A$,
\lt{partialTraceStep\_substrateEmbedMatrix}), so for $k \le j \le j'$ the
iterated partial trace $A_{j'} \to A_k$ factors through $A_j$ and agrees
with the level-$j$ map on the image of the embedding. Consequently the
level maps descend, by the universal property of the direct limit, to a
single well-defined map
\[
E_k \colon \varinjlim_m A_m \longrightarrow M_{3^k}(\mathbb{C})
\qquad (\text{Lean: } \lt{condExp}),
\]
with $E_k[\![(j,A)]\!] = \lt{partialTraceDown}\,(A)$ independent of the
chosen single-level representative. This quotient-invariance is the one
nontrivial well-definedness check behind the properties recorded below.
...
inequality
$\tau_{3^k}\bigl((\mathcal{E}B)^*(\mathcal{E}B)\bigr) \le
\tau_{3^{k+1}}(B^*B)$
...
$3^{k+1} \times 3^k$ matrix (Lean: \lt{Winj}). Then
...
($\varphi(xy)=\varphi(yx)$); positivity is deliberately omitted. The Lean
name retains ``State'' for continuity with the source; since positivity is
dropped, the predicate is properly that of a continuous unital tracial
\emph{functional}, and the theorem is correspondingly stronger than
...
the theorems above. The development is classical, not constructive: choice
enters through mathlib's extension of uniformly continuous maps to a
completion (\lt{UniformSpace.Completion.extension}) and the universal
property of the direct limit, both used pervasively here. The \lt{PF\_Lean4Lean} package (separate
...
\midrule
Tower, limit, completion, instances & \S\ref{sec:tower} & \lt{SubstrateBase3Embed.lean}, \lt{SubstrateDirectLimit.lean}, \lt{SubstrateTimelessFieldCompletion.lean} \\
