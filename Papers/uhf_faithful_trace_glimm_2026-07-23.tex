...
%% theorems at HEAD d5ad2881 (2026-07-23), axioms [propext,
...
%% 4471 jobs, lake build (full) = 8928 jobs, mathlib pin eed770a4,
...
\title[A Machine-Checked UHF Algebra with a Faithful Trace]{A Machine-Checked Construction of the UHF Algebra $M_{3^\infty}$ with a Faithful Tracial State:\\ Formalizing Glimm Simplicity in Lean~4}
...
algebraically simple --- every two-sided ideal is $\bot$ or $\top$ --- which
is, to our knowledge, the first formalization of Glimm's 1960 simplicity
theorem for a UHF algebra~\cite{glimm1960} in any proof assistant, and the
first machine-checked construction of an infinite-dimensional simple
$C^*$-algebra with a canonical faithful trace.
...
a unique tracial state which is automatically faithful. Every graduate
course in operator algebras proves these facts~\cite{davidson1996,
blackadar2006, takesaki1979}. None of them, before the present work, had
...
\item \textbf{Glimm simplicity of $\Tinf$}: every two-sided ideal of
  ...
  \S\ref{sec:condexp}). To our knowledge this is the first formalized
  simplicity theorem for an infinite-dimensional $C^*$-algebra in any
  proof assistant.
...
\subsection{The partial trace $E_k$ (r109)}\label{sec:Ek}
...
\]
The kernel-verified properties are exactly those of a trace-preserving
conditional expectation onto the level-$k$ corner:
...
\subsection{The $\|\cdot\|_2$-contraction, elementarily (r110)}\label{sec:l2}
...
positivity enter at any point --- a small but genuine discovery of the
formalization: for trace-compatible partial traces, the scalar
$2$-norm contraction is a three-line Cauchy--Schwarz fact, and the
Kadison--Schwarz detour that the textbook proof takes is avoidable.
...
Uniform continuity extends $E_k$ to the completion in two guises:
\begin{align*}
\lt{condExpFin}\;k &\colon \Tinf \to M_{3^k}(\mathbb{C}),
& &\text{the matrix-valued extension
  \lt{UniformSpace.Completion.extension}\,($E_k$)},\\
\lt{condExpCompletion}\;k &\colon \Tinf \to \Tinf,
& &\text{its recoordinatization }
  x \mapsto \iota_k(\lt{condExpFin}\,k\,x) \text{ in } \Tinf,
\end{align*}
both $1$-Lipschitz (\lt{condExpFin\_lipschitz},
...
\item[HEAD reference (verified)] \texttt{d5ad2881} (2026-07-23); build and \texttt{\#print axioms} run same-day
\end{description}
...
lake build        # 8,928 jobs (adds PF_Lean4Lean re-elaboration)
\end{verbatim}
...
on and would have been impossible without. To our knowledge, however, no
prior formalization in Lean, Coq, or Isabelle contains a concrete
infinite-dimensional simple $C^*$-algebra, a canonical faithful tracial
state on one, or any version of Glimm's simplicity
theorem~\cite{glimm1960}; we have not attempted a systematic survey of
every ITP library, and we would welcome correction. On the classical
