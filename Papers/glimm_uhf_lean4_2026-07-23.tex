...
If a two-sided ideal $I$ contains an $x$ with $\tuhf(x) \ne 0$, this
estimate at $\varepsilon = \|\tuhf(x)\|$, with a Neumann-series
invertibility argument, forces $1 \in I$. So $\Tinf$ is simple, hence
$\tuhf$ faithful, provided every nonzero ideal contains an element of
nonzero trace.
...
property. Simplicity then follows through Theorem~\ref{thm:tfae}. Throughout
$n = 3^k$.

\subsection{The partial trace}\label{sec:Ek}

Under $M_{3^{n+1}}(\mathbb{C}) \cong M_{3^n}(\mathbb{C}) \otimes
M_3(\mathbb{C})$, given by \lt{levelStepEquiv}, the single-step partial
trace $\mathrm{id}\otimes\tau_3$ is, entrywise,
\[
(\mathcal{E}B)_{ij} \;=\; \frac{1}{3}\sum_{t=0}^{2} B_{(i,t),(j,t)}
\qquad(\lt{partialTraceStep}),
\]
where $(i,t)$ abbreviates $\lt{levelStepEquiv}\,(i,t)$. Iterating $j-k$
steps gives \lt{partialTraceDown}$\colon M_{3^j}\to M_{3^k}$ for $k \le j$,
and descending through single-level representatives defines
...
\[
\tau_{3^{n+1}}(B^*B) \;=\; \frac{1}{3^{n+1}}\sum_{i,s,j,t}\bigl|B_{(j,t),(i,s)}\bigr|^2,
\]
every term of which is nonnegative. Mathlib's \lt{Finset.single\_le\_sum}
closes the single-step bound
$\tau_{3^n}\bigl((\mathcal{E}B)^*(\mathcal{E}B)\bigr) \le \tau_{3^{n+1}}(B^*B)$
...
$3^{n+1}\times 3^n$ matrix (\lt{Winj}). Then
...
positivity is deliberately omitted.
