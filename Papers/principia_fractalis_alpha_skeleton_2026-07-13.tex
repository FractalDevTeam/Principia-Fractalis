...
\end{itemize}
These four facets are four views of one substrate object. The operator-algebra closure identifying the nine $\alpha$-values as the extremal tracial states of the projective-limit von Neumann algebra $\pi(\Tinf)''$ under its Dixmier trace functional --- via $\alpha_{i} = \pi/(10 \cdot \lambda_{i}(\pi(\Tinf)''))$ --- is Conjecture~8.X.2 of the corpus and is catalogued as Priority~1a in $\texttt{OPEN\_PROBLEMS.md}$: an attackable open substrate obligation, not claimed as discharged in this paper.
...
\item[(b)] The strong-canonical Riemann-directed Hilbert--P\'olya program conjecture applied to the substrate's specific $\Tsym$ candidate operator of Section~\ref{sec:t3sym} (bounded self-adjoint on $L^{2}([0,1], dx/x)$ per Theorems~\ref{thm:t3sym-self-adjoint}--\ref{thm:t3sym-bounded}). This is an open community program with Riemann-$\zeta$-directed candidate constructions in Berry--Keating 1999~\cite{berry-keating1999}, Connes 1999~\cite{connes1999}, and Bost--Connes 1995~\cite{bost-connes1995}. In the corpus the mathematical content of hypothesis (b) decomposes into three composable sub-Props; the theorem signature exposes them via two explicit hypotheses (Hardy 1914's positive-on-line-nonemptiness supplying inhabitation of $\texttt{PF\_T3SymIsHilbertPolyaOperator\_Positive}$ downstream via the Wave 58 reduction and Wave 59 mathlib-analytic-identity discharge; the HP-program implication supplied directly), with the compactness prerequisite catalogued as a separate corpus Prop referenced in Section~\ref{sec:t3sym}:
\begin{itemize}[itemsep=1pt,leftmargin=1.5em]
\item[(b.i)] $\texttt{T3SymMercerTail}$ (defined at $\texttt{PF/Analytic/T3SymCompactApproxDischarge.lean}$: existence of a sequence of finite-rank self-adjoint compact operators approximating $\Tsym$ in operator norm; the Mayer 1991~\S 3 nuclear-class content restated in the operator-norm-limit formulation consumed by mathlib's $\texttt{isCompactOperator\_of\_tendsto}$), delivering the discrete Weyl-decay eigenvalue sequence on the operator side;
\item[(b.ii)] $\texttt{PF\_T3SymIsHilbertPolyaOperator\_Positive}$ (defined at $\texttt{PF/Analytic/HilbertPolyaIdentificationBulletproof.lean}$: existence of an enumeration $\text{ev} : \mathbb{N} \to \mathbb{R}$ with $\text{ev}(k) > 0$, $\texttt{ZetaZeroOrdinateValid}$ (each $\text{ev}(k)$ is a positive on-line $\zeta$-zero ordinate), and $\texttt{ZetaZeroOrdinateCompletePositive}$ (every positive on-line $\zeta$-zero ordinate is in the range of $\text{ev}$)), asserting that the substrate's eigenvalue enumeration under universal-coupling image is validity-plus-completeness-bijective with the positive on-line $\zeta$-ordinate set. Per Remark~\ref{rem:spectral-structure-and-hp-correspondence}, this bijectivity commitment is a substrate obligation over and above compactness (b.i): the Weyl-decay counting alone yields linear ordinate lower bound whereas $t_{n} \sim 2\pi n/\log n$ is asymptotically slower than linear, so (b.ii) is not implied by (b.i) and the substrate must supply an explicit substrate-selected reindexing carrying $\{10/(\pi\lambda_n)\}$ onto $\{t_n\}$;
\item[(b.iii)] $\texttt{HilbertPolyaProgramConjecture\_Positive}$ (defined at $\texttt{PF/Analytic/HilbertPolyaIdentificationBulletproof.lean}$: the implication $\texttt{PF\_T3SymIsHilbertPolyaOperator\_Positive} \to \texttt{RiemannHypothesis}$), the HP-program conjecture proper: any substrate operator satisfying (b.ii) forces RH via functional-equation symmetry $s \leftrightarrow 1-s$ from the positive on-line side.
\end{itemize}
Composed, (b.i)$\wedge$(b.ii)$\wedge$(b.iii) yields the corpus theorem $\texttt{hilbert\_polya\_implies\_Clay\_RiemannHypothesis\_Standard\_positive}$, discharging the literal Clay-Standard RH predicate on $\texttt{Complex.riemannZeta}$.
\end{enumerate}
...
\item PDG 2024~\cite{pdg2024} charged-lepton masses via the substrate's Riemann-zero mass formula $m_{n}^{2} = M_{\textsf{Planck}}^{2} \cdot \exp[-2\pi/|\zeta'(\rho_{n})|]$: electron $2.2\%$ off, muon $0.6\%$ off, tau $1.3\%$ off. Honest-scope disclosure that the electron miss ($2.2\%$) exceeds the corpus's abstract-level $\lesssim 1.3\%$ per-generation phrasing, together with the $M_{\textsf{Planck}}$-anchoring status of the derivation, is registered as Priority~5a in $\texttt{OPEN\_PROBLEMS.md}$; the $2.2\%$ figure carried here is the honest-scope number.
