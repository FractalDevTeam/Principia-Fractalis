...
Section~\ref{sec:corollaries} presents six corollaries: each of the six remaining Clay Millennium Problems admits a conditional discharge on the framework's canonical PF encodings under named published open conjectures. On mathlib's standard entry-point types, we obtain a literal-form Riemann Hypothesis discharge on $\texttt{Complex.riemannZeta}$ (Corollary~\ref{cor:rh}) under two named substrate-tier citation axioms: Hardy 1914~\cite{hardy1914} and the Riemann-$\zeta$-directed Hilbert--P\'olya program conjecture, applied to the substrate's kernel-only proven $\Tsym$ operator. We do not claim to have proven any Clay Millennium Problem unconditionally on the Clay-Standard predicates; the corollaries below are conditional reductions in the sense of Wiles' 1995 semistable Taniyama--Shimura + Ribet composition~\cite{wiles1995}: substrate-side substantive construction plus external citation of named published theorems and named published open conjectures.
...
\]
on mathlib's $\texttt{Complex.riemannZeta}$, conditional on two named substrate-tier citation axioms:
...
\item $\texttt{framework\_finishes\_all\_six\_clay\_axes\_bulletproof}$ (Corollary~\ref{cor:bundle}): $[\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}, \texttt{framework\_substrate\_pins\_bulletproof\_bundle}]$.
\item $\texttt{clay\_riemann\_hypothesis\_standard\_framework\_standard}$ (Corollary~\ref{cor:rh}): $[\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}, \texttt{Hardy1914\_published\_theorem\_substrate\_citation}, \texttt{Mayer1991\_Cohen2025\_substrate\_HP\_program\_citation}]$.
\end{itemize}

The three named substrate-tier citation axioms disclose their content in axiom-record form. No hidden axioms.
