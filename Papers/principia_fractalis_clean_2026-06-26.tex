%% principia_fractalis_clean_2026-06-26.tex
%%
...
%% Date: 2026-06-26 (revised; initial deposition 2026-06-25)
...
The forced nine-tuple, composed with the substrate's universal coupling, yields the cosmological-constant ratio $\Lambda_{\textsf{eff}}/\Lambda_0 \approx 10^{-120}$ as the explicit closed form $\exp(-14079\pi/160)$, with agreement to $0.05\%$ on the exponent; the exponent $120$ is itself a derived consequence of four substrate-internal quantities, not an input (\S\ref{subsec:lambdaeff}). The arithmetic spine $78\pi \cdot (19/20) \cdot (19/16) = 14079\pi/160$ is kernel-only.
...
\paragraph{Scope disclaimer: layer separation.} The substrate's content in this paper partitions into three layers. \emph{Layer 1} is the unconditional kernel-only algebraic content of \S\ref{sec:invariants} (the twelve substrate-derived identities and the unique nine-tuple), machine-checked in Lean~4 at the kernel level with zero project axioms. \emph{Layer 2} is the conditional substrate content of \S\ref{sec:scope}: two axis-discharges (RH on the literal mathlib carrier; P~vs~NP) conditional on three named open conjectures (C1, C2, C3) and on Wiles-pattern citations of published-and-proven external theorems (Hardy~1914), with the substrate's substantive contribution being the operator constructions the conjectures are applied to. \emph{Layer 3} is the content of this section (\S\ref{sec:corroborations}): numerical correspondences between substrate-side closed-form expressions and independently published physical measurements. \emph{Machine-verification covers Layer 1; it underwrites the substrate's substantive contributions at Layer 2; it does not extend to Layer 3.} The substrate-side numerical values in Table~\ref{tab:appearances} are verifiable at 60-digit precision via \texttt{mpmath}; the published measurements are verifiable against the cited sources; but the structural significance of their numerical agreement is a statistical question, not a theorem. \S\ref{subsec:lookelsewhere} addresses that statistical question directly, against the natural objection that the substrate's empirical case is a multiple-comparisons artifact rather than evidence of structure.
...
\item $|R_f(\sqrt{2\pi}, 1)| = 19/16 = 1.1875$. The Dirichlet-series modulus at the substrate's quantum-gravity anchor $\alpha_9 = \sqrt{2\pi}$, the substrate's rational expression for the modulus, certified numerically at $N = 200{,}000$ Dirichlet truncation to within $\sim 10^{-5}$ of $19/16$.
\end{enumerate}
...
\item The substrate's $\lambda_0^2$ closed-form spectrum across all nine classes (\texttt{lambda\_0\_squared\_spectrum\_capstone}), the substrate's $\lambda_0(\textsf{NP})$ closed form over $\mathbb{Q}(\sqrt{5})\cdot\pi$ (\texttt{lambda\_0\_NP\_closed\_form\_sqrt5}), and the substrate's P/NP spectral-gap closed form $\Delta = \pi(24 + 11\sqrt{2} - 16\sqrt{5})/220$ with positivity (\texttt{spectral\_gap\_closed\_form\_capstone}); see Appendix~\ref{app:lambdasq}.
\end{itemize}
...
\item The substrate's identification: the substrate's $T_3^{\textsf{sym}}$ on $L^2([0,1], dx/x)$ \emph{is} the HP-program operator --- its spectrum equals the imaginary parts of critical-strip $\zeta$-zeros. The substrate's own conjecture; the substrate's substantive contribution is the explicit candidate operator (kernel-only proven self-adjoint), with five eigenvalue/$\zeta$-zero co-localisations consistent with the substrate's universal coupling $s = 10/(\pi\lambda)$. Formalised as a typed Prop hypothesis (not an axiom) in \texttt{PF/Analytic/RH\_FrameworkStandardDischarge\_NamedAnchors\_2026\_06\_19.lean}.

\item The substrate's polylogarithm spectral conjecture: the substrate's polylog operator algebra has spectral structure forcing $\alpha_3^2 = 2$ and $\alpha_4^2 = (\varphi + 1/4)^2$ on the P and NP classes respectively. The substrate's own conjecture; the operator constructions are kernel-only proven, the conjectured spectral identification is what the substrate adds. Formalised as an explicit typed Prop hypothesis (not an axiom) in \texttt{PF/TuringEncoding/Operators.lean}.
\end{enumerate}
...
\begin{itemize}[itemsep=2pt]
\item \emph{Yang--Mills}~\cite{jaffewitten2006}: continuum SU(3) Yang--Mills on $\mathbb{R}^4$ with mass gap is the canonical open frontier. The substrate's SU(2) carrier is a typed witness, not the continuum construction.
...
\paragraph{The $\alpha_{\textsf{NP}}$ power-expansion recurrence.} The substrate's $\alpha_{\textsf{NP}}$ value sits in the rank-2 $\mathbb{Q}$-module $\mathbb{Q}\cdot\alpha_{\textsf{Hodge}}\oplus\mathbb{Q}\cdot 1$; every power $\alpha_{\textsf{NP}}^{k} = a_k\cdot\alpha_{\textsf{Hodge}} + b_k$ with $a_k, b_k \in \mathbb{Q}$ satisfies the kernel-only recurrence
\[
...
\paragraph{The substrate's squared spectrum partitions cleanly.} The closed-form values of $\lambda_0(\alpha)^2$ across the nine substrate classes fall into three structural strata over the substrate's basis $\mathcal{B}$:
\begin{center}
\begin{tabular}{lll}
\toprule
...
\newblock \emph{Annals of Math.} 141 (3) 443--551, 1995.
