...
The three parts land as separate substrate development arcs in the source corpus (\texttt{r22--r25} operator side, \texttt{r30--r60} $C^*$-algebra completion, \texttt{r81--r101} tracial state). They are connected in the sense that the same base-3 ternary architecture underlies all three; they are not connected in the sense that the tracial state does not require the modified transfer operator, nor conversely. The formalization is oriented toward reproducibility: the pinned toolchain, mathlib revision, exact build commands, and \texttt{\#print axioms} output are documented in \S\ref{sec:reproducibility}, and the entire corpus reproduces from a clean clone in approximately thirty minutes.
...
\paragraph{The operator.} Working on the mathlib Hilbert space $L^2([0,1], dx/x)$ (with the multiplicative Haar weight $dx/x$), the Cohen 2025 modified transfer operator $\widetilde{T}_3^{(3/2)}$~\cite{cohen2025transferOp} is a ternary-fractal transfer operator with phase factors $\{1, -i, -1\}$. Its Hermitian symmetrisation is
\[
...
\end{itemize}
Faithfulness $\tau_{\textsf{UHF}}(x^* x) = 0 \Rightarrow x = 0$ holds at level-$k$ via \texttt{Matrix.trace\_conjTranspose\_mul\_self\_eq\_zero\_iff} and at the pre-trace via \texttt{DirectLimit.exists\_eq\_mk} reduction to a single-level representative composed with \texttt{DirectLimit.of.map\_zero}. Completion-tier faithfulness is delivered by the r101 substrate UHF simplicity discharge, using the level-wise \texttt{IsSimpleRing} witness on every $M_{3^k}(\mathbb{C})$ (Theorem~\ref{thm:simple}) plus the r98 positivity closure of the trace-null set at the completion tier under \texttt{Complex.orderClosedTopology}. Kernel axiom set: $[\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}]$.
\end{theorem}
...
For prior formalized-operator-algebra work in Coq and Lean, see the \texttt{coq-mathcomp/analysis} project and the mathlib operator-algebra files. To our knowledge, no prior work has formalized a UHF $C^*$-algebra construction on a base-3 ternary tower with the tracial-state properties above.

The Cohen 2025 modified transfer operator~\cite{cohen2025transferOp} is developed as a substrate-specific ternary-fractal operator; its precursor lineage passes through Mayer's 1991 Selberg-zeta transfer operator for PSL$(2, \mathbb{Z})$~\cite{mayer1991} and related transfer-operator methods (Berry--Keating 1999~\cite{berry-keating1999}, Connes 1999~\cite{connes1999}). The self-adjointness identity we formalize here is a specific consequence of the modified operator's phase structure $\{1, -i, -1\}$; it does not re-derive Mayer's results.
