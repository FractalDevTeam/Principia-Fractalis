...
\paragraph{Conclusion of the test.} \textbf{The look-elsewhere analysis disposes of Table~\ref{tab:appearances} as evidence.} The retrodictions are descriptive context: they show that the substrate's algebraic mechanism produces values in the same numerical neighbourhood as multiple independent published measurements, which is a property of the substrate's algebraic structure and is interesting as exposition, but it is not statistical evidence under any honest null. The empirical content of the substrate's case lives entirely in the chronologically-pre-registered forward prediction of \S\ref{sec:forward}, where the trials denominator is fixed in advance and the multiple-comparisons problem does not apply.
