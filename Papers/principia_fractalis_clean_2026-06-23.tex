...
Per-row audit: the book's broader physical/cosmological exposition lives in Chapters 11 (Geometric-Unity rescue, $H^2 = 78$), 20 (modified-transfer-operator $T_3^{\textsf{sym}}$ construction underlying the Mayer index match and the Li-7 substrate identification), 26 (cosmological-constant rebuttal $\Lambda_{\textsf{eff}}/\Lambda_0 \approx 10^{-120}$), 27 (consciousness-modified Friedmann content underlying $w_0$ and $S_8$ modulation), and 28 (early-universe abundances including primordial Li-7). The per-row closed-form expressions in Table~\ref{tab:appearances} are substrate-algebraic compositions of the substrate's named constants ($\pi/10$, $c_2 = 19/20$, $k = 1/2$, the nine $\alpha$-values, $78$, $1.1875$); these compositions are verifiable directly at 60-digit precision against the right-hand measurements, independent of which book chapter expands the substrate-side reasoning. Per-row substrate-side derivations of each Table~\ref{tab:appearances} entry that do not yet have a dedicated book-chapter exposition (NuFit neutrino ratio, GWTC-4.0 matches, $S_8$ modulation formula, $w_0$ identification, falsifier-panel F1--F8 labelling) live in the substrate's working corpus and the substrate's Lean files referenced in the open-source repository; consolidation of these into dedicated chapters is the next book revision. The substrate-operator rows verify, via Qiskit Aer simulation (notebook \texttt{QUATUM\_TUNED\_IBM.ipynb} in the corpus), that the substrate's $\alpha$-skeleton eigenvalues are realised at four-decimal precision by the substrate's complexity-class operator implementation; this is operator-level computational verification of the substrate's $\alpha$-pin predictions, not corroboration by independent physical measurement. The cosmological GW redshift-evolution index $\kappa = \pi$ matches the GWTC-4.0 measurement $3.2 \pm 1$; the wide measurement uncertainty admits any value in $[2.2, 4.2]$, and the substrate's $\pi$ sits at its centre.
