...
The structural significance of Proposition~\ref{prop:unique} is this: the substrate's algebra derives twelve cross-class identities from a single source (the substrate's universal coupling $\pi/10$ and its modified-transfer-operator scaling), and these twelve admit a unique positive simultaneous solution expressible in the four-element basis $\{1, \pi, \varphi, \sqrt{2}\}$. The seventeen further algebraic identities (\texttt{framework\_alpha\_skeleton\_over\_determined\_capstone}, kernel-only, enumerated in Appendix~\ref{app:extended}) are corollaries of (I1)--(I12) made explicit; they are not additional independent constraints but structural features of the solution visible at the level of explicit polynomial relations. The nine values that emerge are the substrate's empirically-checkable predictions.
...
$|R_f(\sqrt{2\pi}, 1)| \approx 19/16 = 1.1875$. The Dirichlet-series modulus at the substrate's quantum-gravity anchor $\alpha_9 = \sqrt{2\pi}$. The rational $19/16$ is a substrate-natural approximation to the modulus; the underlying numerical value is certified at $N = 200{,}000$ Dirichlet truncation, with truncation error propagating to the exponent residual (below). No proof of the exact equality $|R_f(\sqrt{2\pi}, 1)| = 19/16$ is claimed.
\end{enumerate}
...
\paragraph{Identification with observation.} The cosmological observation gives $\Lambda_{\textsf{eff}}/\Lambda_0 \sim 10^{-120}$, an order-of-magnitude bracket on a hierarchy spanning $120$ decades. The suppression exponent is $120 \cdot \log 10 = 276.310\ldots$; the substrate's closed form with the $19/16$ rational input gives $14079\pi/160 = 276.440\ldots$. The two agree to within the substrate's input precision: the residual $0.13$ on the exponent corresponds to $\Delta|R_f| \approx 5.6 \times 10^{-4}$ relative to $19/16$, well within the gap between the substrate-natural rational $19/16$ and the true Dirichlet modulus value (the rational is exact only to about three significant figures of the modulus). A sharper substrate-natural rational --- or a closed-form proof of $|R_f(\sqrt{2\pi}, 1)|$ as a substrate-internal quantity --- would close the residual. The current match is order-of-magnitude corroboration with substrate-internal quantities, not a precision agreement.

\paragraph{The structural point.} The exponent $120$ in $\Lambda_{\textsf{eff}}/\Lambda_0 \sim 10^{-120}$ is not assigned by the substrate as an input. It is the order-of-magnitude consequence of combining four substrate-internal quantities: $\dim(E_6)$ from Lie theory (substrate-forced by the $T_\infty$ trinification of book Ch.\,11); $\pi$ from Chern--Weil normalisation on the substrate's $\mathbb{R}_+$ scaling fibre; $c_2$ from the substrate's universal saturation threshold; $|R_f(\sqrt{2\pi}, 1)|$ from the substrate's Dirichlet series at the quantum-gravity anchor (substrate-natural rational $19/16$). The product $78\pi \cdot (19/20) \cdot (19/16) = 14079\pi/160$ has no fit to the cosmological observation; the resulting order-of-magnitude $\exp(-14079\pi/160) \approx 10^{-120.06}$ falls inside the observed hierarchy bracket. The match is structural corroboration at order-of-magnitude precision, with the residual exponent gap of $0.13$ attributable to the rational approximation in input (4).
...
\node[buildbox] (build) at (0,0.9) {\textbf{\$ lake build PF}\\[1pt] \emph{4{,}373 kernel-elaboration jobs replay clean}};
\draw[ar] (t1) -- (build);
\draw[ar] (t2) -- (build);
\draw[ar] (t3) -- (build);
...
\draw[ar] (build) -- (ax);
\end{tikzpicture}
\caption{Verification chain for the substrate's three load-bearing $\alpha$-skeleton capstones (the supreme receipt and its two component capstones; conditional-axis theorems carrying named-conjecture hypotheses are separately audited in Appendix~\ref{app:axioms}). Middle: the build invocation; on a clean clone, \texttt{lake build PF} replays 4{,}373 kernel-elaboration jobs without warnings. Bottom: the literal output of \texttt{\#print axioms} for each of the three depicted theorems --- the three mathlib-foundational axioms \texttt{[propext, Classical.choice, Quot.sound]} and no project axioms. The figure describes what a reader running the corpus from scratch sees in their terminal; it is not a stylised summary.}
...
The paper's \S\ref{sec:invariants} states that the substrate's nine-tuple satisfies seventeen further algebraic corollaries of the twelve baseline identities (I1)--(I12), bundled as \texttt{framework\_alpha\_skeleton\_over\_determined\_capstone} in \texttt{PF/Referee/CrossMillenniumInvariants\_Extended\_2026\_06\_19.lean}. This appendix enumerates them. (R1)--(S2) are kernel-only theorems in \texttt{PF.CrossMillenniumMoreInvariants}; each is an algebraic consequence of (I1)--(I12) made manifest in the corpus, not an independent constraint. They are the substrate's structural signature visible at the polynomial-relation level.
...
The substrate's $\alpha$-skeleton is not a free-parameter assignment; it is the (essentially unique) positive simultaneous solution of (I1)--(I12) over $\mathbb{R}^9$. The seventeen further identities (R1)--(S2) are not additional independent constraints but algebraic \emph{corollaries} of (I1)--(I12) made manifest in the Lean corpus: each is a structural feature the substrate's nine-tuple satisfies as a consequence of the rank-9 constraint system, kernel-only proven for citation convenience. \texttt{cross\_millennium\_invariants\_extended\_bundle} in \texttt{PF/Referee/CrossMillenniumInvariants\_Extended\_2026\_06\_19.lean} re-exports the entire (R1)--(S2) set as a single citable theorem.
...
\]
The $2\times 2$ transition matrix $M = \bigl(\begin{smallmatrix}5/4 & 1\\ 1 & 1/4\end{smallmatrix}\bigr)$ realises multiplication by $\alpha_{\textsf{NP}}$ on the rank-2 $\mathbb{Q}$-module $\mathbb{Q}\cdot\alpha_{\textsf{Hodge}}\oplus\mathbb{Q}\cdot 1$, and its characteristic polynomial is therefore the minimal polynomial of $\alpha_{\textsf{NP}}$ over $\mathbb{Q}$: $\lambda^2 - \tfrac{3}{2}\lambda - \tfrac{11}{16} = 0$, equivalently $16\lambda^2 - 24\lambda - 11 = 0$ --- \emph{the substrate's NP polylog quadratic}. The eigenvalues are $\alpha_{\textsf{NP}}$ and its Galois conjugate $(3 - 2\sqrt{5})/4$. The identification of the recurrence's characteristic polynomial with the NP-class minimal polynomial is structural; it is the same algebraic content viewed at the level of the power-expansion matrix. Three new kernel-only theorems in \texttt{PF/AlphaNPHigherPowers\_2026\_06\_25.lean}: \texttt{$\alpha$\_NP\_cubed} giving $\alpha_{\textsf{NP}}^{3} = (47/16)\,\alpha_{\textsf{Hodge}} + 113/64$; \texttt{$\alpha$\_NP\_fourth} giving $\alpha_{\textsf{NP}}^{4} = (87/16)\,\alpha_{\textsf{Hodge}} + 865/256$; \texttt{$\alpha$\_NP\_power\_recurrence} giving the general recurrence step.
