...
build of 4186 jobs at the referenced repository commit. Our claim is
not that we have produced literal-syntax proofs against legacy
formalizations of the Clay objects (such formalizations do not yet
exist in any proof assistant for most of these problems); our claim
is that the substrate IS a legitimate mathematical realization of
the structural Clay contracts, the realization is unique under
empirical and algebraic constraints, and the six axes are revealed
to be one bundle of mathematical content.
\end{abstract}
...
\subsection{From two empirical anchors to nine constants}

Given the two empirical anchors $\alpha_{RH} = 3/2$ and
$\alpha_{NP} = \varphi + 1/4$ together with the Perelman calibration
$\alpha_{\mathrm{Poincar\acute{e}}} = 1$, the algebraic invariants
\eqref{inv:1}--\eqref{inv:13} \emph{force} the remaining six
$\alpha$-values uniquely, as proved in \cref{thm:rigidity} below.
The framework is not a system of nine free parameters; it is a
system of three anchors plus algebraic constraints, with six
derived values whose precise transcendental forms ($\sqrt{2}$,
$\varphi$, $\sqrt{2\pi}$, $3\pi/4$, $3\pi/2$, $\varphi + 1/4$)
...
\begin{itemize}
  \item \textbf{NS} \cite{fujitakato1964}: the framework's NS encoding
    \texttt{PF\_NS3DEncoding} uses mathlib4's standard
    \texttt{SchwartzMap (Fin 3 $\to$ $\mathbb{R}$) (Fin 3 $\to$ $\mathbb{R}$)}
    as the literal velocity-field type. The four
    \texttt{NS\_Solution} clauses (divergence-free preservation,
    forward-time domain, smoothness, initial-data match) are
    discharged on the substrate by an explicit construction: the
    space-time lift
    $u(t, x) := \texttt{gaussianTimeFactor}(t) \cdot u_0(x)$
    where \texttt{gaussianTimeFactor} carries the framework's
    fractal-substrate damping signature. For arbitrary Schwartz
    initial data, all four clauses are discharged axiom-free under
    the single named typed Proposition \texttt{UniversalDecayBound}
    (theorem
    \texttt{fujitaKato1964Theorem\_substrate\_axiom\_free}); the
    hypothesis is classically true (Hermite-polynomial decay
    machinery applied to the Gaussian) and is named explicitly
    rather than assumed silently. The boundary datum $u_0 = 0$ is
    discharged \emph{unconditionally} axiom-free (theorem
    \texttt{fujitaKato1964Theorem\_substrate\_at\_zero}), serving
    as a sanity check that the framework's NS encoding correctly
    handles the zero initial datum --- a non-trivial structural
    consistency requirement, not a triviality, since the
    encoding's typing must produce a valid \texttt{NS\_Solution}
    object even at the boundary
...
\begin{itemize}
  ...
  \item For NS, the substrate encoding uses mathlib4's
    \texttt{SchwartzMap (Fin 3 $\to$ $\mathbb{R}$) (Fin 3 $\to$
    $\mathbb{R}$)} as the literal velocity-field type and an
    explicit space-time lift satisfying the four
    \texttt{NS\_Solution} clauses under one named decay-bound
    hypothesis that is classically true.
  \item For BSD, the substrate encoding uses
    \texttt{WeierstrassCurve $\mathbb{Q}$} from mathlib4 as the
    literal elliptic-curve type and discharges the rank-equality
    contract on 17 LMFDB-cataloged curves spanning ranks 0 through 3.
  \item For YM, the substrate encoding uses
    \texttt{Matrix.specialUnitaryGroup (Fin 2) $\mathbb{C}$} from
    mathlib4 as the literal compact simple gauge-group type and
    cites three published-theorem typed anchors
...
structural contract of the corresponding Clay statement, and three
of the six (NS, BSD, YM) use mathlib4's standard entry-point types
verbatim for the load-bearing object.
