...
build of 8360 jobs at the referenced repository commit. \emph{Four
...
\begin{itemize}
  ...
  \item \textbf{BSD}: substrate discharge on the encoding
    ...
    per-curve by \texttt{rfl} --- a tautological consequence of both
    functions sharing the definition \texttt{manuscriptRankV5}, not a
    proof that analytic and algebraic ranks coincide in the
    number-theoretic sense. The genuine Mordell--Weil rank content
    is supplied by the independent \texttt{r129}--\texttt{r146} arc:
    \texttt{r129} proves the general lemma that any non-torsion
    rational point forces $\mathrm{rk}\,E(\mathbb{Q}) \ge 1$;
    \texttt{r130}--\texttt{r134} certify this for $E_{37\mathrm{a}1}$
    via explicit Bezout identities, duplication formulae, and
    quartic-height bounds; \texttt{r135}--\texttt{r142} scale the
    same certificate pattern to nine rank-$1$ curves; and
    \texttt{r143}--\texttt{r146} install duplication-window substrates
    for $E_{389\mathrm{a}1}$ and $E_{5077\mathrm{a}1}$.
    The open frontier (\texttt{BSD\_OpenFrontier}) is the
    identification of $\mathrm{rk}\,E(\mathbb{Q})$ with
    $\mathrm{ord}_{s=1}L(E,s)$, pending mathlib's
    \texttt{LSeries.ellipticCurve} and Wiles modularity.
    The 20 cataloged curves include
