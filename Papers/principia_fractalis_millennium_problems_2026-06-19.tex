...
The substrate's algebraic locus in $\mathbb{R}^9$ is independently corroborated by the Qiskit~Aer simulation two-instance observation of the canonical pair $(\alpha_{\textsf{RH}}=3/2,\;\alpha_{\textsf{NP}}\approx 1.868)$, plus the cosmological brackets, the particle-physics anomaly retrodictions documented in \S\ref{sec:empirical}, the universal \texttt{fractal\_coherence} = 100 across all 141 data rows of the 142-line benchmark CSV with exact-canonical hits on the Clay-axis-named rows, and the eight typed falsifiers' zero-triggered status (F1, F2, F5, F7 forward-runnable at current measurement precision; F3, F4, F6, F8 consistency-check brackets, see \S\ref{sec:falsifiers}).
