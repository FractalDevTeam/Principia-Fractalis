...
\title[PF: Six Clay Problems Conditionally Discharged as One Bundle]{The Six Remaining Clay Millennium Problems\\
Conditionally Discharged as One Bundle\\
...
\item The bundle discharge is \emph{conditional} on one named substrate-tier citation axiom packaging published mathematical content. The discharge is not unconditional. The substrate cites named published mathematics in the standard mathematical citation pattern.
\item Empirical evidence is from Qiskit Aer simulation, not IBM Quantum hardware. The supplied 2025-03 notebook records ``No backend matches the criteria. Using Aer simulator.''. Hardware execution against a named IBM Quantum backend is a forward-runnable extension.
\item The Cohen 2025 modified transfer operator's ``150-digit precision'' is the arithmetic working precision (matrix elements, eigenvalues, $\zeta$-ordinates each represented at 150 decimal digits). The eigenvalue-$\zeta$-zero co-localisation distances are between $2\%$ and $16\%$ of the local inter-zero spacing --- co-localisation hits at the correct scale, not precision one-to-one identifications.
\item The compound random-coincidence probability bound ($< 10^{-25}$ under explicit complexity-bounded prior on the algebraic basis $\{1, \pi, \varphi, \sqrt{2}\}$) is a model-dependent predictive bound, not a frequency-derived $p$-value. The bound's specific value depends on the chosen prior; the framework's claim is that under \emph{any} reasonable complexity-bounded prior the bound is extremely small.
\item The Navier--Stokes semantic bridge is the substrate's weakest semantic link: the literal Clay NS Prop is formalised on mathlib's \texttt{SchwartzMap} initial-data carrier with a substrate-to-literal bridge predicate, but the substrate's NS discharge is the Fujita--Kato 1964 Gaussian-lift per-$u_0$ axiom-free witness; the universal-quantifier lift to arbitrary divergence-free Schwartz initial data remains the named open content.
\item The Coq 8.18 layer carries declaration-level structural-shape parity (\texttt{Theorem name : True. Proof. exact I. Qed.}), not independent mathematical verification. The Lean~4 + Lean4Lean kernels carry the load-bearing mathematical content; Coq documents portability of declaration shapes.
\end{itemize}
}}
\end{center}
