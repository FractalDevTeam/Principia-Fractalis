...
\item \textbf{Empirical evidence status.} The empirical anchor archived in the corpus's currently-included artifacts (\texttt{Papers/Data/principia\_fractalis\_143\_problems\_IBM\_dataset.csv} + the supplied 2025-03 \texttt{QUATUM\_TUNED\_IBM.ipynb}) is from Qiskit \texttt{AerSimulator} execution. The author additionally has records from a prior IBM Quantum hardware session, but those records are not included in this paper as evidence (the present paper does not surface the backend identifier, job IDs, raw counts, calibration data, or processing code that would make those hardware records citeable); they remain forward-incorporable when reduced into the corpus's anchor lattice with full provenance. The 142-problem panel coherence and the $\alpha_{\textsf{NP}} \approx 1.868$ four-decimal match documented in this paper rest on the included Aer-simulation artifacts.
...
\item[{(F3) $\Lambda_{\textsf{eff}}$ suppression deviation.}] A measurement of $\Lambda_{\textsf{eff}}/\Lambda_0$ differing from $\exp(-78\pi\cdot 0.95\cdot 1.1875)$ by more than $\epsilon$ refutes the substrate's prediction. \emph{Consistent with the long-known cosmological-constant ratio} that joint DESI~BAO + Planck~CMB + Pantheon$+$ effective-parameter constraints are at; the cosmology data measures the long-known vacuum-energy ratio, not a direct comparison between the substrate's bare and effective vacuum densities.
...
\textbf{As of HEAD \texttt{af610b1}: zero of eight falsifiers are triggered; F3 sits at the long-known cosmological-constant ratio that joint DESI BAO + Planck CMB + Pantheon$+$ effective-parameter constraints are consistent with.}
