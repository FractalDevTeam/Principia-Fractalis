...
\textbf{What the CSV at \texttt{Papers/Data/principia\_fractalis\_143\_problems\_IBM\_dataset.csv} actually contains (a referee can verify directly):} 142 lines (1 header + 141 data rows). Every row records \texttt{fractal\_coherence = 100} --- this is the substrate's universal fractal-coherence metric, empirically corroborated across the panel. The \texttt{peak\_alpha} column records the AerSimulator-measured $\alpha$-peak position for each problem and is distributed broadly across $[0.97, 2.92]$; specific exact-canonical hits include the Riemann Hypothesis row at $\alpha = 1.5 = 3/2$ (matching $\alpha_{\textsf{RH}}$) and the P-versus-NP row at $\alpha = 1.868$ (matching $\alpha_{\textsf{NP}} = \varphi + 1/4$ to four decimals); five additional rows register $\alpha = 1.5$. The remaining $\sim 134$ rows record diverse $\alpha$-peak positions; the substrate's claim about these rows is that the framework's classification rule (Chapter~21 polylog spectral derivation) ASSIGNS each to a canonical $\alpha$-class regardless of measured $\alpha$-peak position. \emph{The corpus's \texttt{universal\_fractal\_coherence} Lean theorem certifies the framework's classification schema is self-consistent (every slot in the 143-slot Lean schema has the canonical value the classification assigns); it does not certify that the CSV's \texttt{peak\_alpha} column clusters at canonical values.} The substrate's substantive empirical anchor from this panel is the universal \texttt{fractal\_coherence = 100} together with the Clay-axis exact hits on Riemann Hypothesis and P-versus-NP rows. The benchmark runtime is the Qiskit \texttt{AerSimulator}, as the CSV does not record an IBM hardware backend identifier or job ID and the driver code defaults to \texttt{AerSimulator}; the dataset filename retains ``IBM'' as the run-environment identifier from the original 2025-03 notebook configuration. See also the 142-sample / 143-schema characterization at the end of \S\ref{subsec:full-toe-scope}.
