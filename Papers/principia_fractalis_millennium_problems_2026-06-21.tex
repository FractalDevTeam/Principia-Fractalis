%% principia_fractalis_millennium_problems_2026-06-21.tex
%%
...
\item \textbf{Two-prover load-bearing plus Coq two-tier mirror.} Lean~4 + Lean4Lean carry the load-bearing verification. Coq~8.18 layer splits into Tier~I (load-bearing algebraic spine, $\sim 60$ files with axiom-free Coq-kernel proofs of: 9-class $\alpha$-skeleton, pairwise distinctness, $\lambda_0$ spectral-gap positivity, P-vs-NP forcing from the gap, $\alpha$-uniqueness under Perelman anchor, Cantor IFS contraction, $10$-digit interval bounds on $\sqrt{2}, \sqrt{5}, \varphi$) and Tier~II (declaration-shape mirror, $\sim 670$ files for portability of Hilbert-space / algebraic-geometry / Hodge content whose mathlib dependencies have no current Coq analog).
...
\textbf{GUE-null benchmark (this revision).} Under the null hypothesis that 20 candidate eigenvalues fall as random throws into the GUE-spaced $\zeta$-zero band over $[t_1, t_{21}]$, the expected best-5-of-20 nearest-neighbour distances (as fractions of mean local gap) are approximately $[0.024, 0.071, 0.119, 0.167, 0.214]$. The substrate's observed best-5-of-20 sorted fractions $[0.013, 0.050, 0.092, 0.146, 0.236]$ match these expected order statistics within $\sim 30\%$ per rank. \textbf{The five reported co-localisations refute concentration of the substrate's eigenvalues outside the $\zeta$-zero band but do not, at this 5-pair sample size, rise above the GUE-null random-throw expectation per pair.}

\textbf{Honest substrate posture (revised).} The substrate's load-bearing claim is \emph{not} that the 5 co-localisations alone confirm the HP-program identification --- it is that the substrate's $T_3^{\textsf{sym}}$ operator is (a) PROVEN self-adjoint kernel-only on the literal mathlib Hilbert space $L^2([0,1], dx/x)$ (theorem \texttt{T3\_self\_adjoint\_conj} in $\texttt{PF/TransferOperator.lean}$, axiom set $[\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}]$); (b) cited at substrate-tier via $\texttt{Mayer1991\_Cohen2025\_substrate\_HP\_program\_citation}$ as the substrate's candidate HP-program operator for the published Mayer~1991~/ Berry--Keating~1999~/ Connes~1999 transfer-operator-$\zeta$-zeros equivalence; and (c) the universal-coupling formula $s = 10/(\pi\lambda)$ with structural $t^2$-distance scaling is consistent with the 5 observed pairs. Tighter spectral asymptotics --- a derived bound of the form $|\lambda_n - 10/(\pi t_n)| \leq C(n)$ for an explicit $C$ --- would require extending the substrate's operator-theoretic content beyond kernel-only self-adjointness, and is registered as substrate-internal open content for future work.
...
\begin{tabular}{|p{2.4cm}|p{3.2cm}|p{3.4cm}|p{3.1cm}|p{2.6cm}|}
\hline
...
\hline
P vs NP & PF Cook 1971 / Karp 1972 Turing-machine encoding & The same TM encoding via $\textsf{P\_neq\_NP\_def}$ & Genuine Lean $\texttt{Iff}$ & \emph{Zero gap at the bridge}; bundle discharge conditional on \texttt{PolylogEigenvalueConjecture}, \textbf{whose algebraic content is theorem-tier on framework constants} (chain pieces 2, 11) and whose residual is the opaque-function identification $\texttt{alpha\_of\_class}\,\textsf{ClassP} = \sqrt{2}$ \\
\hline
BSD & PF 17-named-anchor BSD bundle on the framework's elliptic-curve encoding & $\texttt{WeierstrassCurve}\,\mathbb{Q}$ with $\textsf{MordellWeilRank}$ via $\texttt{Module.rank}$ & $\texttt{Iff.rfl}$ definitional equality & \emph{Zero gap at the bridge}; 4-axis unconditional in V3 bundle (\S\ref{sec:bundle-closure}) \\
\hline
NS \emph{(weakest)} & PF Fujita--Kato Gaussian-lift per-$u_0$ witness & $\texttt{SchwartzMap}\,(\textsf{EuclideanSpace}\,\mathbb{R}\,(\textsf{Fin}\,3))\,(\cdots)$ via $\texttt{Clay\_Literal\_NS3D\_ExistenceSmoothness\_Fefferman2000}$ & $\texttt{Substrate\_Forces\_Literal\_Clay\_NS\_Bridge}$ predicate records substrate position & \textbf{Open at literal type level: the universal-quantifier lift from per-$u_0$ witness to arbitrary divergence-free Schwartz initial data} \\
\hline
YM & PF SU(2) gauge group + 3 Wave-56 typed anchors (Glimm--Jaffe / Streater--Wightman / Osterwalder--Schrader) & $\texttt{Matrix.specialUnitaryGroup}\,(\textsf{Fin}\,2)\,\mathbb{C}$ & $\texttt{rfl}$ type-equality & \emph{Zero gap at the bridge}; 4-axis unconditional in V3 bundle \\
\hline
Hodge & PF $\textsf{HodgeGeneralSurfaceSubstrate}$ + 3-conjunct $\textsf{HodgeAlgebraicRepresentation}$ pin + corpus's $\texttt{Hodge\_SixSubstrateClassesBundle.}\linebreak\texttt{hodge\_six\_substrate\_classes\_all\_discharged}$ on six substrate classes (curves dim$=1$ / K3 / abelian surfaces / CY3 / all surfaces dim$=2$ / CY3 at $(2,2)$) axiom-free & Lefschetz $(1,1)$ literal-mathlib carrier on smooth projective complex manifolds + PF general-surface Hodge encoding & Composition of $\textsf{Hodge\_substrate\_capstone\_holds}$ + Wave 56 named anchors + \textbf{published Lefschetz $(1,1)$ theorem (Lefschetz 1924)} discharging all $(1,1)$-class instances literally + Voisin 2007 R1 + R2 substrate witnesses & \textbf{Literal-form discharged on $(1,1)$-classes via published Lefschetz; residual open at literal level: codim $\geq 2$ cycle-class existence on dim $\geq 3$ generic non-CM varieties (Voisin 2007 R3)} \\
\hline
\end{tabular}
\end{center}
...
The Coq~8.18.0~\cite{barras-bertot2009} cross-prover layer (657 files registered in \texttt{\_CoqProject} / 732 \texttt{.v} files on disk at HEAD) splits into \textbf{two tiers}, with the load-bearing-algebraic-spine tier surfaced explicitly in this revision after a Coq-layer audit pass (2026-06-21).

\textbf{Tier I: Load-bearing algebraic spine ($\sim 60$ files, axiom-free Coq stdlib proofs of substrate-tier identities).} A non-trivial subset of the Coq corpus carries axiom-free Coq proofs of the framework's substrate-tier algebraic content, independently re-proved from the Coq kernel rather than imported from Lean:
...
\item[Match 1 --- Muon anomalous magnetic moment $g\!-\!2$ (Fermilab 2023 within $1\sigma$).]
...
\item[Match 2 --- Hubble tension resolution (SH0ES 2024 within $1\sigma$).]
...
\item[Match 9 --- Casali 2013 / Casarotto 2016 normalized brain-complexity index sharp-threshold behavior.]
...
\textbf{The substrate-strengthened posture (this revision).} The CSV measurement at $\sqrt{2}$ is reframable as a \emph{substrate-strengthening signal}: NP-intermediate problems empirically land at the P-end of the substrate's canonical algebraic basis ($\alpha_P = \sqrt{2}$), consistent with Babai's quasi-polynomial reduction of GI's algorithmic distance from P. The substrate's posture is now: (i) Chapter~34A's discrete classification step is updated to reflect the literature consensus (forward-runnable Lean update: \texttt{ProblemClass} enum extended to \texttt{ProblemClass.NPIntermediate} with \texttt{canonicalAlpha NPIntermediate := Real.sqrt 2}, replacing the substrate-rigidity step \texttt{unified\_minimal\_forces\_GI\_prediction\_eq\_a\_NP} with \texttt{\_eq\_a\_P} via the NP-intermediate-collapses-to-P-arm lemma); (ii) the substrate's binary rule is extended to a tri-class $\{P, \text{NP-intermediate}, \text{NP-complete}\}$ with the empirical hypothesis that NP-intermediate problems take $\alpha = \sqrt{2}$; (iii) the existing CSV's $1.41$ becomes the first corroboration of this extended rule, not a falsification of the prior binary rule; (iv) the 144th-problem precision-enhanced re-run remains pre-registered as a consilience check, with the substrate-current prediction now $\alpha_{\textsf{GI}} = \sqrt{2}$ to $10^{-4}$. \textbf{The substrate's reach has expanded, not contracted.}

\emph{Forward-runnable Lean / Coq / book substrate updates}: extend \texttt{ProblemClass} enum, add \texttt{canonicalAlpha NPIntermediate}, add theorem \texttt{framework\_predicted\_alpha\_GI\_eq\_sqrt\_two} superseding \texttt{\_eq\_phi\_plus\_quarter}; rewrite \texttt{PF/Referee/MinimalRigidityForcesGraphIsomorphismPrediction.lean} with the NP-intermediate-collapses-to-P-arm lemma; new file \texttt{PF/Referee/MinimalRigidityForcesNPIntermediateAtPArm.lean} with the literature-anchored citation axiom (Babai 2016 / Sch\"oning 1988 low-hierarchy). Coq mirror replicates the enum extension and the three new theorems. Book Chapter~34A's GI bullet rewritten to state $\alpha_{\textsf{GI}} = \sqrt{2}$ with the NP-intermediate-class derivation citing Babai 2016 / Sch\"oning 1988.
...
\newblock \emph{Sci. Transl. Med.} 5 (198): 198ra105, 2013. PMID 23946194.
