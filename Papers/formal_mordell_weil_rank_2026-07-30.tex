...
%%
%% Machine-checked canonical heights and independence of rational points:
%% rank >= 2 for 389a1, rank >= 3 for 5077a1, and the general criterion.
%%
%% Every mathematical claim below is either a kernel-verified Lean 4 theorem
%% at HEAD d7a01afc (2026-08-02), axioms [propext, Classical.choice,
%% Quot.sound], zero project axioms, zero sorries, no native_decide, or
%% standard cited literature. Verified build data: lake build PF = 4688
%% jobs, lake build (full) = 9361 jobs, mathlib pin eed770a4, toolchain
...
\title[Machine-checked canonical heights and independence of rational
points]{Machine-checked canonical heights and independence of rational
...
Gross--Zagier. The same window that constructs $\hh$ also shows
$E(\mathbb{Q})$ is torsion-free, which discharges the non-torsion side
conditions outright.

The construction is then carried out a second time, for
$E' = \mathtt{5077a1}$, $y^2+y = x^3-7x+6$ --- the smallest-conductor curve of
rank $3$ --- yielding $3 \le \rank E'(\mathbb{Q})$. There the pairing is shown
\emph{bi-additive}, by the Jordan--von Neumann argument applied to the
unconditional parallelogram law, and independence follows from a $3\times3$
Gram determinant. Two by-products are curve-independent and are, we think, the
most reusable part: a nonvanishing Gram determinant of any symmetric
bi-additive form on any abelian group forces $\rank \ge n$; and the entire
canonical-height construction follows from a single doubling window, with no
elliptic curve in the hypotheses. As a further by-product the machinery yields
...
This paper supplies both, for two explicit curves, together with machinery
that is in two places curve-independent. The main theorems are the following.
...
\begin{theorem}[\lt{torsion\_eq\_zero}, \lt{canheight\_pos\_of\_ne\_zero}]
\label{thm:torsfree}
The curve $E = \mathtt{389a1}$ has $E(\mathbb{Q})_{\mathrm{tors}} = 0$:
the only point of finite order is $O$. Equivalently $\hh(R) > 0$ for every
$R \ne O$, so the non-torsion hypotheses in Theorem~\ref{thm:height}(3)
and~(4) hold for every nonzero point.
\end{theorem}

\begin{theorem}[\lt{E5077a1\_rank\_ge\_three}]\label{thm:main3}
Let $E' $ be $y^2+y = x^3-7x+6$ (Cremona \texttt{5077a1}), the
smallest-conductor curve of rank $3$. Then
\[
  3 \;\le\; \operatorname{rank}_{\mathbb{Z}} E'(\mathbb{Q}).
\]
The points $(-3,0)$, $(-2,3)$, $(0,2)$ are $\mathbb{Z}$-linearly independent.
\end{theorem}

The route to Theorem~\ref{thm:main3} differs from Theorem~\ref{thm:main} in one
essential respect, and the difference is the reason the second curve is worth
the space. For two points one can avoid bi-additivity of the height pairing
altogether (Proposition~\ref{prop:reldet}); for three one cannot. What makes
the three-point case work is that $E'(\mathbb{Q})$ is torsion-free, so the
parallelogram law holds with \emph{no} side conditions, and then the
Jordan--von Neumann argument --- a function satisfying the parallelogram
identity on an abelian group is a quadratic form --- gives bi-additivity for
free. With bi-additivity available, independence is ordinary linear algebra,
and in fact does not mention elliptic curves at all:

\begin{theorem}[\lt{rank\_ge\_of\_gram\_det\_ne\_zero}]\label{thm:gram}
Let $G$ be an abelian group and $B \colon G \times G \to \mathbb{R}$ symmetric
and additive in each variable. If $P_1,\dots,P_n \in G$ have
$\det\bigl(B(P_i,P_j)\bigr) \ne 0$, then $n \le \rank_{\mathbb{Z}} G$.
\end{theorem}

Nor, it turns out, does the construction of $\hh$:

\begin{theorem}[\lt{HeightWindow}, \lt{canheight\_window\_shift}]\label{thm:generic}
Let $G$ be an abelian group, $\lambda \colon G \to \mathbb{R}_{\ge 0}$, and
$\kappa \ge 1$ with
$\lvert \lambda(R+R) - 4\lambda(R)\rvert \le \log\kappa$ for all $R$. Then
$\hh(R) := \lim_n \lambda(2^nR)/4^n$ exists, and satisfies $\hh \ge 0$,
$\hh(R+R) = 4\hh(R)$, $\hh(2^nR) = 4^n\hh(R)$, and
$\lvert \hh(R) - \lambda(R)\rvert \le \log\kappa/3$.
\end{theorem}

Theorems~\ref{thm:gram} and~\ref{thm:generic} are what we would offer for
reuse: between them, a new curve needs only its duplication window, its
quasi-parallelogram bounds, its torsion-freeness and its chain numerics, and
the rest is instantiation. Both were extracted after the two curves were done
by hand, and each was checked against both instances rather than merely
asserted (\lt{GenericFitsBothCurves\_r172}).
...
\centering
\pffig{fig_arc_dag}
\caption{Dependency structure of the rank-two argument. Boxes are Lean files,
...
\subsection{The torsion subgroup is trivial}

The bound $\nh(X(2^nR)) \le 12$ inside the proof of
Theorem~\ref{thm:torsion} is not merely finite but small, because this
curve's duplication constant is a perfect cube: $1728 = 12^3$. That makes
the torsion subgroup decidable by inspection.

\begin{proof}[Proof of Theorem~\ref{thm:torsfree}, as formalized]
Let $R$ be torsion, so $\hh(R) = 0$, and suppose $R \ne O$, say
$R = (x,y)$. Since $\hh(2^nR) = 4^n\hh(R) = 0$, the window gives
$\nh(X(2^nR)) \le 12$ for \emph{every} $n$; we use $n = 0, 1, 2$. By
Proposition~\ref{prop:dbl}, $X(2R) = f(x)/g(x)$ and
$X(4R) = f(x_1)/g(x_1)$ where $x_1 = f(x)/g(x)$, both defined because $g$
has no rational zero (\lt{g\_ne\_zero}). So $x$ is a rational satisfying
\[
  \nh(x) \le 12, \qquad \nh(x_1) \le 12, \qquad
  \nh\bigl(f(x_1)/g(x_1)\bigr) \le 12 .
\]
There are exactly $183$ rationals of na\"ive height at most $12$. Of these
only $-1$, $0$ and $1$ meet the second condition, mapping to $10/9$, $3$
and $6$; and none meets the third, the second iterates being
$47503/16641$, $114/121$ and $1431/961$. Contradiction.
\end{proof}

\begin{remark}
The verification uses neither the curve equation nor any squareness test: it
never asks which $x$ carry a rational $y$, only iterates a rational function
and compares heights. The decision procedure therefore stays inside
$\mathbb{Q}$-arithmetic ($\gcd$, $+$, $\times$), with no integer square root
and no existential to eliminate, and is discharged by \lt{decide +kernel}
over the $183$ candidates. Two implementation notes may save a reader time.
First, $\lt{Finset.Icc}\,(-12)\,(12)$ over $\mathbb{Z}$ is built from an
order embedding that the kernel does not unfold, and the reduction gets
stuck; presenting the candidate set over \lt{Finset.range} fixes this.
Second, \lt{decide +kernel} performs the reduction in the kernel and
elaborates to \lt{of\_decide\_eq\_true rfl}; it is \emph{not}
\lt{native\_decide} and introduces no \lt{Lean.ofReduceBool}, as the axiom
report confirms.
\end{remark}

\begin{corollary}[\lt{canheight\_pos\_of\_ne\_zero}]\label{cor:hpos}
$\hh(R) > 0$ for every $R \ne O$. Hence Theorem~\ref{thm:height}(3) holds
for all $P, Q$ with $P, Q, P{\pm}Q \ne O$, and Theorem~\ref{thm:height}(4)
for every $R \ne O$, with no torsion hypothesis to check.
\end{corollary}

This argument is specific to \texttt{389a1}. The enumeration bound is
$\kappa^{1/3}$ for the curve's duplication constant $\kappa$; for
\texttt{5077a1}, where $\kappa = 105754$, it would be $47$, a far larger
search.
...
\texttt{eed770a4}. At repository HEAD \texttt{d7a01afc} (2026-08-02):
\lt{lake build PF} completes in $4688$ jobs and \lt{lake build} in $9361$
...
\lt{RegulatorIndependence389a1\_r152}, \lt{RegulatorPositive389a1\_r153},
\lt{E389a1RankTwo\_r154} and \lt{TorsionTrivial389a1\_r155}.
\end{flushleft}
...
\emph{Rank four and beyond.} With Theorem~\ref{thm:gram} the algebra is
finished for every $n$: a fourth curve needs three curve-side ingredients ---
its duplication window, its quasi-parallelogram bounds and its torsion-freeness
--- and then four points with a nonvanishing $4\times4$ Gram determinant. The
height theory itself is already generic (Theorem~\ref{thm:generic}). What is
\emph{not} yet extracted is the quasi-parallelogram layer, which remains
written out per curve; doing for it what Theorem~\ref{thm:generic} does for the
height is the obvious next piece of engineering.

\emph{Torsion.} Theorem~\ref{thm:torsfree} settles \texttt{389a1}, and the
same route works for any curve whose $\kappa^{1/3}$ is small enough to
enumerate. Making it uniform would need either a general bound on $\kappa$
or the classical injection of torsion into $E(\mathbb{F}_p)$, which rests on
formal-group input this development does not have.
