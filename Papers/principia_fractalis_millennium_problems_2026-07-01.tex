...
\end{equation*}
vs.\ measured $m_H = 125.10 \pm 0.14$~GeV (PDG 2024~\cite{pdg2024}). Substrate-native ingredients: $\dim E_6 = 78$ (BRST $H^2$ dimension, kernel-only Lean-decidable at \texttt{WeinsteinGUResonantRescue.lean}); $\varphi$ (golden ratio; $H_3$ Coxeter scale; $\alpha_{\textsf{NP}} = \varphi + \tfrac{1}{4}$; $\mathbb{Q}(\sqrt{5}) = \mathbb{Q}(\varphi)$); $\ln 3$ (base-3 substrate constant; qutrit maximum entanglement entropy $S_{\textsf{max}}^{(3)} = \log 3$ per Book Ch~7; ternary Wilson RG step; F8 falsifier scale-step unit). \textbf{Mechanism status}: a natural principal-eigenvalue-of-$E_6$-invariant-operator reading of $78 \cdot \varphi$ is mathematically closed off (Coxeter-element eigenvalues on the 78-adjoint lie in $\mathbb{Q}(\zeta_{12}) \cap \mathbb{R} = \mathbb{Q}(\sqrt{3})$, which excludes $\varphi \in \mathbb{Q}(\sqrt{5})$). The substrate-shape-compatible reading is a \emph{three-substrate composition}: 78 from the $E_6$ BRST substrate; $\varphi$ from the $H_3$ icosahedral / $\mathbb{Q}(\sqrt{5})$ substrate (at character-trace level: $\chi_{\textsf{adj}}(5a) = 2 + 1/(2\varphi)$, class-sum $3 + \varphi$ over order-5 conjugacy classes of $W(E_6)$); $\ln 3$ from the base-3 ternary substrate (qutrit entropy consumption or one ternary Wilson RG step). The composition operator has not yet been constructed; the numerical hit is catalogued as pattern data awaiting mechanism.
