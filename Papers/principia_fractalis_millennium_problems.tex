...
\cite{voisin2007hodge}. Each axis carries named research-engineering
bridges from the framework's typed encoding to the full mathlib
precision tier (Mayer compactness hookup for RH; continuum SU($N$)
Wightman lift for YM; Voisin obstruction for Hodge; the universal
Mordell--Weil bridge for BSD; the literal $\mathrm{ClassP} \ne
\mathrm{ClassNP}$ for $P$ vs $NP$), each explicitly named in
...
theorem (in
\code{PF/TuringEncoding/AlphaRealizationNoGo.lean}), the named residual
is precisely Clay-equivalent at the named encoding.
