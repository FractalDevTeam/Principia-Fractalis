...
(\code{hpos\_def\_LogWeightedL2}). Two named residuals consumed by
the discharge:
\begin{itemize}
\item \textbf{(F)} The compactness / spectral-theorem hookup of
  $T_{3}^{\mathrm{sym}}$ on
  \code{LogWeightedL2} \cite{mayer1991thermodynamic}: formalization
  lift of a published mathematical result to mathlib precision.
\item \textbf{(M)} The surjectivity of the eigenvalue-to-zero map
  (\code{RH\_OpenFrontier}): \emph{this is an open conjecture in
  number theory}; discharging it would be a new positive result on
  the Hilbert-P\'olya program \cite{berry1999riemann, connes1996formule}.
\end{itemize}
...
\item \textbf{(M)} The Osterwalder--Schrader / Wightman reconstruction
  of continuum SU($N$) Yang--Mills theory
  \cite{wightman1956, osterwalder1973}: \emph{open mathematics, deep
  open problem in constructive quantum field theory}; this is the
  Clay-stated mass-gap problem at its literal precision tier.
\end{itemize}
...
The named residual:
\begin{itemize}
\item \textbf{(M)} The lift to the literal $H^{2,2}(X_{5}, \mathbb{Q})
  \times$ Chow cycle-class map: the Voisin 2007 \cite{voisin2007hodge}
  obstruction is isolated by the framework but \emph{not discharged}.
  The literal Hodge conjecture at full precision is open mathematics.
\end{itemize}
...
encoding is machine-verified for each axis. At the literal Clay-statement
mathlib precision tier, four of the six unsolved axes have residual
content that is genuinely open mathematics (Type~\textbf{(M)} gaps:
RH surjectivity, $P$ vs $NP$, YM continuum reconstruction, Hodge
literal H${}^{2,2}$ lift), and two have residual content that is
formalization (Type~\textbf{(F)} gaps: NS Sobolev/Leray
infrastructure, BSD universal Mordell-Weil bridge). The
Type~\textbf{(M)} gaps are the conventional open mathematical content
of the corresponding Clay problems. Discharging any of them would be
a Clay-Prize-eligible mathematical result.
...
where Type~\textbf{(M)} gaps live and these are not closed.
\end{honest>
