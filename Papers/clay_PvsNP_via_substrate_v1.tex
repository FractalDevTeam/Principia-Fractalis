...
proof is machine-verified in Lean~4 at HEAD \texttt{700bb29} of
...
The proof is machine-verified in Lean~4 at HEAD \texttt{700bb29}
...
$\{\texttt{propext},\,\texttt{Classical.choice},\,\texttt{Quot.sound}\}$.
The full build at HEAD \texttt{700bb29} reproduces the result.
...
\section*{Note on the V3 refactor}

The carrier module
\texttt{PF.TuringEncoding.PNPClassSeparationCarrierV3}
(landed in V2.1.1, HEAD \texttt{700bb29}) strengthens the V2
Bool-valued carrier by replacing the V2
...
the honest-scope 3-clause bundle is
\texttt{pnp\_carrier\_V3\_honest\_scope\_capstone}.  Honest scope:
the TMConfig wiring closes the \emph{structural-shape defect}
(every machine now carries a TM-config-valued run rather than an
opaque \texttt{True}) but does NOT close the
...
reaches an accepting TM state'') --- which is the V4 refactor
target.  The V3 refactor is conservative over V2 and V1: every
existing consequence remains valid, and no axiom is added.
