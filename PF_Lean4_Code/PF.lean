...
import PF.Operators.VAlphaPMapDischarge  -- 2026-05-25 Wave 11 — discharges the two remaining facts in `KatoRellichInputPMap α`: (1) matrix-coefficient identity `⟪Halpha_PMap α ⟨e_n, _⟩, e_m⟫ = ((h_alpha_basis α n m : ℝ) : ℂ)` via inner_add_left/inner_smul_left/lp.inner_single_left + four-way case-split on m vs (n, n+1, n-1); (2) `IsFormalAdjoint Halpha_PMap Halpha_PMap` via `Submodule.span_induction₂` + bilinearity of inner + `h_alpha_basis_symm`. Yields `KatoRellichInputPMap_proven α` (axiom-free, for every real α). Unconditional capstones `H_alpha_ground_state_eq_pi_10_alpha_PMap_unconditional` and `H_alpha_spectral_gap_positive_PMap_unconditional`. ZERO project axioms, ZERO sorries.
