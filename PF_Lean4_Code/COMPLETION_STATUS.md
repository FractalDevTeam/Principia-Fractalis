...
**Canonical PF/ axioms:** **1** (`alpha_class_polylog_eigenvalue_conjecture`), down from 41 at rev-2 cycle start
**Total Code Sorries:** 0
**Total Build Warnings:** 0
**Build Status:** ✅ `lake build` — 5626 jobs clean

The axiom-elimination arc has run in three phases:
...
- **Conditional theorem:** `P_neq_NP_via_spectral_gap : ClassP ≠ ClassNP`, depending on the single project axiom `alpha_class_polylog_eigenvalue_conjecture` (formal encoding of Ch 21's polylog Conjecture + branch-selection Heuristic + golden-modulation Conjecture).
...
Build completed successfully (5626 jobs).
# Errors: 0
# Warnings: 0  (full warning sweep completed 2026-05-15 commit ae46797:
#               45 unused-var warnings, 26 unused-simp-arg, 3 deprecated
#               imports, 2 deprecated lemmas, 1 push_cast, 1 intro-merge.)
```
...
The Lean 4 canonical library `PF/` carries **1 explicitly-disclosed axiom** (`alpha_class_polylog_eigenvalue_conjecture`), 0 sorries, 0 warnings, with `lake build` completing all 5626 jobs cleanly. The axiom is the formal encoding of the manuscript's Ch 21 Conjecture + Heuristic content for the P-vs-NP spectral derivation; **retiring it is original mathematical research, not formalization labor.**

The Lean theorems are CONDITIONAL REDUCTIONS, not unconditional proofs of the Millennium Problems they target:
* `P_neq_NP_via_spectral_gap` — conditional on the one project axiom above.
