...
For each segment (idx, L, U, n, MHI), emit two Lean files:
  * PF/Analytic/RiemannXiBox{BOX_K}Panels/Seg{idx:02d}.lean
  * PF/Numerics/Box{BOX_K}Seg{idx:02d}M.lean
...
def _qs(n, d):
    """Rational -> Lean numeral, in a form the `approx` tactic can discharge.

    `approx` closes `q in approx (Interval.ofRat q)` from the @[approx] set in
    vendor/interval/Interval/Interval/Conversion.lean, which covers
    `OfNat a / OfNat b` with a,b >= 2 (`ofNat_div_mem_approx_ofRat`) and
    `1 / OfNat b` (`one_div_ofNat_mem_approx_ofRat`). There is NO lemma for a
    bare integer literal, so an integer-valued coefficient leaves the goal
    `67 in approx (Interval.ofRat 67)` unsolved. `67/1` fails identically,
    because 1 is not `AtLeastTwo`.

    This is what blocked box 103 (p1 = 7/8 gives the pi-coefficient 67 at n = 1)
    and box 116 (p1 = 1 gives an integer at every n), and why the other 16 boxes
    build: their p1 keeps every coefficient non-integral.

    Fix: render an integer n >= 2 as `2n/2`. This is EXACT -- no nudge, no loss
    of margin -- and matches `ofNat_div_mem_approx_ofRat` with a = 2n, b = 2.
    0 and 1 are left alone: they are not `AtLeastTwo`, and they already occur in
    the 16 boxes that build, so other @[approx] lemmas cover them."""
    if d == 1:
        return "%d/2" % (2 * n) if n >= 1 else str(n)
    return "%d/%d" % (n, d)
...
def compute_node(u_frac):
    ...
    p23 = mp.power(u_mp, mp.mpf(_a1(BOX_SHI).numerator) / _a1(BOX_SHI).denominator)
    p25 = mp.power(u_mp, mp.mpf(_a2(BOX_SHI).numerator) / _a2(BOX_SHI).denominator)
    log_u = mp.log(u_mp)
...
BOX_K = None          # box index, set from argv
BOX_SLO = None        # Fraction, sigma_lo
BOX_SHI = None        # Fraction, sigma_hi


P1 = P2 = None                         # M-certificate p1/p2 literals
MCONST = MCOEF = None                  # folded M constants
MCOEF1 = MCOEF2 = MCOEF3 = None        # (n+1)^2-scaled pi coefficients
ALO1 = ALO2 = AHI1 = AHI2 = None       # real-side exponent literals
ALO1_I = ALO2_I = AHI1_I = AHI2_I = None  # interval-side |exponent| literals
SLO_S = SHI_S = None                   # sigma endpoints as Lean literals


def _lit(e):
    """Real-side exponent literal, e.g. -(21/32 : R)."""
    if e < 0:
        return "-(%s : \u211d)" % _qs((-e).numerator, (-e).denominator)
    return "(%s : \u211d)" % _qs(e.numerator, e.denominator)


def _ilit(e):
    """Interval-side |exponent| as ofRat, e must be negative."""
    m = -e
    return "_root_.Interval.ofRat (%s)" % _qs(m.numerator, m.denominator)


def _set_box(k, slo, shi):
    global BOX_K, BOX_SLO, BOX_SHI
    global ALO1, ALO2, AHI1, AHI2, ALO1_I, ALO2_I, AHI1_I, AHI2_I, SLO_S, SHI_S
    BOX_K, BOX_SLO, BOX_SHI = k, F(slo), F(shi)
    a1l, a2l = _a1(BOX_SLO), _a2(BOX_SLO)
    a1h, a2h = _a1(BOX_SHI), _a2(BOX_SHI)
    ALO1, ALO2, AHI1, AHI2 = _lit(a1l), _lit(a2l), _lit(a1h), _lit(a2h)
    ALO1_I, ALO2_I = _ilit(a1l), _ilit(a2l)
    AHI1_I, AHI2_I = _ilit(a1h), _ilit(a2h)
    SLO_S = "%d/%d" % (BOX_SLO.numerator, BOX_SLO.denominator)
    SHI_S = "%d/%d" % (BOX_SHI.numerator, BOX_SHI.denominator)
    global P1, P2, MCONST, MCOEF, MCOEF1, MCOEF2, MCOEF3
    _p1 = (1 + BOX_SHI) / 2
    _p2 = _p1 * (_p1 + 1)
    P1 = "%d/%d" % (_p1.numerator, _p1.denominator)
    P2 = "%d/%d" % (_p2.numerator, _p2.denominator)
    _mc = _p2 + F(15, 2) + F(225, 4) + 15 * _p1
    _co = 2 * _p1 + 15
    MCONST = _qs(_mc.numerator, _mc.denominator)
    MCOEF = _co
    _c1, _c2, _c3 = _co * 1, _co * 4, _co * 9
    MCOEF1 = _qs(_c1.numerator, _c1.denominator)
    MCOEF2 = _qs(_c2.numerator, _c2.denominator)
    MCOEF3 = _qs(_c3.numerator, _c3.denominator)
...
def _pow_real(sig_exp):
    """Lean real-side exponent literal for u^(exp)."""
    e = sig_exp
    if e < 0:
        return "-(%s : \u211d)" % _qs((-e).numerator, (-e).denominator)
    return "(%s : \u211d)" % _qs(e.numerator, e.denominator)
...
def _pow_ivl(sig_exp):
    """Interval-side |exponent| literal; caller supplies the sign."""
    e = -sig_exp
    return "_root_.Interval.ofRat (%s)" % _qs(e.numerator, e.denominator)


DIGITS_RE_LO = 10   # r331b rework: Re lower-bound node rounding (was 5, vacuous)


def compute_node_tight_im(u_frac):
    """Tight sigma-uniform Im LOWER product at a node: D_LO * sin * omega.

    D_LO = u^a1(sigma_lo) - u^a2(sigma_lo) is box_pow_diff_lb.  Only used at
    sin >= 0 nodes, where D_LO * S * W is the adverse-LOW product.
    """
    u_mp = mp.mpf(u_frac.numerator) / u_frac.denominator
    e1, e2 = _a1(BOX_SLO), _a2(BOX_SLO)
    dlo = (mp.power(u_mp, mp.mpf(e1.numerator) / e1.denominator)
           - mp.power(u_mp, mp.mpf(e2.numerator) / e2.denominator))
    log_u = mp.log(u_mp)
    si = mp.sin(mp.mpf(15) / 2 * log_u)
    om = mp.exp(-PI * u_mp) + mp.exp(-4 * PI * u_mp) + mp.exp(-9 * PI * u_mp)
    return dlo * si * om
...
def compute_node_tight_lo(u_frac):
    ...
    u_mp = mp.mpf(u_frac.numerator) / u_frac.denominator
    e1, e2 = _a1(BOX_SLO), _a2(BOX_SLO)
    alo = (mp.power(u_mp, mp.mpf(e1.numerator) / e1.denominator)
           + mp.power(u_mp, mp.mpf(e2.numerator) / e2.denominator))
    log_u = mp.log(u_mp)
    c = mp.cos(mp.mpf(15) / 2 * log_u)
    om = mp.exp(-PI * u_mp) + mp.exp(-4 * PI * u_mp) + mp.exp(-9 * PI * u_mp)
...
def canonical_node_bounds(u_frac, cos_sign, sin_sign):
    ...
    if sin_sign == 'nn':
        assert s_mp >= 0
        # box-parametric: D_LO * S * W, floored.  Was F(0) -> valid but lossy
        # for any box with sigma_lo > 1/2 (cost box 2 its entire margin).
        im_lo = outward_lo(compute_node_tight_im(u_frac), DIGITS_RE_LO)
...
def rat_R(f):
    return f"({_qs(f.numerator, f.denominator)} : ℝ)"


def rat_I(f):
    return f"({_qs(f.numerator, f.denominator)} : _root_.Interval)"
...
def _im_tight_lemma(idx, u_str, u_rat, im_lo):
    """Emit u{idx}_im_tight_ge : IM_LO <= D_LO * sin * omega   (sin >= 0 nodes)."""
    tight_expr = f"""(({u_str} : \u211d) ^ ({ALO1}) - ({u_str} : \u211d) ^ ({ALO2}))
        * Real.sin ((15/2 : \u211d) * Real.log ({u_str} : \u211d))
        * omegaPartial 3 ({u_str} : \u211d)"""
    tight_interval = f"""((_root_.Interval.exp (-(_root_.Interval.log
        (_root_.Interval.ofRat ({u_rat})) * {ALO1_I}))
      - _root_.Interval.exp (-(_root_.Interval.log
        (_root_.Interval.ofRat ({u_rat})) * {ALO2_I})))
     * _root_.Interval.sin ((15/2 : _root_.Interval) * _root_.Interval.log
        (_root_.Interval.ofRat ({u_rat})))
     * (_root_.Interval.exp (-(_root_.Interval.pi *
          _root_.Interval.ofRat ({u_rat})))
        ...
        + _root_.Interval.exp (-(_root_.Interval.pi * (9 : _root_.Interval) *
          _root_.Interval.ofRat ({u_rat})))))"""
    return f"""
private theorem u{idx}_im_tight_ge :
    {rat_R(im_lo)}
      \u2264 {tight_expr} := by
  have hu0 : (0 : \u211d) < ({u_str} : \u211d) := by norm_num
  rw [rpow_neg_eq_exp hu0, rpow_neg_eq_exp hu0, omegaPartial_3_closed]
...
def _im_adverse_lemma(idx, u_str, u_rat, sin_sign, im_lo=None, im_hi=None):
    ...
    diff_expr = f"""(({u_str} : ℝ) ^ ({AHI1}) - ({u_str} : ℝ) ^ ({AHI2}))
      * Real.sin ((15/2 : ℝ) * Real.log ({u_str} : ℝ))
      * omegaPartial 3 ({u_str} : ℝ)"""
...
def emit_node(idx, u, cos_sign, sin_sign):
    ...
    if sin_sign == 'nn':
        im_adv_lem = im_adv_lem + _im_tight_lemma(idx, u_str, u_rat, im_lo)
    ...
    if cos_sign == 'np':
        ...
        re_lo_proof = (
            f"have hAlo : ({u_str} : ℝ)^({ALO1}) + ({u_str} : ℝ)^({ALO2})\n"
            f"        ≤ ({u_str} : ℝ)^(σ/2 - 1) + ({u_str} : ℝ)^((1-σ)/2 - 1) :=\n"
            f"      by\n"
            f"        have h := PrincipiaTractalis.RiemannXiBoxParametric.box_pow_sum_lb\n"
            f"          (σlo := ({SLO_S} : ℝ)) hu1 (by norm_num) hσ0\n"
            f"        have e1 : (({SLO_S} : ℝ))/2 - 1 = {ALO1} := by norm_num\n"
            f"        have e2 : (1 - ({SLO_S} : ℝ))/2 - 1 = {ALO2} := by norm_num\n"
            f"        rw [e1, e2] at h; exact h\n"
            f"    have hCW : (0 : ℝ) ≤ Real.cos ((15/2 : ℝ) * Real.log ({u_str} : ℝ))\n"
            f"        * omegaPartial 3 ({u_str} : ℝ) := mul_nonneg hC hW_nn\n"
            f"    have hchain : (({u_str} : ℝ)^({ALO1}) + ({u_str} : ℝ)^({ALO2}))\n"
            f"          * Real.cos ((15/2 : ℝ) * Real.log ({u_str} : ℝ))\n"
            f"          * omegaPartial 3 ({u_str} : ℝ)\n"
            f"        ≤ (({u_str} : ℝ)^(σ/2 - 1) + ({u_str} : ℝ)^((1-σ)/2 - 1))\n"
            f"          * Real.cos ((15/2 : ℝ) * Real.log ({u_str} : ℝ))\n"
            f"          * omegaPartial 3 ({u_str} : ℝ) := by\n"
            f"      calc (({u_str} : ℝ)^({ALO1}) + ({u_str} : ℝ)^({ALO2}))\n"
            f"            * Real.cos ((15/2 : ℝ) * Real.log ({u_str} : ℝ))\n"
            f"            * omegaPartial 3 ({u_str} : ℝ)\n"
            f"          = (({u_str} : ℝ)^({ALO1}) + ({u_str} : ℝ)^({ALO2}))\n"
            f"            * (Real.cos ((15/2 : ℝ) * Real.log ({u_str} : ℝ))\n"
    ...
    if sin_sign == 'nn':
        ...
        im_lo_proof = (
            f"have hDlo : ({u_str} : ℝ)^({ALO1}) - ({u_str} : ℝ)^({ALO2})\n"
            f"        ≤ ({u_str} : ℝ)^(σ/2 - 1) - ({u_str} : ℝ)^((1-σ)/2 - 1) := by\n"
            f"      have h := PrincipiaTractalis.RiemannXiBoxParametric.box_pow_diff_lb\n"
            f"        (σlo := ({SLO_S} : ℝ)) hu1 hσ0\n"
            f"      have e1 : (({SLO_S} : ℝ))/2 - 1 = {ALO1} := by norm_num\n"
            f"      have e2 : (1 - ({SLO_S} : ℝ))/2 - 1 = {ALO2} := by norm_num\n"
            f"      rw [e1, e2] at h; exact h\n"
            f"    have hSW : (0 : ℝ) ≤ Real.sin ((15/2 : ℝ) * Real.log ({u_str} : ℝ))\n"
            f"        * omegaPartial 3 ({u_str} : ℝ) := mul_nonneg hS hW_nn\n"
            f"    have hchain : (({u_str} : ℝ)^({ALO1}) - ({u_str} : ℝ)^({ALO2}))\n"
            f"          * Real.sin ((15/2 : ℝ) * Real.log ({u_str} : ℝ))\n"
            f"          * omegaPartial 3 ({u_str} : ℝ)\n"
            f"        ≤ (({u_str} : ℝ)^(σ/2 - 1) - ({u_str} : ℝ)^((1-σ)/2 - 1))\n"
            f"          * Real.sin ((15/2 : ℝ) * Real.log ({u_str} : ℝ))\n"
            f"          * omegaPartial 3 ({u_str} : ℝ) := by\n"
            ...
            f"            * (Real.sin ((15/2 : ℝ) * Real.log ({u_str} : ℝ))\n"
            f"              * omegaPartial 3 ({u_str} : ℝ)) :=\n"
            f"          mul_le_mul_of_nonneg_right hDlo hSW\n"
            f"        _ = (({u_str} : ℝ)^(σ/2 - 1) - ({u_str} : ℝ)^((1-σ)/2 - 1))\n"
            f"            * Real.sin ((15/2 : ℝ) * Real.log ({u_str} : ℝ))\n"
            f"            * omegaPartial 3 ({u_str} : ℝ) := by ring\n"
            f"    linarith [u{idx}_im_tight_ge, hchain]"
        )
...
theorem node_{idx}_bounds (σ : ℝ)
  ...
  have hA_hi : ({u_str} : ℝ)^(σ/2 - 1) + ({u_str} : ℝ)^((1-σ)/2 - 1)
      ≤ ({u_str} : ℝ)^({AHI1}) + ({u_str} : ℝ)^({AHI2}) :=
    by
    have h := PrincipiaTractalis.RiemannXiBoxParametric.box_pow_sum_ub (σhi := ({SHI_S} : ℝ)) hu1
      (by linarith) hσ1
    have e1 : (({SHI_S} : ℝ))/2 - 1 = {AHI1} := by norm_num
    have e2 : (1 - ({SHI_S} : ℝ))/2 - 1 = {AHI2} := by norm_num
  ...
  have hD_hi : ({u_str} : ℝ)^(σ/2 - 1) - ({u_str} : ℝ)^((1-σ)/2 - 1)
      ≤ ({u_str} : ℝ)^({AHI1}) - ({u_str} : ℝ)^({AHI2}) :=
    by
    have h := PrincipiaTractalis.RiemannXiBoxParametric.box_pow_diff_ub (σhi := ({SHI_S} : ℝ)) hu1 hσ1
    have e1 : (({SHI_S} : ℝ))/2 - 1 = {AHI1} := by norm_num
    have e2 : (1 - ({SHI_S} : ℝ))/2 - 1 = {AHI2} := by norm_num
    rw [e1, e2] at h; exact h
  have hW_nn : (0 : ℝ) ≤ omegaPartial 3 ({u_str} : ℝ) := omegaPartial_nonneg_here 3 _
  have hAstar_nn : (0 : ℝ) ≤ ({u_str} : ℝ)^({AHI1}) + ({u_str} : ℝ)^({AHI2}) := by
    have h := box0_pow_sum_nonneg (({SHI_S}) : ℝ) hu1
    have eq1 : (({SHI_S} : ℝ))/2 - 1 = {AHI1} := by norm_num
    have eq2 : (1 - ({SHI_S} : ℝ))/2 - 1 = {AHI2} := by norm_num
    rw [eq1, eq2] at h; exact h
  have hDstar_nn : (0 : ℝ) ≤ ({u_str} : ℝ)^({AHI1}) - ({u_str} : ℝ)^({AHI2}) := by
    have h := box0_pow_diff_nonneg (σ := ({SHI_S} : ℝ)) (by norm_num) hu1
    have eq1 : (({SHI_S} : ℝ))/2 - 1 = {AHI1} := by norm_num
    have eq2 : (1 - ({SHI_S} : ℝ))/2 - 1 = {AHI2} := by norm_num
    rw [eq1, eq2] at h; exact h
...
def _re_consumer_cos_np(u_str, idx):
    ...
    (A_lo := 0)
    (A_hi := ({u_str} : ℝ)^({AHI1}) + ({u_str} : ℝ)^({AHI2}))
...
def _im_consumer_sin_nn(u_str, idx):
    ...
    (D_lo := 0)
    (D_hi := ({u_str} : ℝ)^({AHI1}) - ({u_str} : ℝ)^({AHI2}))
    (S_lo := 0)
...
def compute_T_bounds(L):
    ...
    _mc = mp.mpf(F(MCONST).numerator) / F(MCONST).denominator
    _co = mp.mpf(MCOEF.numerator) / MCOEF.denominator
    consts = [_mc + (n + 1)**4 * PI**2 + (n + 1)**2 * _co * PI
              for n in range(3)]
    ...
    for t in T:
        if t < 1e-30:
            ...
            continue
        target = float(t) * 1.0005   # was 1.10; 0.05% outward slack is
        # still far above Interval resolution (~1e-15 rel) and recovers ~1.8e-5
        # of xi-space margin that the 10% inflation was discarding.
        denom = 10
...
def emit_M_certificate(idx, L, MHI):
    ...
    _p1 = (1 + BOX_SHI) / 2
    _p2 = _p1 * (_p1 + 1)
    P1 = f"{_p1.numerator}/{_p1.denominator}"
    P2 = f"{_p2.numerator}/{_p2.denominator}"
    ns = f"Box{BOX_K}Seg{idx:02d}M"
...
theorem seg{idx}_T0_closed :
    thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) 0
      = Real.exp (-({e0_re})) * (({MCONST} : ℝ) + π * π + ({MCOEF1} : ℝ) * π) := by
...
theorem seg{idx}_T1_closed :
    thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) 1
      = Real.exp (-({e1_re})) * (({MCONST} : ℝ) + 16 * (π * π) + ({MCOEF2} : ℝ) * π) := by
  unfold thetaPowTermM
  have h_abs : |(15 : ℝ)| = 15 := by rw [abs_of_pos]; norm_num
...
theorem seg{idx}_T2_closed :
    thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) 2
      = Real.exp (-({e2_re})) * (({MCONST} : ℝ) + 81 * (π * π) + ({MCOEF3} : ℝ) * π) := by
  unfold thetaPowTermM
  have h_abs : |(15 : ℝ)| = 15 := by rw [abs_of_pos]; norm_num
...
noncomputable def T0_mirror : _root_.Interval :=
  _root_.Interval.exp (-({e0_iv})) *
    (_root_.Interval.ofRat ({MCONST})
     + _root_.Interval.pi * _root_.Interval.pi
     + _root_.Interval.ofRat ({MCOEF1}) * _root_.Interval.pi)
...
noncomputable def T1_mirror : _root_.Interval :=
  _root_.Interval.exp (-({e1_iv})) *
    (_root_.Interval.ofRat ({MCONST})
     ...
     + _root_.Interval.ofRat ({MCOEF2}) * _root_.Interval.pi)
...
noncomputable def T2_mirror : _root_.Interval :=
  _root_.Interval.exp (-({e2_iv})) *
    (_root_.Interval.ofRat ({MCONST})
     ...
     + _root_.Interval.ofRat ({MCOEF3}) * _root_.Interval.pi)

theorem seg{idx}_T0_lt : thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) 0 < {rat_R(B0)} := by
  rw [seg{idx}_T0_closed]
  refine _root_.Interval.approx_lt
    T0_mirror
    {rat_I(B0)}
    (Real.exp (-({e0_re})) * (({MCONST} : ℝ) + π * π + ({MCOEF1} : ℝ) * π))
    {rat_R(B0)}
    ?_
    (by approx)
    ?_
...
theorem seg{idx}_T1_lt : thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) 1 < {rat_R(B1)} := by
  rw [seg{idx}_T1_closed]
  refine _root_.Interval.approx_lt
    T1_mirror
    {rat_I(B1)}
    (Real.exp (-({e1_re})) * (({MCONST} : ℝ) + 16 * (π * π) + ({MCOEF2} : ℝ) * π))
    {rat_R(B1)}
    ?_
    (by approx)
    ?_
...
theorem seg{idx}_T2_lt : thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) 2 < {rat_R(B2)} := by
  rw [seg{idx}_T2_closed]
  refine _root_.Interval.approx_lt
    T2_mirror
    {rat_I(B2)}
    (Real.exp (-({e2_re})) * (({MCONST} : ℝ) + 81 * (π * π) + ({MCOEF3} : ℝ) * π))
    {rat_R(B2)}
    ?_
    (by approx)
    ?_
...
theorem seg{idx}_M_expand :
    2 * ∑ n ∈ Finset.range 3, thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) n
      = 2 * (thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) 0
             + thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) 1
             + thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) 2) := by
  simp [Finset.sum_range_succ]

theorem box{BOX_K}_seg{idx}_M_le_MHI :
    2 * ∑ n ∈ Finset.range 3, thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) n
      ≤ {rat_R(MHI)} := by
...
def emit_panel_file(idx, panel_num, node_data_list):
    """Emit one PF/Analytic/RiemannXiBox{BOX_K}Panels/Seg{idx:02d}P{panel_num}.lean.
    ...
    return f"""/-
# PF.Analytic.RiemannXiBox{BOX_K}Panels.{ns}
...
namespace PrincipiaTractalis.RiemannXiBox{BOX_K}Panels.{ns}
...
end PrincipiaTractalis.RiemannXiBox{BOX_K}Panels.{ns}
"""
...
def emit_segment_file(idx, L, U, n, MHI):
    ...
    ns = f"Seg{idx:02d}"
    M_ns = f"Box{BOX_K}Seg{idx:02d}M"

    # Panel imports
    panel_imports = "\n".join(
        f"import PF.Analytic.RiemannXiBox{BOX_K}Panels.Seg{idx:02d}P{p_num}"
        for p_num, _, _, _, _, _ in panel_meta)
    panel_opens = "\n".join(
        f"open PrincipiaTractalis.RiemannXiBox{BOX_K}Panels.Seg{idx:02d}P{p_num}"
        for p_num, _, _, _, _, _ in panel_meta)
...
private lemma seg{idx}_error_bound_le :
    (2 * ∑ n' ∈ Finset.range 3, thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) n')
      * (({U_str} : ℝ) - {L_str}) ^ 3 / (24 * ({n} : ℕ) ^ 2)
      ≤ ({rat_R(E_SEG)}) := by
  have hM := box{BOX_K}_seg{idx}_M_le_MHI
  set M := 2 * ∑ n' ∈ Finset.range 3, thetaPowTermM 15 ({P1}) ({P2}) ({L_str}) n' with hM_def
...
theorem box{BOX_K}_seg{idx}_re_integral_lower (σ : ℝ)
    (hσ0 : ({SLO_S} : ℝ) ≤ σ) (hσ1 : σ ≤ {SHI_S}) :
  ...
  have herr := PrincipiaTractalis.RiemannXiBoxParametric.box_re_midpoint_error_on_segment (N := 3) (σlo := ({SLO_S} : ℝ)) (σhi := ({SHI_S} : ℝ)) (p1 := ({P1} : ℝ)) (p2 := ({P2} : ℝ)) (by norm_num) (by norm_num) (by norm_num) (by norm_num) (by norm_num) hσ0 hσ1 hL hLU hn
...
theorem box{BOX_K}_seg{idx}_re_integral_upper (σ : ℝ)
    (hσ0 : ({SLO_S} : ℝ) ≤ σ) (hσ1 : σ ≤ {SHI_S}) :
...
theorem box{BOX_K}_seg{idx}_im_integral_lower (σ : ℝ)
    (hσ0 : ({SLO_S} : ℝ) ≤ σ) (hσ1 : σ ≤ {SHI_S}) :
    ({rat_R(IM_INT_LO)}) ≤ ∫ u in ({L_str} : ℝ)..{U_str}, realThetaImIntegrandN 3 σ 15 u := by
  have hL : (1 : ℝ) ≤ {L_str} := by norm_num
  have hLU : ({L_str} : ℝ) ≤ {U_str} := by norm_num
  have hn : 0 < ({n} : ℕ) := by decide
  have herr := PrincipiaTractalis.RiemannXiBoxParametric.box_im_midpoint_error_on_segment (N := 3) (σlo := ({SLO_S} : ℝ)) (σhi := ({SHI_S} : ℝ)) (p1 := ({P1} : ℝ)) (p2 := ({P2} : ℝ)) (by norm_num) (by norm_num) (by norm_num) (by norm_num) (by norm_num) hσ0 hσ1 hL hLU hn
  have hbound := seg{idx}_error_bound_le
  have habs := abs_le.mp (le_trans herr hbound)
...
theorem box{BOX_K}_seg{idx}_im_integral_upper (σ : ℝ)
    (hσ0 : ({SLO_S} : ℝ) ≤ σ) (hσ1 : σ ≤ {SHI_S}) :
...
def emit_all_segments(indices, panels_dir=None, m_dir="PF/Numerics"):
    if panels_dir is None:
        panels_dir = f"PF/Analytic/RiemannXiBox{BOX_K}Panels"
    os.makedirs(panels_dir, exist_ok=True)
    os.makedirs(m_dir, exist_ok=True)
    for idx in indices:
        spec = next(s for s in SEGMENTS if s['idx'] == idx)
        L, U, n = spec['L'], spec['U'], spec['n']
        _T, _B = compute_T_bounds(L)
        MHI = sum(_B) * 2
        m_path = os.path.join(m_dir, f"Box{BOX_K}Seg{idx:02d}M.lean")
...
if __name__ == "__main__":
    # usage: emit_box_segment.py <box_k> <sigma_lo> <sigma_hi> [seg ...]
    if len(sys.argv) < 4:
        print("usage: emit_box_segment.py <box_k> <sigma_lo> <sigma_hi> [seg ...]")
        sys.exit(2)
    _k = int(sys.argv[1])
    _slo, _shi = F(sys.argv[2]), F(sys.argv[3])
    if not (F(1, 2) <= _slo < _shi <= F(1)):
        print("bad sigma box: need 1/2 <= slo < shi <= 1")
        sys.exit(2)
    _set_box(_k, _slo, _shi)
    print(f"BOX {_k}: sigma in [{_slo}, {_shi}]  "
          f"ALO1={ALO1} ALO2={ALO2} AHI1={AHI1} AHI2={AHI2}")
    indices = [int(x) for x in sys.argv[4:]] or list(range(1, 41))
