/-
# PF.QuasiParallelogramUpper5077a1_r161

★★★ 2026-07-30 — r161: THE UPPER QUASI-PARALLELOGRAM BOUND FOR 5077a1 ★★★

The assembly of W2 for the rank-3 curve `y² + y = x³ − 7x + 6`.  Joining

  * r160 — `x(P₁+P₂) + x(P₁−P₂) = Sfnum/dd`, `x(P₁+P₂)·x(P₁−P₂) = Prnum/dd`
  * r148e — heights of the roots of an integer quadratic: `≤ 2·max(|A|,|B|,|C|)`
  * r159 — `|DD| ≤ 4H₁²H₂²`, `|Sf| ≤ 57H₁²H₂²`, `|Pf| ≤ 114H₁²H₂²`

gives, for any two on-curve rational points with distinct x-coordinates,

    `h(x(P₁+P₂)) · h(x(P₁−P₂)) ≤ 228 · h(x₁)² · h(x₂)²`

and in logarithmic form the upper half of the quasi-parallelogram law,

    `lognh(P₁+P₂) + lognh(P₁−P₂) ≤ 2·lognh(P₁) + 2·lognh(P₂) + log 228`.

The constant is `2 · max(4, 57, 114) = 2 · 114 = 228`, against `2 · 17 = 34`
for 389a1.

The chain of identifications: with `x₁ = a₁/b₁`, `x₂ = a₂/b₂` reduced, the
rational coefficients `Sfnum/dd`, `Prnum/dd` are *exactly* `Sf/DD`, `Pf/DD`
for the integer forms of r157 — so the integer quadratic whose roots are
`x(P₁±P₂)` has coefficient triple `(DD, Sf, Pf)`, whose sizes r159 bounds.

**r148e is reused verbatim, not ported.**  Its statement,
`naiveHeight_mul_le_of_quadratic {x₃ x₄ : ℚ} {A B C : ℤ}`, mentions no curve
anywhere: it is a fact about the roots of an integer quadratic over ℚ.  That
is one stone the 389a1 arc needed that this one does not.

HONEST SCOPE.  The UPPER half only.  The lower half — needed for the exact
parallelogram law in the canonical-height limit — follows by applying this
bound to the pair `(P+Q, P−Q)`, whose sum and difference are `2P` and `2Q`,
together with r144's duplication lower bound; that is the next stone and
requires point-level group manipulation, not just x-coordinate algebra.  No
canonical-height statement, no parallelogram law, and no rank claim is made
here.
...
-/
import PF.SecantBridge5077a1_r160
import PF.QuadraticRootHeights_r148e
import PF.QuasiParallelogramSize5077a1_r159

namespace PrincipiaTractalis.QuasiParallelogramUpper5077a1
...
open PrincipiaTractalis.QuasiParallelogramSize5077a1
...
theorem coeff_max_le (x₁ x₂ : ℚ) :
    max (max (DD x₁.num ((x₁.den : ℤ)) x₂.num ((x₂.den : ℤ))).natAbs
      ...
      ≤ 114 * naiveHeight x₁ ^ 2 * naiveHeight x₂ ^ 2 := by
  ...
  -- NB: unlike 389a1, where `Sf`'s coefficient 17 was the largest, here the
  -- largest is `Pf`'s 114; so `hP` closes the outer branch directly and BOTH
  -- `hD` (4) and `hS` (57) must be weakened to 114.
  refine max_le (max_le ?_ ?_) hP
...
For two on-curve rational points of 5077a1 with distinct x-coordinates:
...
theorem height_prod_upper {x₁ x₂ y₁ y₂ : ℚ} (hx : x₁ ≠ x₂)
    ...
    naiveHeight (xAdd x₁ x₂ y₁ y₂)
        * naiveHeight (xAdd x₁ x₂ y₁ (E5077a1.toAffine.negY x₂ y₂))
  ...
  calc naiveHeight (xAdd x₁ x₂ y₁ y₂)
    ...
    _ ≤ 2 * (114 * naiveHeight x₁ ^ 2 * naiveHeight x₂ ^ 2) :=
        Nat.mul_le_mul_left 2 (coeff_max_le x₁ x₂)
    _ = 228 * naiveHeight x₁ ^ 2 * naiveHeight x₂ ^ 2 := by ring
...
theorem log_upper {x₁ x₂ y₁ y₂ : ℚ} (hx : x₁ ≠ x₂)
    ...
    Real.log (naiveHeight (xAdd x₁ x₂ y₁ y₂))
        + Real.log (naiveHeight (xAdd x₁ x₂ y₁ (E5077a1.toAffine.negY x₂ y₂)))
      ≤ 2 * Real.log (naiveHeight x₁) + 2 * Real.log (naiveHeight x₂)
        + Real.log 228 := by
  have h1 : (1 : ℕ) ≤ naiveHeight (xAdd x₁ x₂ y₁ y₂) := one_le_naiveHeight _
  have h2 : (1 : ℕ) ≤ naiveHeight (xAdd x₁ x₂ y₁ (E5077a1.toAffine.negY x₂ y₂)) :=
  ...
  have hR : ((naiveHeight (xAdd x₁ x₂ y₁ y₂) : ℝ))
      * ((naiveHeight (xAdd x₁ x₂ y₁ (E5077a1.toAffine.negY x₂ y₂)) : ℝ))
      ≤ 228 * (naiveHeight x₁ : ℝ) ^ 2 * (naiveHeight x₂ : ℝ) ^ 2 := by
  ...
  have hpos2 : (0 : ℝ)
      < (naiveHeight (xAdd x₁ x₂ y₁ (E5077a1.toAffine.negY x₂ y₂)) : ℝ) := by
    exact_mod_cast naiveHeight_pos _
  ...
  have hrhs : Real.log (228 * (naiveHeight x₁ : ℝ) ^ 2 * (naiveHeight x₂ : ℝ) ^ 2)
      = Real.log 228 + 2 * Real.log (naiveHeight x₁)
...
end PrincipiaTractalis.QuasiParallelogramUpper5077a1

#print axioms PrincipiaTractalis.QuasiParallelogramUpper5077a1.DD_ne_zero
#print axioms PrincipiaTractalis.QuasiParallelogramUpper5077a1.DD_mul_sum
#print axioms PrincipiaTractalis.QuasiParallelogramUpper5077a1.DD_mul_prod
#print axioms PrincipiaTractalis.QuasiParallelogramUpper5077a1.coeff_max_le
#print axioms PrincipiaTractalis.QuasiParallelogramUpper5077a1.height_prod_upper
#print axioms PrincipiaTractalis.QuasiParallelogramUpper5077a1.log_upper
