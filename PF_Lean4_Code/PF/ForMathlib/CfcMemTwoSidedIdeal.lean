...
Authors: Pablo Cohen
-/
import PF.ForMathlib.TwoSidedIdealClosure
...
* `TwoSidedIdeal.cfcₙ_mem_of_isClosed`: the main result, `cfcₙ f a ∈ I` for `a ∈ I` and
  `I` closed.
* `TwoSidedIdeal.cfcₙ_mem_closure`: for a general two-sided ideal `I` with `a ∈ I` we have
  `cfcₙ f a ∈ I.closure` (see `TwoSidedIdeal.closure`).
-/
...
/-- The non-unital continuous functional calculus maps into the closure of a two-sided ideal: if
`a ∈ I` and `f : ℝ → ℝ`, then `cfcₙ f a ∈ I.closure`. -/
theorem cfcₙ_mem_closure {I : TwoSidedIdeal A} {a : A} (ha : a ∈ I) (f : ℝ → ℝ) :
    cfcₙ f a ∈ I.closure :=
  cfcₙ_mem_of_isClosed I.isClosed_closure (I.le_closure ha) f

end TwoSidedIdeal

-- Axiom audit (to be removed before upstreaming)
#print axioms TwoSidedIdeal.smul_mem_of_isClosed
#print axioms TwoSidedIdeal.cfcₙHom_mem_of_isClosed
#print axioms TwoSidedIdeal.cfcₙ_mem_of_isClosed
#print axioms TwoSidedIdeal.cfcₙ_mem_closure
