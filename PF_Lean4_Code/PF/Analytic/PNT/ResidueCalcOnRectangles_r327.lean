/-
# r327 — RESIDUE CALCULUS ON RECTANGLES (ported for Principia Fractalis)

SPDX-License-Identifier: Apache-2.0

## Upstream chain

1. `anthropics/zeta-23-lean` at commit `cec57f919ccf34e5fa5372b4ba332f7c848bbb6e`
   (Lean v4.33.0-rc2 / mathlib 51e6992efd06126df61a496bebf8f49482a4e129),
   file `Zeta23/FromPNTPlus/ResidueCalcOnRectangles.lean`.
   Copyright (c) 2026 Anthropic, PBC.  Apache-2.0.
2. `AlexKontorovich/PrimeNumberTheoremAnd` at commit
   `6a380f0c4658c04a420a9eb00b1ed62a1e3fde01` (tag v4.32.2), file
   `PrimeNumberTheoremAnd/ResidueCalcOnRectangles.lean`.
   Copyright the PrimeNumberTheoremAnd contributors.  Apache-2.0
   (http://www.apache.org/licenses/LICENSE-2.0).

## PF-side modifications (2026-08-25)

* Renamed to `PF/Analytic/PNT/ResidueCalcOnRectangles_r327.lean` inside PF's
  pinned tree (Lean v4.24.0-rc1 / mathlib v4.24.0-rc1).
* Redirected intra-project import
  `Zeta23.FromPNTPlus.Rectangle → PF.Analytic.PNT.Rectangle_r327`.
* Dropped upstream `Zeta23.FromPNTPlus.Tactic.AdditiveCombination` — the sole
  use of `additive_combination` (in `RectanglePullToNhdOfPole'`) is patched to
  `linear_combination` from mathlib.
* No mathematical content changed.  Residue calculus on rectangles.
  Zero project axioms.
-/
...
import PF.Analytic.PNT.Rectangle_r327
...
/-! ### PF port cutout — RectanglePullToNhdOfPole family removed

Upstream defines three lemmas here — `RectanglePullToNhdOfPole'`,
`RectanglePullToNhdOfPole`, `RectanglePullToNhdOfPole''` — that pull a
rectangle integral onto a small square around a pole.

They are NOT used anywhere in the rectangle argument-principle chain
(`residueTheorem_finset`, `finite_zeros_rectangle`,
`rectangleIntegral'_mul_logDeriv_of_poles`, `rectangleIntegral'_mul_logDeriv`,
`rectangleIntegral'_mul_logDeriv'`).  They are also not used by
`ResidueTheoremInRectangle` (which the argument principle does invoke).

`RectanglePullToNhdOfPole'` uses upstream's own `additive_combination` tactic
(from `Zeta23/FromPNTPlus/Tactic/AdditiveCombination.lean`) which in turn
depends on `Mathlib.Tactic.LinearCombinationPrime` — a module not present in
PF's pinned mathlib v4.24.0-rc1.  Mathlib's `linear_combination` cannot
substitute because the ambient type `E` is only an `AddCommGroup`/`NormedSpace`,
not a `CommSemiring`.

Rather than port the tactic or manually reconstruct the additive identity
across eight rectangle splits, we drop the three lemmas here — they are
outside the minimum dependency closure of the rectangle argument principle
and can be ported separately if a future PF module needs them.
-/
...
-- PF port note: upstream prefixes this lemma with
-- `set_option backward.isDefEq.respectTransparency false in`.  That option
-- is not registered in Lean v4.24.0-rc1; dropping it, since the lemma
-- elaborates without the transparency hint on the pinned toolchain.
...
lemma IsBigO_to_BddAbove {f : ℂ → ℂ} {p : ℂ}
  ...
  · refine bddAbove_def.mpr ?_
    use c
    intro y hy
    simp only [Function.comp_apply, mem_image, Set.mem_diff, mem_singleton_iff] at hy
...
theorem BddAbove_on_rectangle_of_bdd_near {z w p : ℂ} {f : ℂ → ℂ}
  ...
  have : U \ {p} = (U \ W) ∪ ((U ∩ W) \ {p}) := by
    ext x
    simp only [Set.mem_diff, mem_singleton_iff, mem_union, mem_inter_iff]
    constructor
    · intro ⟨xu, x_not_p⟩
      tauto
    · intro h
  ...
  · apply IsCompact.bddAbove_image
    ...
    · apply f_cont.norm.mono
      ...
      simpa
  · exact V_prop.mono
      (image_mono <| Set.diff_subset_diff_left <| subset_trans inter_subset_right W_subset)


/-! ## §END — PF port cutoff

Upstream `ResidueCalcOnRectangles.lean` continues beyond this point with a
"residue calculus" scaffold (`HasSimplePolesOn`, `residue`, `sumResiduesIn`,
`ResidueTheoremOnRectangleWithSimplePole'`, `RectangleIntegral'_eq_sumResiduesIn`, ...)
that is NOT used by the rectangle argument-principle theorem in
`PF/Analytic/RectangleArgumentPrinciple_r327.lean`.  Dropped here to keep the
port minimal and stay clear of newer mathlib APIs that PF's pinned mathlib
v4.24.0-rc1 does not expose (`meromorphicOrderAt_id_sub_const`,
`MeromorphicAt.fun_sum`, `Filter.codiscreteWithin_mono`, `tendsto_finsetSum`,
`Filter.limUnder`, ...).

If those downstream lemmas become useful for future PF work, port them
separately.
-/
