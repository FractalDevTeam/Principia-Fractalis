/-
# r327 — RECTANGLE GEOMETRY (ported for Principia Fractalis)
...
1. `anthropics/zeta-23-lean` at commit `cec57f919ccf34e5fa5372b4ba332f7c848bbb6e`
   ...
   file `Zeta23/FromPNTPlus/Rectangle.lean`.
...
* Renamed to `PF/Analytic/PNT/Rectangle_r327.lean` inside PF's pinned tree
  (Lean v4.24.0-rc1 / mathlib v4.24.0-rc1).
* No mathematical content changed.  Rectangle geometry.  Zero project axioms.
-/
...
lemma square_neg (p : ℂ) (c : ℝ) : Square p (-c) = Square p c := by
  -- PF port note: upstream uses `simpa [Square] using! Rectangle.symm`.
  -- The `using!` variant is not stable in mathlib v4.24.0-rc1 for this shape,
  -- and simp's normalization of `-(-c)` etc leaves the goal in a form where
  -- `Rectangle.symm` no longer matches by `assumption`.  We do the same swap
  -- manually with `ring_nf` on the two corner expressions.
  show Rectangle (-(↑(-c) : ℂ) - (↑(-c) : ℂ) * I + p) ((↑(-c) : ℂ) + (↑(-c) : ℂ) * I + p) =
       Rectangle (-(↑c : ℂ) - (↑c : ℂ) * I + p) ((↑c : ℂ) + (↑c : ℂ) * I + p)
  rw [show (↑(-c) : ℂ) = -(↑c : ℂ) from by push_cast; ring]
  rw [show -(-(↑c : ℂ)) - -(↑c : ℂ) * I + p = (↑c : ℂ) + (↑c : ℂ) * I + p from by ring]
  rw [show -(↑c : ℂ) + -(↑c : ℂ) * I + p = -(↑c : ℂ) - (↑c : ℂ) * I + p from by ring]
...
theorem Complex.nhds_hasBasis_square (p : ℂ) : (𝓝 p).HasBasis (0 < ·) (Square p ·) := by
  suffices
    ...
    by
      -- PF port note: `using!` variant of `simpa` unavailable / unstable
      -- in mathlib v4.24.0-rc1 for this shape; substitute the equivalent
      -- explicit-argument form.
      have h := this.map equivRealProdCLM.symm.toHomeomorph
      simpa only [← nhds_prod_eq, Homeomorph.map_nhds_eq, Homeomorph.image_preimage] using h
