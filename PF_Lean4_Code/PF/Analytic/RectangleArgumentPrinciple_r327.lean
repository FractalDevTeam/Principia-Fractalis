/-
# r327 — RECTANGLE ARGUMENT PRINCIPLE (ported for Principia Fractalis)

SPDX-License-Identifier: Apache-2.0

## Upstream

`anthropics/zeta-23-lean` at commit `cec57f919ccf34e5fa5372b4ba332f7c848bbb6e`
(Lean v4.33.0-rc2 / mathlib 51e6992efd06126df61a496bebf8f49482a4e129),
file `Zeta23/Analytic/RectangleLogDeriv.lean`.
Copyright (c) 2026 Anthropic, PBC.  Apache-2.0.

## PF-side modifications (2026-08-25)

* Renamed to `PF/Analytic/RectangleArgumentPrinciple_r327.lean` inside PF's
  pinned tree (Lean v4.24.0-rc1 / mathlib v4.24.0-rc1).
* Redirected intra-project import
  `Zeta23.FromPNTPlus.ResidueCalcOnRectangles → PF.Analytic.PNT.ResidueCalcOnRectangles_r327`.
* `finite_zeros_rectangle` originally invoked `isDiscrete_of_codiscreteWithin`
  from a newer mathlib.  PF's pin has the underlying facts under the names
  `discreteTopology_of_codiscreteWithin` (from
  `Mathlib.Topology.DiscreteSubset`) and `IsCompact.finite`
  (from `Mathlib.Topology.Compactness.Compact`); the proof is patched in
  place to use those directly.
...
Zero project axioms; ordinary foundations only.
-/
import PF.Analytic.PNT.ResidueCalcOnRectangles_r327
...
theorem finite_zeros_rectangle {f : ℂ → ℂ} {z w : ℂ} (hf : AnalyticOnNhd ℂ f (Rectangle z w))
  ...
  · exact hcomp.of_isClosed_subset
      ...
      inter_subset_left
  · -- PF port note: upstream (against a newer mathlib) invokes
    -- `isDiscrete_of_codiscreteWithin` here.  Mathlib v4.24.0-rc1 exposes the
    -- same fact under the name `discreteTopology_of_codiscreteWithin`
    -- (`Mathlib/Topology/DiscreteSubset.lean:135`), with statement:
    --   `s ∈ codiscreteWithin U → DiscreteTopology (sᶜ ∩ U)`.
    -- Applied to `s = f ⁻¹' {0}ᶜ`, `U = Rectangle z w`, we get
    -- `DiscreteTopology ((f ⁻¹' {0}ᶜ)ᶜ ∩ Rectangle z w)`, and since
    -- `(f ⁻¹' {0}ᶜ)ᶜ = f ⁻¹' {0}` this matches the required subtype after
    -- `inter_comm`.
    rw [inter_comm]
    have hcodis : (f ⁻¹' {0})ᶜ ∈ codiscreteWithin (Rectangle z w) :=
      hf.preimage_zero_mem_codiscreteWithin hfx hx ⟨⟨x, hx⟩, hconv.isPreconnected⟩
    have hdt := discreteTopology_of_codiscreteWithin hcodis
    have hEq : ((f ⁻¹' {0})ᶜ)ᶜ ∩ Rectangle z w = f ⁻¹' {0} ∩ Rectangle z w := by
      congr 1
      exact compl_compl _
    rw [hEq] at hdt
    exact hdt
