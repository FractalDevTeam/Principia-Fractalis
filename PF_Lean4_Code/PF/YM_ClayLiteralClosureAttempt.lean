...
/-- **The Schwartz space `𝓢(ℝ⁴, ℝ)`** as a typed abbreviation — real-valued
    Schwartz functions on Euclidean 4-space. This is the Wightman field
    test-function space. -/
abbrev SchwartzSpaceR4 : Type := SchwartzMap (EuclideanSpace ℝ (Fin 4)) ℝ
...
theorem OS_eigvec_three_halves_ne_zero :
  ...
  simp [OS_eigvec_three_halves, PiLp.zero_apply] at h0
...
theorem OS_eigvec_one_half_ne_zero : OS_eigvec_one_half ≠ 0 := by
  ...
  simp [OS_eigvec_one_half, PiLp.zero_apply] at h0
...
noncomputable def osEmbed : SchwartzSpaceR4 →ₗ[ℝ] H_Wightman where
  ...
  map_smul' c f := by
    funext i
    fin_cases i <;> simp [SchwartzMap.smul_apply, PiLp.smul_apply,
...
theorem osEmbed_symmetric_eq_smul_eigvec
  ...
  · simp [osEmbed, OS_eigvec_three_halves,
          PiLp.smul_apply, Matrix.cons_val_zero]
  · simp [osEmbed, OS_eigvec_three_halves, hf_sym,
          PiLp.smul_apply, Matrix.cons_val_one, Matrix.head_cons]
