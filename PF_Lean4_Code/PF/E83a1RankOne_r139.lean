/-
# PF.E83a1RankOne_r139

★★★ 2026-07-28 — THE FIVE-STONE PATTERN, REPLICATED FOR 83a1 ★★★

For the rank-1 curve **83a1** (`y² + xy + y = x³ + x² + x`) and its rational
point `P = (0, 0)`, this file proves — in ONE self-contained replica of the
37a1 arc (r131–r134) —

  * `P_nonTorsion : ¬ IsOfFinAddOrder P83` — the point `(0, 0)` has infinite
    order in `E83a1(ℚ)`;
  * `E83a1_rank_ge_one : 1 ≤ Module.rank ℤ E83a1.toAffine.Point` — the FLAG.

Construction (all five stones inlined, new data for 83a1):
  ...
  2. **B3 (Bézout/gcd/size).** Homogenized pair `F(a,b) = a⁴ − 3a²b² − 2ab³ + b⁴`,
     ...
     `83·b⁶` and `83·a⁷`, hence `gcd(F, D) ∣ 83` for coprime inputs, and the
     cofactor coefficient sums give the size bound with
     `κ = 319 + 151 = 470` (`b`-branch `159 + 59 = 218 ≤ 470`).
  3. **B4 (height inequality).** `naiveHeight x ^ 4 ≤ 470 · naiveHeight (f x / g x)`
     for EVERY rational `x`, reusing r133's curve-generic `naiveHeight_div_int`.
  4. **B5 (the chain).** From `(0, 0)`: x-values `0 → 1 → −3/16 → 83281/1600`,
     heights `1, 1, 16, 83281`; the driver (r130, `κ = 470`) fires at index 3
     since `naiveHeight (xs 3) = 83281 > 470` (`gcd(83281, 1600) = 1`).
...
concludes `1 ≤ Module.rank ℤ E83a1(ℚ)` — a LOWER bound only.  It does not
...
namespace PrincipiaTractalis.E83a1RankOne
...
/-! ## §1 — the curve 83a1 and the duplication polynomials -/

/-- **Curve 83a1**: `y² + xy + y = x³ + x² + x`, i.e.
`(a₁, a₂, a₃, a₄, a₆) = (1, 1, 1, 1, 0)`. -/
def E83a1 : WeierstrassCurve ℚ := ⟨1, 1, 1, 1, 0⟩

@[simp] lemma E83a1_a₁ : E83a1.a₁ = 1 := rfl
@[simp] lemma E83a1_a₂ : E83a1.a₂ = 1 := rfl
@[simp] lemma E83a1_a₃ : E83a1.a₃ = 1 := rfl
@[simp] lemma E83a1_a₄ : E83a1.a₄ = 1 := rfl
@[simp] lemma E83a1_a₆ : E83a1.a₆ = 0 := rfl

/-- The duplication numerator `f(x) = x⁴ − 3x² − 2x + 1`
(= `x⁴ − b₄x² − 2b₆x − b₈` for 83a1: `b₄ = 3`, `b₆ = 1`, `b₈ = −1`). -/
def f (x : ℚ) : ℚ := x ^ 4 - 3 * x ^ 2 - 2 * x + 1

/-- The duplication denominator `g(x) = 4x³ + 5x² + 6x + 1`
(= `ψ₂² = 4x³ + b₂x² + 2b₄x + b₆` for 83a1: `b₂ = 5`). -/
def g (x : ℚ) : ℚ := 4 * x ^ 3 + 5 * x ^ 2 + 6 * x + 1

/-- Homogenized duplication numerator for 83a1: `b⁴·f(a/b)`. -/
...
/-! ## §2 — the two Bézout identities (resultant 83² at cofactor level) -/

/-- **Bézout identity, `b`-side**: eliminates `a` down to `83·b⁶`. -/
theorem bezout_b (a b : ℤ) :
    (48 * a ^ 2 + 40 * a * b + 71 * b ^ 2) * F a b
      + (-12 * a ^ 3 + 5 * a ^ 2 * b + 30 * a * b ^ 2 + 12 * b ^ 3) * G3 a b
      = 83 * b ^ 6 := by
  simp only [F, G3]; ring

/-- **Bézout identity, `a`-side**: eliminates `b` down to `83·a⁷`, already in
the `D = b·G3` denominator form. -/
theorem bezout_a (a b : ℤ) :
    (83 * a ^ 3 + 76 * a ^ 2 * b + 136 * a * b ^ 2 + 24 * b ^ 3) * F a b
      + (-19 * a ^ 3 + 52 * a ^ 2 * b + 56 * a * b ^ 2 - 24 * b ^ 3) * D a b
      = 83 * a ^ 7 := by
...
theorem bezout_b_D (a b : ℤ) :
    ((48 * a ^ 2 + 40 * a * b + 71 * b ^ 2) * b) * F a b
      + (-12 * a ^ 3 + 5 * a ^ 2 * b + 30 * a * b ^ 2 + 12 * b ^ 3) * D a b
      = 83 * b ^ 7 := by
  simp only [F, D, G3]; ring

/-! ## §3 — the gcd bound: for coprime `(a, b)`, `gcd (F, D) ∣ 83` -/

/-- **The gcd bound.** For coprime `a b : ℤ`, the gcd of the duplication
numerator and denominator divides `83`: the gcd divides `83·a⁷` and `83·b⁷`,
and coprimality of `a⁷, b⁷` squeezes it into `83`. -/
theorem gcd_dvd_83 {a b : ℤ} (h : IsCoprime a b) :
    (Int.gcd (F a b) (D a b) : ℤ) ∣ 83 := by
  have hdF : (Int.gcd (F a b) (D a b) : ℤ) ∣ F a b := Int.gcd_dvd_left _ _
  have hdD : (Int.gcd (F a b) (D a b) : ℤ) ∣ D a b := Int.gcd_dvd_right _ _
  have hb7 : (Int.gcd (F a b) (D a b) : ℤ) ∣ 83 * b ^ 7 := by
    rw [← bezout_b_D a b]
    exact dvd_add (hdF.mul_left _) (hdD.mul_left _)
  have ha7 : (Int.gcd (F a b) (D a b) : ℤ) ∣ 83 * a ^ 7 := by
    rw [← bezout_a a b]
    exact dvd_add (hdF.mul_left _) (hdD.mul_left _)
  have h7 : IsCoprime (a ^ 7) (b ^ 7) := h.pow
  obtain ⟨u, v, huv⟩ := h7
  have key : (83 : ℤ) = u * (83 * a ^ 7) + v * (83 * b ^ 7) := by
    linear_combination (-83 : ℤ) * huv
  rw [key]
  exact dvd_add (ha7.mul_left u) (hb7.mul_left v)

/-- `gcd_dvd_83` in `ℕ`-form. -/
theorem gcd_dvd_83_nat {a b : ℤ} (h : IsCoprime a b) :
    Int.gcd (F a b) (D a b) ∣ 83 := by
  exact_mod_cast gcd_dvd_83 h

/-! ## §4 — the size lower bound

`83·H⁷ ≤ 470·H³·max |F| |D|` with `H = max |a| |b|`. Cofactor coefficient
sums: `a`-side `319 + 151 = 470`, `b`-side `159 + 59 = 218 ≤ 470`. -/
...
`|(48a² + 40ab + 71b²)·b| ≤ 159·H³`. -/
private theorem c1_bound (a b : ℤ) :
    ((48 * a ^ 2 + 40 * a * b + 71 * b ^ 2) * b).natAbs
      ≤ 159 * (max a.natAbs b.natAbs) ^ 3 := by
  have h48 : ((48 : ℤ)).natAbs = 48 := rfl
  have h40 : ((40 : ℤ)).natAbs = 40 := rfl
  have h71 : ((71 : ℤ)).natAbs = 71 := rfl
  ...
  have e3 : ((71 : ℤ) * b ^ 2).natAbs = 71 * b.natAbs ^ 2 := by
    rw [Int.natAbs_mul, Int.natAbs_pow, h71]
  calc ((48 * a ^ 2 + 40 * a * b + 71 * b ^ 2) * b).natAbs
      = (48 * a ^ 2 + 40 * a * b + 71 * b ^ 2).natAbs * b.natAbs :=
        Int.natAbs_mul _ _
    _ ≤ ((48 * a ^ 2 + 40 * a * b).natAbs + (71 * b ^ 2).natAbs) * b.natAbs :=
        Nat.mul_le_mul (Int.natAbs_add_le _ _) le_rfl
    _ ≤ ((((48 : ℤ) * a ^ 2).natAbs + ((40 : ℤ) * a * b).natAbs)
          + ((71 : ℤ) * b ^ 2).natAbs) * b.natAbs :=
        Nat.mul_le_mul (Nat.add_le_add_right (Int.natAbs_add_le _ _) _) le_rfl
    _ = (48 * a.natAbs ^ 2 + 40 * a.natAbs * b.natAbs + 71 * b.natAbs ^ 2)
          * b.natAbs := by rw [e1, e2, e3]
    _ = 48 * (a.natAbs ^ 2 * b.natAbs) + 40 * (a.natAbs * b.natAbs ^ 2)
          + 71 * b.natAbs ^ 3 := by ring
    _ ≤ 159 * (max a.natAbs b.natAbs) ^ 3 := by
        have m1 := mono21 a b
        have m2 := mono12 a b
        have m3 := mono03 a b
        linarith
...
`|−12a³ + 5a²b + 30ab² + 12b³| ≤ 59·H³`. -/
private theorem c2_bound (a b : ℤ) :
    (-12 * a ^ 3 + 5 * a ^ 2 * b + 30 * a * b ^ 2 + 12 * b ^ 3).natAbs
  ...
  have e4 : ((12 : ℤ) * b ^ 3).natAbs = 12 * b.natAbs ^ 3 := by
    rw [Int.natAbs_mul, Int.natAbs_pow, h12']
  calc (-12 * a ^ 3 + 5 * a ^ 2 * b + 30 * a * b ^ 2 + 12 * b ^ 3).natAbs
...
`|83a³ + 76a²b + 136ab² + 24b³| ≤ 319·H³`. -/
private theorem c3_bound (a b : ℤ) :
    (83 * a ^ 3 + 76 * a ^ 2 * b + 136 * a * b ^ 2 + 24 * b ^ 3).natAbs
      ≤ 319 * (max a.natAbs b.natAbs) ^ 3 := by
  have h83 : ((83 : ℤ)).natAbs = 83 := rfl
  have h76 : ((76 : ℤ)).natAbs = 76 := rfl
  have h136 : ((136 : ℤ)).natAbs = 136 := rfl
  have h24 : ((24 : ℤ)).natAbs = 24 := rfl
  have e1 : ((83 : ℤ) * a ^ 3).natAbs = 83 * a.natAbs ^ 3 := by
    rw [Int.natAbs_mul, Int.natAbs_pow, h83]
  have e2 : ((76 : ℤ) * a ^ 2 * b).natAbs = 76 * a.natAbs ^ 2 * b.natAbs := by
    rw [Int.natAbs_mul, Int.natAbs_mul, Int.natAbs_pow, h76]
  have e3 : ((136 : ℤ) * a * b ^ 2).natAbs = 136 * a.natAbs * b.natAbs ^ 2 := by
    rw [Int.natAbs_mul, Int.natAbs_mul, Int.natAbs_pow, h136]
  have e4 : ((24 : ℤ) * b ^ 3).natAbs = 24 * b.natAbs ^ 3 := by
    rw [Int.natAbs_mul, Int.natAbs_pow, h24]
  calc (83 * a ^ 3 + 76 * a ^ 2 * b + 136 * a * b ^ 2 + 24 * b ^ 3).natAbs
      ≤ (83 * a ^ 3 + 76 * a ^ 2 * b + 136 * a * b ^ 2).natAbs
          + (24 * b ^ 3).natAbs := Int.natAbs_add_le _ _
    _ ≤ ((83 * a ^ 3 + 76 * a ^ 2 * b).natAbs + ((136 : ℤ) * a * b ^ 2).natAbs)
          + ((24 : ℤ) * b ^ 3).natAbs :=
        Nat.add_le_add_right (Int.natAbs_add_le _ _) _
    _ ≤ ((((83 : ℤ) * a ^ 3).natAbs + ((76 : ℤ) * a ^ 2 * b).natAbs)
            + ((136 : ℤ) * a * b ^ 2).natAbs)
          + ((24 : ℤ) * b ^ 3).natAbs :=
        Nat.add_le_add_right (Nat.add_le_add_right (Int.natAbs_add_le _ _) _) _
    _ = 83 * a.natAbs ^ 3 + 76 * a.natAbs ^ 2 * b.natAbs
          + 136 * a.natAbs * b.natAbs ^ 2 + 24 * b.natAbs ^ 3 := by
        rw [e1, e2, e3, e4]
    _ ≤ 319 * (max a.natAbs b.natAbs) ^ 3 := by
        have m1 := mono30 a b
        have m2 := mono21 a b
        have m3 := mono12 a b
        have m4 := mono03 a b
...
`|−19a³ + 52a²b + 56ab² − 24b³| ≤ 151·H³`. -/
private theorem c4_bound (a b : ℤ) :
    (-19 * a ^ 3 + 52 * a ^ 2 * b + 56 * a * b ^ 2 - 24 * b ^ 3).natAbs
      ≤ 151 * (max a.natAbs b.natAbs) ^ 3 := by
  have h19 : ((-19 : ℤ)).natAbs = 19 := rfl
  have h52 : ((52 : ℤ)).natAbs = 52 := rfl
  have h56 : ((56 : ℤ)).natAbs = 56 := rfl
  have h24 : ((24 : ℤ)).natAbs = 24 := rfl
  have e1 : ((-19 : ℤ) * a ^ 3).natAbs = 19 * a.natAbs ^ 3 := by
    rw [Int.natAbs_mul, Int.natAbs_pow, h19]
  have e2 : ((52 : ℤ) * a ^ 2 * b).natAbs = 52 * a.natAbs ^ 2 * b.natAbs := by
    rw [Int.natAbs_mul, Int.natAbs_mul, Int.natAbs_pow, h52]
  have e3 : ((56 : ℤ) * a * b ^ 2).natAbs = 56 * a.natAbs * b.natAbs ^ 2 := by
    rw [Int.natAbs_mul, Int.natAbs_mul, Int.natAbs_pow, h56]
  have e4 : ((24 : ℤ) * b ^ 3).natAbs = 24 * b.natAbs ^ 3 := by
    rw [Int.natAbs_mul, Int.natAbs_pow, h24]
  calc (-19 * a ^ 3 + 52 * a ^ 2 * b + 56 * a * b ^ 2 - 24 * b ^ 3).natAbs
      ≤ (-19 * a ^ 3 + 52 * a ^ 2 * b + 56 * a * b ^ 2).natAbs
          + (24 * b ^ 3).natAbs := Int.natAbs_sub_le _ _
    _ ≤ ((-19 * a ^ 3 + 52 * a ^ 2 * b).natAbs + ((56 : ℤ) * a * b ^ 2).natAbs)
          + ((24 : ℤ) * b ^ 3).natAbs :=
        Nat.add_le_add_right (Int.natAbs_add_le _ _) _
    _ ≤ ((((-19 : ℤ) * a ^ 3).natAbs + ((52 : ℤ) * a ^ 2 * b).natAbs)
            + ((56 : ℤ) * a * b ^ 2).natAbs)
          + ((24 : ℤ) * b ^ 3).natAbs :=
        Nat.add_le_add_right (Nat.add_le_add_right (Int.natAbs_add_le _ _) _) _
    _ = 19 * a.natAbs ^ 3 + 52 * a.natAbs ^ 2 * b.natAbs
          + 56 * a.natAbs * b.natAbs ^ 2 + 24 * b.natAbs ^ 3 := by
        rw [e1, e2, e3, e4]
    _ ≤ 151 * (max a.natAbs b.natAbs) ^ 3 := by
        have m1 := mono30 a b
        have m2 := mono21 a b
        have m3 := mono12 a b
        have m4 := mono03 a b
...
/-- The `b`-branch of the size bound: `83·|b|⁷ ≤ 470·H³·max |F| |D|`
(the natural constant is `218`; we relax to `470` to share one constant). -/
theorem size_bound_b (a b : ℤ) :
    83 * b.natAbs ^ 7
      ≤ 470 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs := by
  have h83 : ((83 : ℤ)).natAbs = 83 := rfl
  have h0 : ((83 : ℤ) * b ^ 7).natAbs = 83 * b.natAbs ^ 7 := by
    rw [Int.natAbs_mul, Int.natAbs_pow, h83]
  have hterm1 : (((48 * a ^ 2 + 40 * a * b + 71 * b ^ 2) * b) * F a b).natAbs
      ≤ 159 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs := by
    rw [Int.natAbs_mul]
    exact Nat.mul_le_mul (c1_bound a b) (le_max_left _ _)
  have hterm2 : ((-12 * a ^ 3 + 5 * a ^ 2 * b + 30 * a * b ^ 2 + 12 * b ^ 3)
        * D a b).natAbs
      ≤ 59 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs := by
    rw [Int.natAbs_mul]
    exact Nat.mul_le_mul (c2_bound a b) (le_max_right _ _)
  calc 83 * b.natAbs ^ 7 = ((83 : ℤ) * b ^ 7).natAbs := h0.symm
    _ = (((48 * a ^ 2 + 40 * a * b + 71 * b ^ 2) * b) * F a b
          + (-12 * a ^ 3 + 5 * a ^ 2 * b + 30 * a * b ^ 2 + 12 * b ^ 3)
              * D a b).natAbs := by
        rw [bezout_b_D a b]
    _ ≤ (((48 * a ^ 2 + 40 * a * b + 71 * b ^ 2) * b) * F a b).natAbs
          + ((-12 * a ^ 3 + 5 * a ^ 2 * b + 30 * a * b ^ 2 + 12 * b ^ 3)
              * D a b).natAbs :=
        Int.natAbs_add_le _ _
    _ ≤ 159 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs
          + 59 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs :=
        Nat.add_le_add hterm1 hterm2
    _ = 218 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs := by
        ring
    _ ≤ 470 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs :=
        Nat.mul_le_mul (Nat.mul_le_mul (by norm_num) le_rfl) le_rfl

/-- The `a`-branch of the size bound: `83·|a|⁷ ≤ 470·H³·max |F| |D|`. -/
theorem size_bound_a (a b : ℤ) :
    83 * a.natAbs ^ 7
      ≤ 470 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs := by
  have h83 : ((83 : ℤ)).natAbs = 83 := rfl
  have h0 : ((83 : ℤ) * a ^ 7).natAbs = 83 * a.natAbs ^ 7 := by
    rw [Int.natAbs_mul, Int.natAbs_pow, h83]
  have hterm1 : ((83 * a ^ 3 + 76 * a ^ 2 * b + 136 * a * b ^ 2 + 24 * b ^ 3)
        * F a b).natAbs
      ≤ 319 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs := by
    rw [Int.natAbs_mul]
    exact Nat.mul_le_mul (c3_bound a b) (le_max_left _ _)
  have hterm2 : ((-19 * a ^ 3 + 52 * a ^ 2 * b + 56 * a * b ^ 2 - 24 * b ^ 3)
        * D a b).natAbs
      ≤ 151 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs := by
    rw [Int.natAbs_mul]
    exact Nat.mul_le_mul (c4_bound a b) (le_max_right _ _)
  calc 83 * a.natAbs ^ 7 = ((83 : ℤ) * a ^ 7).natAbs := h0.symm
    _ = ((83 * a ^ 3 + 76 * a ^ 2 * b + 136 * a * b ^ 2 + 24 * b ^ 3) * F a b
          + (-19 * a ^ 3 + 52 * a ^ 2 * b + 56 * a * b ^ 2 - 24 * b ^ 3)
              * D a b).natAbs := by
        rw [bezout_a a b]
    _ ≤ ((83 * a ^ 3 + 76 * a ^ 2 * b + 136 * a * b ^ 2 + 24 * b ^ 3) * F a b).natAbs
          + ((-19 * a ^ 3 + 52 * a ^ 2 * b + 56 * a * b ^ 2 - 24 * b ^ 3)
              * D a b).natAbs :=
        Int.natAbs_add_le _ _
    _ ≤ 319 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs
          + 151 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs :=
        Nat.add_le_add hterm1 hterm2
    _ = 470 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs := by
        ring

/-- **The size lower bound.** For all `a b : ℤ`, with `H = max |a| |b|`:
`83·H⁷ ≤ 470·H³·max |F(a,b)| |D(a,b)|`. -/
theorem size_bound (a b : ℤ) :
    83 * (max a.natAbs b.natAbs) ^ 7
      ≤ 470 * (max a.natAbs b.natAbs) ^ 3 * max (F a b).natAbs (D a b).natAbs := by
...
/-- Pure-`ℕ` descent arithmetic: from `83·H⁷ ≤ 470·H³·(M·d)` with `d ≤ 83`
and `1 ≤ H`, cancel `H³` to get `83·H⁴ ≤ 470·83·M`. -/
private theorem descend {H M d : ℕ} (hd : d ≤ 83) (hH : 1 ≤ H)
    (hkey : 83 * H ^ 7 ≤ 470 * H ^ 3 * (M * d)) : 83 * H ^ 4 ≤ 470 * 83 * M := by
  have hH0 : 0 < H := hH
  have h2 : (83 * H ^ 4) * H ^ 3 ≤ (470 * 83 * M) * H ^ 3 := by
    calc (83 * H ^ 4) * H ^ 3 = 83 * H ^ 7 := by ring
      _ ≤ 470 * H ^ 3 * (M * d) := hkey
      _ ≤ 470 * H ^ 3 * (M * 83) := Nat.mul_le_mul le_rfl (Nat.mul_le_mul le_rfl hd)
      _ = (470 * 83 * M) * H ^ 3 := by ring
  exact Nat.le_of_mul_le_mul_right h2 (pow_pos hH0 3)

/-- **The reduced height bound.** For coprime `a b` with `b ≠ 0`, after
dividing the duplication pair `(F, D)` by `gcd(F, D)` (which divides `83`),
...
theorem reduced_height_bound {a b : ℤ} (h : IsCoprime a b) (hb : b ≠ 0)
    (_hD : D a b ≠ 0) :
    83 * (max a.natAbs b.natAbs) ^ 4
      ≤ 470 * 83 *
          max ((F a b / (Int.gcd (F a b) (D a b) : ℤ)).natAbs)
              ((D a b / (Int.gcd (F a b) (D a b) : ℤ)).natAbs) := by
  have hd83 : Int.gcd (F a b) (D a b) ≤ 83 :=
    Nat.le_of_dvd (by norm_num) (gcd_dvd_83_nat h)
  ...
  rw [hsplit] at key
  exact descend hd83 hH1 key

/-- `reduced_height_bound` with the common factor `83` cancelled:
`H⁴ ≤ 470·max |F/g| |D/g|`. -/
theorem reduced_height_bound' {a b : ℤ} (h : IsCoprime a b) (hb : b ≠ 0)
    (hD : D a b ≠ 0) :
  ...
  have key2 : 83 * (max a.natAbs b.natAbs) ^ 4
      ≤ 83 * (470 *
          max ((F a b / (Int.gcd (F a b) (D a b) : ℤ)).natAbs)
              ((D a b / (Int.gcd (F a b) (D a b) : ℤ)).natAbs)) := by
    calc 83 * (max a.natAbs b.natAbs) ^ 4
        ≤ 470 * 83 *
            max ((F a b / (Int.gcd (F a b) (D a b) : ℤ)).natAbs)
...
/-! ## §6 — the duplication formula on the curve (B2 for 83a1, `a₁ = 1`)

The on-curve square is `w² = g(x)` for `w = 2y + x + 1` (`= 2y + a₁x + a₃`). -/

/-- On 83a1, `(2y + x + 1)² = g(x)`: four times the Weierstrass equation. -/
theorem w_sq {x y : ℚ} (h : E83a1.toAffine.Equation x y) :
    (2 * y + x + 1) ^ 2 = g x := by
  rw [Affine.equation_iff] at h
  simp only [E83a1_a₁, E83a1_a₂, E83a1_a₃, E83a1_a₄, E83a1_a₆] at h
  simp only [g]
...
83a1 is 2-torsion, so doubling never leaves the affine chart. -/
...
/-- On 83a1, `2y + x + 1 ≠ 0` for every rational affine point: otherwise
`g(x) = (2y + x + 1)² = 0`, contradicting `g_ne_zero`. -/
theorem w_ne_zero {x y : ℚ} (h : E83a1.toAffine.Equation x y) :
    2 * y + x + 1 ≠ 0 := by
  intro h0
  apply g_ne_zero x
  rw [← w_sq h, h0]
...
/-- `negY` on 83a1 is `-y - x - 1`. -/
lemma negY_eq (x y : ℚ) : E83a1.toAffine.negY x y = -y - x - 1 := by
  simp only [Affine.negY, E83a1_a₁, E83a1_a₃]
  ring

/-- No rational affine point of 83a1 is 2-torsion: `y ≠ negY x y`. -/
theorem y_ne_negY {x y : ℚ} (h : E83a1.toAffine.Equation x y) :
    y ≠ E83a1.toAffine.negY x y := by
  rw [negY_eq]
  intro hy
  exact w_ne_zero h (by linarith)

/-- The tangent slope at a rational affine point of 83a1 is
`(3x² + 2x + 1 − y)/(2y + x + 1)`. -/
theorem slope_eq {x y : ℚ} (h : E83a1.toAffine.Nonsingular x y) :
    E83a1.toAffine.slope x x y y
      = (3 * x ^ 2 + 2 * x + 1 - y) / (2 * y + x + 1) := by
  rw [Affine.slope_of_Y_ne rfl (y_ne_negY h.left), negY_eq]
  have hnum : 3 * x ^ 2 + 2 * E83a1.toAffine.a₂ * x + E83a1.toAffine.a₄
      - E83a1.toAffine.a₁ * y = 3 * x ^ 2 + 2 * x + 1 - y := by
    simp only [E83a1_a₁, E83a1_a₂, E83a1_a₄]
    ring
  have hden : y - (-y - x - 1) = 2 * y + x + 1 := by ring
  rw [hnum, hden]

/-- **The duplication formula for 83a1.**  For any rational affine point
...
theorem dbl_x {x y : ℚ} (h : E83a1.toAffine.Nonsingular x y) :
    ∃ (x' y' : ℚ) (h' : E83a1.toAffine.Nonsingular x' y'),
      Point.some h + Point.some h = Point.some h' ∧ x' = f x / g x := by
  have hy : y ≠ E83a1.toAffine.negY x y := y_ne_negY h.left
  ...
  simp only [E83a1_a₁, E83a1_a₂, E83a1_a₃, E83a1_a₄, E83a1_a₆] at hEq
  ...
  rw [slope_eq h]
  simp only [Affine.addX, E83a1_a₁, E83a1_a₂]
  rw [← hM, ← hsq]
  field_simp
  ring

/-! ## §7 — the duplication height inequality on 83a1 (B4, κ = 470) -/
...
theorem duplication_height_bound (x : ℚ) :
  ...
  calc naiveHeight x ^ 4
    ...
    _ = 470 * naiveHeight (f x / g x) := by rw [← hfg]

/-- **Duplication height step on the curve.** For any rational affine point
`P = some h` of 83a1 at `(x, y)`: `P + P` is an affine point `some h'` at
some `(x', y')` with `naiveHeight x ^ 4 ≤ 470 * naiveHeight x'`. -/
theorem dbl_height {x y : ℚ} (h : E83a1.toAffine.Nonsingular x y) :
    ∃ (x' y' : ℚ) (h' : E83a1.toAffine.Nonsingular x' y'),
      Point.some h + Point.some h = Point.some h' ∧
        naiveHeight x ^ 4 ≤ 470 * naiveHeight x' := by
  obtain ⟨x', y', h', hadd, hx'⟩ := dbl_x h
  refine ⟨x', y', h', hadd, ?_⟩
  rw [hx']
  exact duplication_height_bound x

/-! ## §8 — the base point `P = (0, 0)` on 83a1 -/

/-- `(0, 0)` is a nonsingular rational point of 83a1: `a₆ = 0`, and the
Y-partial there is `a₃ = 1 ≠ 0`. -/
theorem P83_nonsingular : E83a1.toAffine.Nonsingular 0 0 := by
  rw [Affine.nonsingular_zero]
  refine ⟨rfl, Or.inl ?_⟩
  show (1 : ℚ) ≠ 0
  norm_num

/-- **The base point** `P83 = (0, 0) ∈ E83a1(ℚ)`. -/
noncomputable def P83 : E83a1.toAffine.Point := Point.some P83_nonsingular
...
noncomputable def chain : ℕ → Σ' (x y : ℚ), E83a1.toAffine.Nonsingular x y :=
  Nat.rec (motive := fun _ => Σ' (x y : ℚ), E83a1.toAffine.Nonsingular x y)
    ⟨0, 0, P83_nonsingular⟩
    fun _ c =>
...
/-- The chain as points of the group `E83a1(ℚ)`. -/
noncomputable def pts (n : ℕ) : E83a1.toAffine.Point := Point.some (chain n).2.2

@[simp] lemma xs_zero : xs 0 = 0 := rfl

lemma pts_zero : pts 0 = P83 := rfl
...
/-- Each chain step satisfies the quartic height inequality (κ = 470). -/
lemma height_step (n : ℕ) :
    naiveHeight (xs n) ^ 4 ≤ 470 * naiveHeight (xs (n + 1)) :=
  (dbl_height (chain n).2.2).choose_spec.choose_spec.choose_spec.2

/-- The chain is the `2ⁿ`-multiples of the base point: `pts n = 2ⁿ • P83`. -/
lemma pts_eq_two_pow_smul (n : ℕ) : pts n = ((2 : ℤ) ^ n) • P83 := by
  induction n with
...
/-- `x(4P) = −3/16`. -/
lemma xs_two : xs 2 = -3 / 16 := by
  rw [xs_succ 1, xs_one]; norm_num [f, g]

/-- `x(8P) = 83281/1600`. -/
lemma xs_three : xs 3 = 83281 / 1600 := by
  rw [xs_succ 2, xs_two]; norm_num [f, g]

/-- `naiveHeight (x(8P)) = 83281` — the fraction is already reduced:
`gcd(83281, 1600) = 1` (`83281` is odd and ends in `1`, `1600 = 2⁶·5²`). -/
lemma naiveHeight_xs_three : naiveHeight (xs 3) = 83281 := by
  have h3 : xs 3 = ((83281 : ℤ) : ℚ) / ((1600 : ℤ) : ℚ) := by
    rw [xs_three]; norm_num
  have hg : Int.gcd 83281 1600 = 1 := by norm_num
  rw [h3, naiveHeight_div_int 83281 1600 (by norm_num), hg]
  norm_num

/-- The threshold check: `x(8P)` clears the curve constant `κ = 470`. -/
lemma threshold : 470 < naiveHeight (xs 3) := by
...
theorem xs_shifted_infinite : (Set.range fun n => xs (n + 3)).Infinite :=
  infinite_of_duplication_step (κ := 470) (fun n => xs (n + 3))
...
def X : E83a1.toAffine.Point → ℚ
  | .zero => 0
  | @Point.some _ _ _ x _ _ => x

lemma X_pts (n : ℕ) : X (pts n) = xs n := rfl

/-- **CAPSTONE 1 — `P = (0, 0)` is non-torsion on 83a1.** If `P83` had
finite order, `AddSubgroup.zmultiples P83` would be a finite set; its image
under `X` would be finite, yet it contains the infinite range of §10 (every
`pts (n+3) = 2^(n+3) • P83` is a `ℤ`-multiple of `P83`). -/
theorem P_nonTorsion : ¬ IsOfFinAddOrder P83 := by
  intro hfin
  have hfinite := hfin.finite_zmultiples
  have hsub : (Set.range fun n => xs (n + 3))
      ⊆ X '' (AddSubgroup.zmultiples P83 : Set _) := by
    rintro q ⟨n, rfl⟩
...
/-! ## §12 — THE FLAG: rank ≥ 1 for 83a1 -/

/-- The r129 certificate, discharged: `P83` has infinite order in
`E83a1(ℚ)`. -/
theorem P83_certificate : NonTorsionCertificate E83a1.toAffine P83 :=
  P_nonTorsion

/-- **CAPSTONE 2 — the Mordell–Weil rank of 83a1 is at least 1.** -/
theorem E83a1_rank_ge_one : 1 ≤ Module.rank ℤ E83a1.toAffine.Point :=
  mordellWeil_rank_ge_one E83a1.toAffine P83 P_nonTorsion

end PrincipiaTractalis.E83a1RankOne

#print axioms PrincipiaTractalis.E83a1RankOne.bezout_b
#print axioms PrincipiaTractalis.E83a1RankOne.bezout_a
#print axioms PrincipiaTractalis.E83a1RankOne.gcd_dvd_83
#print axioms PrincipiaTractalis.E83a1RankOne.dbl_x
#print axioms PrincipiaTractalis.E83a1RankOne.duplication_height_bound
#print axioms PrincipiaTractalis.E83a1RankOne.dbl_height
#print axioms PrincipiaTractalis.E83a1RankOne.xs_three
#print axioms PrincipiaTractalis.E83a1RankOne.naiveHeight_xs_three
#print axioms PrincipiaTractalis.E83a1RankOne.xs_shifted_infinite
#print axioms PrincipiaTractalis.E83a1RankOne.P_nonTorsion
#print axioms PrincipiaTractalis.E83a1RankOne.P83_certificate
#print axioms PrincipiaTractalis.E83a1RankOne.E83a1_rank_ge_one
