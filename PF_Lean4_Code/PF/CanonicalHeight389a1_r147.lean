/-
# PF.CanonicalHeight389a1_r147

★★★ 2026-07-28 — W3 OF THE INDEPENDENCE ARC: THE CANONICAL HEIGHT ★★★

The canonical (Néron–Tate) height on `E389a1(ℚ)`, constructed as the real
limit `ĥ(R) := lim_{n} log h(x(2ⁿR)) / 4ⁿ`, from the TWO-SIDED duplication
window of W0 (r143) + W1 (r145):

  `H⁴ ≤ 1728·h(x(2P))`  and  `h(x(2P)) ≤ 17·H⁴`
  ⟹  `|log h(2R) − 4·log h(R)| ≤ log 1728`
  ⟹  `hseq R n := log h(2ⁿR)/4ⁿ` is Cauchy with explicit geometric modulus
  ⟹  `ĥ(R) := lim hseq R` exists (ℝ is complete).

Core API proven here:
  * `canheight_nonneg  : 0 ≤ ĥ(R)`
  * `canheight_dbl     : ĥ(R + R) = 4·ĥ(R)`
  * `canheight_two_pow : ĥ(2ⁿR) = 4ⁿ·ĥ(R)`
  * `canheight_window  : |ĥ(R) − log h(R)| ≤ log 1728 / 3`
  * `finite_of_naiveHeight_le` — Northcott for ℚ
  * `canheight_eq_zero_torsion : ĥ(R) = 0 → R torsion`
  * `canheight_of_torsion      : R torsion → ĥ(R) = 0`
  * `canheight_eq_zero_iff_torsion`

HONEST SCOPE. NO parallelogram law (that is W2, the quasi-parallelogram
Bézout certificates — still computing); hence no quadraticity, no pairing,
no regulator, no rank statement. This stone is exactly: the limit exists,
its doubling law, the window against the naive height, and the
torsion ⟺ ĥ = 0 characterization via Northcott.
...
-/
import PF.DuplicationHeightUpper389a1_r145
import Mathlib.Analysis.SpecificLimits.Basic
import Mathlib.Analysis.SpecialFunctions.Log.Basic

namespace PrincipiaTractalis.CanonicalHeight389a1

open PrincipiaTractalis.NaiveHeightQ
open PrincipiaTractalis.E389a1RankOne
open PrincipiaTractalis.DuplicationHeightUpper389a1
...
(`X O = 0`, `H(0) = 1`, `log 1 = 0`). -/
noncomputable def lognh (R : E389a1.toAffine.Point) : ℝ :=
...
theorem lognh_nonneg (R : E389a1.toAffine.Point) : 0 ≤ lognh R :=
  Real.log_nonneg (one_le_naiveHeight_real _)

lemma X_zero : X (0 : E389a1.toAffine.Point) = 0 := rfl

@[simp] theorem lognh_zero : lognh (0 : E389a1.toAffine.Point) = 0 := by
  unfold lognh
...
/-- The r143 + r145 two-sided window `H⁴/1728 ≤ h(f x/g x) ≤ 17·H⁴`,
...
/-- **The doubling window**: `|log h(R + R) − 4·log h(R)| ≤ log 1728`,
for EVERY point of `E389a1(ℚ)` including `O` (both sides vanish there;
affine points stay affine under doubling — no rational 2-torsion). -/
theorem lognh_dbl_window (R : E389a1.toAffine.Point) :
    |lognh (R + R) - 4 * lognh R| ≤ Real.log 1728 := by
  cases R with
  | zero =>
...
noncomputable def hseq (R : E389a1.toAffine.Point) (n : ℕ) : ℝ :=
  lognh (((2 : ℤ) ^ n) • R) / 4 ^ n

lemma two_pow_succ_zsmul (R : E389a1.toAffine.Point) (n : ℕ) :
    ((2 : ℤ) ^ (n + 1)) • R = ((2 : ℤ) ^ n) • R + ((2 : ℤ) ^ n) • R := by
  have h2 : ((2 : ℤ) ^ (n + 1)) = 2 ^ n + 2 ^ n := by ring
  rw [h2, add_zsmul]

lemma hseq_zero_eq (R : E389a1.toAffine.Point) : hseq R 0 = lognh R := by
  simp [hseq]

theorem hseq_nonneg (R : E389a1.toAffine.Point) (n : ℕ) : 0 ≤ hseq R n :=
  div_nonneg (lognh_nonneg _) (by positivity)

/-- The one-step estimate: `|hseq R (n+1) − hseq R n| ≤ log 1728 / 4^(n+1)`. -/
theorem hseq_step (R : E389a1.toAffine.Point) (n : ℕ) :
    |hseq R (n + 1) - hseq R n| ≤ Real.log 1728 / 4 ^ (n + 1) := by
  ...
  calc |hseq R (n + 1) - hseq R n|
    ...
    _ ≤ Real.log 1728 / 4 ^ (n + 1) := by gcongr

/-- The geometric form of the step estimate, in `dist`. -/
theorem hseq_dist_step (R : E389a1.toAffine.Point) (n : ℕ) :
    dist (hseq R n) (hseq R (n + 1)) ≤ Real.log 1728 / 4 * (1 / 4) ^ n := by
  rw [Real.dist_eq, abs_sub_comm]
  calc |hseq R (n + 1) - hseq R n|
      ≤ Real.log 1728 / 4 ^ (n + 1) := hseq_step R n
    _ = Real.log 1728 / 4 * (1 / 4) ^ n := by
        rw [div_pow, one_pow, div_mul_div_comm, mul_one, ← pow_succ']

/-- **The approximants form a Cauchy sequence** (geometric modulus,
`C = log 1728 / 4`, `r = 1/4`). -/
theorem hseq_cauchySeq (R : E389a1.toAffine.Point) : CauchySeq (hseq R) :=
  cauchySeq_of_le_geometric (1 / 4) (Real.log 1728 / 4) (by norm_num)
    (hseq_dist_step R)

/-! ## §5 — the canonical height -/

/-- **The canonical height** on `E389a1(ℚ)`:
`ĥ(R) = lim_{n → ∞} log h(x(2ⁿR)) / 4ⁿ`. -/
noncomputable def canheight (R : E389a1.toAffine.Point) : ℝ :=
  limUnder atTop (hseq R)

/-- The approximants converge to the canonical height (ℝ is complete). -/
theorem tendsto_hseq (R : E389a1.toAffine.Point) :
    Tendsto (hseq R) atTop (𝓝 (canheight R)) :=
  (hseq_cauchySeq R).tendsto_limUnder

/-- **(a) Nonnegativity**: `0 ≤ ĥ(R)`. -/
theorem canheight_nonneg (R : E389a1.toAffine.Point) : 0 ≤ canheight R :=
  ge_of_tendsto' (tendsto_hseq R) (hseq_nonneg R)

/-- Index shift: `hseq (R + R) n = 4 · hseq R (n + 1)`. -/
lemma hseq_add_self (R : E389a1.toAffine.Point) (n : ℕ) :
...
theorem canheight_dbl (R : E389a1.toAffine.Point) :
...
/-- **(c) The window**: `|ĥ(R) − log h(R)| ≤ log 1728 / 3`
(the geometric tail `Σ (log 1728/4)·(1/4)ⁿ = log 1728/3`). -/
theorem canheight_window (R : E389a1.toAffine.Point) :
    |canheight R - lognh R| ≤ Real.log 1728 / 3 := by
  have hd := dist_le_of_le_geometric_of_tendsto₀ (1 / 4) (Real.log 1728 / 4)
    (by norm_num) (hseq_dist_step R) (tendsto_hseq R)
  rw [Real.dist_eq, hseq_zero_eq] at hd
  have hconst : Real.log 1728 / 4 / (1 - 1 / 4) = Real.log 1728 / 3 := by
    rw [show (1 : ℝ) - 1 / 4 = 3 / 4 by norm_num, div_div]
    norm_num
  rw [hconst] at hd
  calc |canheight R - lognh R| = |lognh R - canheight R| := abs_sub_comm _ _
    _ ≤ Real.log 1728 / 3 := hd

/-- Iterated doubling law: `ĥ(2ⁿR) = 4ⁿ·ĥ(R)`. -/
theorem canheight_two_pow (R : E389a1.toAffine.Point) (n : ℕ) :
...
/-- For fixed `x`, at most two rationals `y` satisfy the 389a1 equation:
subtracting two instances gives `(y − y₀)(y + y₀ + 1) = 0`. -/
theorem finite_equation_fiber (x : ℚ) :
    {y : ℚ | E389a1.toAffine.Equation x y}.Finite := by
  rcases Set.eq_empty_or_nonempty {y : ℚ | E389a1.toAffine.Equation x y}
    with he | hne
...
(Avoids the `(0, 0) ∈ E389a1(ℚ)` collision an `X`-only map would have.) -/
def toCoords : E389a1.toAffine.Point → Option (ℚ × ℚ)
  | .zero => none
...
/-- Points of x-height ≤ 12 land in the fixed finite coordinate set. -/
lemma toCoords_mem_of_le {Q : E389a1.toAffine.Point}
    (hle : naiveHeight (X Q) ≤ 12) :
    toCoords Q ∈ (insert none (Option.some ''
      {p : ℚ × ℚ | naiveHeight p.1 ≤ 12 ∧ E389a1.toAffine.Equation p.1 p.2})
        : Set (Option (ℚ × ℚ))) := by
  cases Q with
...
/-- If `ĥ(R) = 0` then every `2ⁿR` has x-height at most 12:
`log h(2ⁿR) ≤ log 1728 / 3` ⟹ `h³ ≤ 1728` ⟹ `h ≤ 12`. -/
theorem naiveHeight_le_twelve_of_canheight_zero {R : E389a1.toAffine.Point}
    (h0 : canheight R = 0) (n : ℕ) :
    naiveHeight (X (((2 : ℤ) ^ n) • R)) ≤ 12 := by
  have hw := canheight_window (((2 : ℤ) ^ n) • R)
  rw [canheight_two_pow, h0, mul_zero] at hw
  have hl : lognh (((2 : ℤ) ^ n) • R) ≤ Real.log 1728 / 3 := by
    have h := (abs_le.mp hw).1
    linarith
  unfold lognh at hl
  set q : ℚ := X (((2 : ℤ) ^ n) • R) with hq
  have h3 : Real.log ((naiveHeight q : ℝ) ^ 3) ≤ Real.log 1728 := by
    rw [Real.log_pow]
    push_cast
    linarith
  have hcube : ((naiveHeight q : ℝ)) ^ 3 ≤ 1728 :=
    (Real.log_le_log_iff (pow_pos (naiveHeight_real_pos q) 3)
      (by norm_num)).mp h3
  have hnat : naiveHeight q ^ 3 ≤ 1728 := by exact_mod_cast hcube
  by_contra hgt
  push_neg at hgt
  have h13 : 13 ≤ naiveHeight q := hgt
  have h2197 : 2197 ≤ naiveHeight q ^ 3 := by
    calc (2197 : ℕ) = 13 ^ 3 := by norm_num
      _ ≤ naiveHeight q ^ 3 := Nat.pow_le_pow_left h13 3
  exact absurd (le_trans h2197 hnat) (by norm_num)

/-- The doubling orbit of a point with `ĥ = 0` is finite. -/
theorem finite_orbit_of_canheight_zero {R : E389a1.toAffine.Point}
    (h0 : canheight R = 0) :
    (Set.range fun n : ℕ => ((2 : ℤ) ^ n) • R).Finite := by
  have hTfin : {p : ℚ × ℚ | naiveHeight p.1 ≤ 12 ∧
      E389a1.toAffine.Equation p.1 p.2}.Finite := by
    apply Set.Finite.subset (Set.Finite.biUnion (finite_of_naiveHeight_le 12)
  ...
  rintro P ⟨n, rfl⟩
  rw [Set.mem_preimage]
  exact toCoords_mem_of_le (naiveHeight_le_twelve_of_canheight_zero h0 n)
...
theorem canheight_eq_zero_torsion {R : E389a1.toAffine.Point}
...
theorem canheight_of_torsion {R : E389a1.toAffine.Point}
    (h : IsOfFinAddOrder R) : canheight R = 0 := by
  have hfinite : ((AddSubgroup.zmultiples R :
      AddSubgroup E389a1.toAffine.Point) : Set E389a1.toAffine.Point).Finite :=
    h.finite_zmultiples
...
theorem canheight_eq_zero_iff_torsion (R : E389a1.toAffine.Point) :
    canheight R = 0 ↔ IsOfFinAddOrder R :=
  ⟨canheight_eq_zero_torsion, canheight_of_torsion⟩

end PrincipiaTractalis.CanonicalHeight389a1

#print axioms PrincipiaTractalis.CanonicalHeight389a1.lognh_dbl_window
#print axioms PrincipiaTractalis.CanonicalHeight389a1.hseq_cauchySeq
#print axioms PrincipiaTractalis.CanonicalHeight389a1.canheight_nonneg
#print axioms PrincipiaTractalis.CanonicalHeight389a1.canheight_dbl
#print axioms PrincipiaTractalis.CanonicalHeight389a1.canheight_window
#print axioms PrincipiaTractalis.CanonicalHeight389a1.canheight_two_pow
#print axioms PrincipiaTractalis.CanonicalHeight389a1.finite_of_naiveHeight_le
#print axioms PrincipiaTractalis.CanonicalHeight389a1.canheight_eq_zero_torsion
#print axioms PrincipiaTractalis.CanonicalHeight389a1.canheight_of_torsion
#print axioms PrincipiaTractalis.CanonicalHeight389a1.canheight_eq_zero_iff_torsion
