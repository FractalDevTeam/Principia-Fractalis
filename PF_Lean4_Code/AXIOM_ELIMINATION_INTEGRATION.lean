...
theorem encodeConfig_tape_eq (c₁ c₂ : TMConfig)
  ...
  have extract_position : ∀ c : TMConfig, ∀ j : ℕ, j < c.tape.length →
    ...
    admit -- TODO: List decomposition and prime factorization - standard but lengthy
  ...
  admit -- TODO: List comparison via p-adic valuations
...
lemma combine_padicValNat_facts (c : TMConfig) :
  ...
  · have : padicValNat 2 (encodeConfig c) = c.state := by
      ...
      admit -- TODO: Extract from encodeConfig_state_eq proof
  ...
  · have : padicValNat 3 (encodeConfig c) = c.head := by
      ...
      admit -- TODO: Extract from encodeConfig_head_eq proof
    unfold encodeConfig at this
    exact this
  ...
  · intro j hj
    have : padicValNat (nthPrime (j + 2)) (encodeConfig c) =
      ...
      admit -- TODO: Extract from extract_position proof
    unfold encodeConfig at this
    exact this
